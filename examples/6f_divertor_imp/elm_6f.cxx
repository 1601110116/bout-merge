...
bool pos_filter_zf;
BoutReal pos_filter_width, pos_filter_length;
/*****************************************************************/
...
const Field3D lowPass_pos2(const Field3D &var, const Field3D &prof)
{
  ...
  tmp_result = lowPass(var, low_pass_z, 0);
        ...
        for (int jy=0;jy<mesh->ngy;jy++)
	{
	  ...
	  if (keep_zonalPF && ( ((y_ind>jysep1) && (y_ind<=jysep1_2)) || ((y_ind>jysep2_1) && (y_ind<=jysep2)) ) )
          {
            if ( prof2d[jx][jy] < 0. )
	      {
  ...
  for(jx=0;jx<mesh->ngx;jx++)
    {
	  ...
	  if ( (dindx < PF_limit_range) && ((indy <= jysep1) || ((indy > jysep1_2) && (indy <= jysep2_1)) || (indy > jysep2)) )
	    {
	      for(jz=0; jz<mesh->ngz; jz++)
		result[jx][jy][jz] = 0.;
	    }
...
const Field3D sink_zonal_core(const Field3D &var, int filter_index)
{
  ...
  for(jx=0;jx<mesh->ngx;jx++)
	...
	{
	  indy = mesh->YGLOBAL(jy);
	  if ( ((indy > jysep1) && (indy <= jysep1_2)) || ((indy > jysep2_1) || (indy <= jysep2)) )
	    {
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, pos_filter_zf,         false); // /home/users/xiaty/bout_nersc/examples/6f_divertor_imp
