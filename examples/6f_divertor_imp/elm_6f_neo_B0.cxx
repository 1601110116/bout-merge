...
const char CXXVERSION[] = "v3.4.3_090717.3_J.G.Chen";
...
int physics_init(bool restarting) {
  ...
  output.write("                Pbar = %e Pa\n", KB * Tebar * eV_K * Nbar * density);
  ...
  if ((!T0_fake_prof) && n0_fake_prof) {
    if (n0_p0_0p3) {
      ...
      BoutReal P00 = 23072.3; // P0 at magnetic axis, for cbm18_dens6
      P00 = P00 / (KB * (Tebar)*eV_K * Nbar * density);
  ...
  */
  ...
  if (diamag && diamag_phi0) {
    if (experiment_Er) {
      if (diamag_er) {
        ...
        Er0.x = Er_tmp / (B0 * Bbar * Va * Rxy * Bpxy);
        Er0.y = 0.;
        Er0.z = 0.;
        ...
        Er0_balance = Upara0 * Grad(Pi0) / (N0 * B0);
        ...
        Ve0net = ((Er0 - Er0_balance) ^ B0vec) / B0;
        Ve0net.toCovariant();
        ...
        mesh->get(phi0, "Phi_0");
        phi0 /= B0 * Bbar * Lbar * Va;
        // BoutReal N0tmp = max(N0,true);
        // phi0_net =  phi0 + Upara0*Pi0/N0tmp/B0;
        // U0_net = Delp2(phi0_net*B0) * N0tmp/B0;
        U0_net = N0 / B0 * (Delp2(B0 * phi0) + Upara0 * Delp2(Pi0) / N0);
        mesh->communicate(U0_net);
        U0_net.applyBoundary("dirichlet");
        SAVE_ONCE(U0_net);
      }
  ...
  if ((gamma_i_BC > 0.0) && (gamma_e_BC > 0.0)) {
    ...
    Jpar_sh0 *= c_se0 - vth_e0 / (2.0 * sqrt(PI)) * exp(-ee * (phi0 * B0 * Va * Lbar * Bbar) / (KB * Te0 * Tebar * eV_K));
    // Jpar_sh0 *= c_se0 -  vth_e0/(2.0*sqrt(PI)) * ( 1. - ee*(phi0*B0*Va*Lbar*Bbar)/(KB*Te0*Tebar*eV_K) );
...
int physics_run(BoutReal t) {
  ...
  if (impurity_prof) {
    ...
    if (laplace_alpha <= 0.0)
      ...
      phi = invert_laplace(ubyn, phi_flags, NULL, &density_tmp, NULL) / B0;
    ...
    if (laplace_alpha <= 0.0)
      ...
      phi = invert_laplace(ubyn, phi_flags, NULL, &N0, NULL) / B0;
    // output.write("I see you 0.21! \n");//xia
  }
  ...
  if ((gamma_i_BC > 0.0) && (gamma_e_BC > 0.0)) {
    ...
    if (nonlinear) {
      ...
      Jpar_sh *= c_set - vth_et / (2.0 * sqrt(PI)) * exp(-ee * ((phi + phi0) * Va * Lbar * Bbar * B0) / (KB * Te_tmp * Tebar * eV_K));
      // Jpar_sh *= c_set -  vth_et/(2.0*sqrt(PI)) * (1. - ee*((phi+phi0)*Va*Lbar*Bbar*B0)/(KB*Te_tmp*Tebar*eV_K) );
      Jpar_sh -= Jpar_sh0;
      ...
      Jpar_sh *= vth_et / (2.0 * sqrt(PI)) * (ee * ((phi)*Va * Lbar * Bbar * B0) / (KB * Te0 * Tebar * eV_K));
      // Jpar_sh -= Jpar_sh0;
      Jpar_sh *= MU0 * Lbar / (B0 * Bbar);
      SBC_Dirichlet(Jpar, Jpar_sh, PF_limit, PF_limit_range);
    }
  ...
  if (compress0) {
    if (nonlinear)
      // Ni*Zi + Nimp*Zimp = Ne
      // if (impurity_prof)
      // Vepar = Vipar*N_tmp*Zi/Ne_tmp + Vipar*N_imp0*Z_imp/Ne_tmp - B0 * (Jpar) / Ne_tmp * Vepara;
      // else
      Vepar = Vipar - B0 * (Jpar) / Ne_tmp * Vepara;
    else
      // if (impurity_prof)
      // Vepar = Vipar*N0*Zi/Ne0 + Vipar*N_imp0*Z_imp/Ne0 - B0 * (Jpar) / Ne0 * Vepara;
      // else
  ...
  if (output_ohm) {
    ohm_phi = -Grad_parP(B0 * phi, CELL_CENTRE) / B0 - bracket(B0 * phi0, Psi, bm_exb);
  ...
  if (experiment_Er && KH_term)
    ddt(U) -= bracket(B0 * phi, U0_net, bm_exb);
  ...
  if (gyroviscous) {
    ...
    test_er0 = -Div( B0vec^Ve0 ) + Div(B0vec)*(B0vec*Er0*B0);
    // test_phi0 = bracket(phi0, U, bm_exb)*B0;
    // test_er0 = V_dot_Grad(Ve0, U);
    ...
    if (diamag_er && diamag_phi0)
      ...
      GradcPhi = (Ve0 ^ B0vec) * Grad_perp(B0 * phi) / (B0 * B0);
    else
    ...
    if (impurity_prof && impurity_gyro) {
      ...
      bracketPhiPimp0 = bracket(B0 * phi, P_imp0, bm_exb);
      ...
      ddt(U) += 0.5 * Upara_imp * Upara2 * bracket(B0 * phi, Dperp2Pimp0, bm_exb) / B0;
  ...
  if (output_transfer) {
    ...
    T_G += 0.5 * Upara2 * bracket(B0 * phi, Dperp2Pi, bm_exb) / B0;
  ...
  if (continuity) {
    ddt(Ni) -= 2.0 * N0 / B0 * b0xcv * Grad(B0 * phi);
    if (experiment_Er)
      // ddt(Ni) -= 2.0 * Ni/B0 *  b0xcv*Grad(B0*phi0);
      // ddt(Ni) -= 2.0 * Nipara1 * b0xcv*Grad(Pi0) / N0/B0;    // balanced by above
      ddt(Ni) -= 2.0 * Ni / B0 * b0xcv * (Ve0net ^ B0vec); // net flow
    if (diamag) {
      // ddt(Ni) -= 2.0 * Ni/B0 *  b0xcv*Grad(phi0*B0);
      ddt(Ni) -= 2.0 * Nipara1 * b0xcv * Grad(Pi) / B0;
    }
    if (nonlinear) {
      ddt(Ni) -= 2.0 * Ni / B0 * b0xcv * Grad(B0 * phi);
    }
  }
  ...
  */
  ...
  if (diamag && diamag_phi0) {
    if (diamag_er)
      ...
      ddt(Ti) -= bracket(B0 * phi0, Ti, bm_exb); // Equilibrium flow
  }

  if (nonlinear) {
    ddt(Ti) -= bracket(B0 * phi, Ti, bm_exb); // Advection
  }
  ...
  if (continuity) {
    ddt(Te) -= 4.0 / 3.0 * Te0 / B0 * b0xcv * Grad(B0 * phi);
    // net flow: Ve0dia is not balanced by Grad(Pe0) part
    ...
    if (nonlinear) {
      ddt(Te) -= 4.0 / 3.0 * Te / B0 * b0xcv * Grad(B0 * phi);
      if (diamag)
  ...
  if (output_vradial) {
    if (diamag_er)
      Vexb = Ve0 + (B0vec ^ Grad(B0 * phi)) / (B0 * B0);
    else
      Vexb = (B0vec ^ Grad(B0 * (phi + phi0))) / (B0 * B0);
...
int precon(BoutReal t, BoutReal gamma, BoutReal delta) {
  ...
  // TODO: diamag in U?
  phi_tmp = invert_laplace(U * B0 / Ntemp, phi_flags, NULL) / B0;
  mesh->communicate(phi_tmp);
  phi_tmp.applyBoundary();
  Ni = ni_tmp - gamma * bracket(B0 * phi_tmp, N0, bm_exb);
  Ti = ti_tmp2 - gamma * bracket(B0 * phi_tmp, Ti0, bm_exb);
  Te = te_tmp2 - gamma * bracket(B0 * phi_tmp, Te0, bm_exb);
  Psi = psi_tmp - gamma * Grad_par(B0 * phi_tmp) / B0;
