...
PRO process_grid, rz_grid, mesh, output=output, poorquality=poorquality, $
  ...
  pressure = p0_smooth(pressure)  ;xiaty
...
;      fpol = deriv_smooth(fpol)    ;xiaty
      fprime[xi,yi] = 0.  
;      fpol = SPLINE(rz_grid.npsigrid, rz_grid.fpol, mesh.psixy[xi,yi[0]], /double)   ;xiaty
;      fprime[xi,yi] = SPLINE(rz_grid.npsigrid, fp, mesh.psixy[xi,yi[0]], /double)  ;xiaty
    ENDELSE
;    fprime = fprime_smooth(fprime)    ;xiaty
  ...
  REPEAT BEGIN
   ...
   ; IF NOT period THEN jpar0[xi,yi] = 0.0  ;xiaty
  ENDREP UNTIL last
...
;  s = file_write(handle, "fpol", fpol)
;  s = file_write(handle, "fprime", fprime)
