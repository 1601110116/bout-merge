...
//output << max(c_se0, true) << "\t" << max(vth_e0, true) << endl;
      ...
      Jpar_sh0 *= c_se0 -  vth_e0/(2.0*sqrt(PI)) * exp(-3.0);
...
//  output.write("I see you 2! \n");//xia
  ...
  if ( (gamma_i_BC > 0.0) && (gamma_e_BC > 0.0) )
      ...
      if (nonlinear)
	{
	  ...
	  Jpar_sh *= c_set -  vth_et/(2.0*sqrt(PI)) * exp( - ee*((phi)*Va*Lbar*Bbar)/(KB*Te_tmp*Tebar*eV_K) -3.0*Te0/Te_tmp );
  ...
  if (continuity)
    {
      ...
      if (experiment_Er)
        ...
        // ddt(Ni) -= 2.0 * Nipara1 * b0xcv*Grad(Pi0) / B0;    // balanced by above
        ddt(Ni) -= 2.0 * Ni/B0 *  b0xcv* (Ve0net^B0vec)/ (B0*B0);      // net flow
      if (diamag)
	{
  ...
  if (continuity)
    {
      ...
      if (experiment_Er)
        ...
        // ddt(Ti) -= 4.0/3.0 * Tipara2 * Ti/N0 * b0xcv*Grad(Pi0) / B0;   // balanced by above
        ddt(Ti) -=4.0/3.0 * Ti/B0 * b0xcv* (Ve0net^B0vec) / (B0*B0);                  // net flow
      if (diamag)
  ...
  if (continuity)
    {
      ...
      ddt(Te) -= 4.0/3.0 * Te/B0 * b0xcv * (Ve0^B0vec) / (B0*B0);
