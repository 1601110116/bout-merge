...
const char CXXVERSION[] = "v3.4.4_090717.5_J.G.Chen";
...
int physics_run(BoutReal t) {
  ...
  if (!emass) {
    ...
    if (spitzer_resist)
      ddt(Psi) = -Grad_parP(phi, CELL_CENTRE) / B0 - eta_spitzer * Jpar / B0;
    else
    ...
    */
    ...
    if (spitzer_resist)
      ddt(Ajpar) = -Grad_parP(phi, CELL_CENTRE) / B0 - eta_spitzer * Jpar / B0;
    else
  ...
  */
  ...
  if (energy_exch) {
    ...
    if (spitzer_resist) {
      if (BScurrent)
        ddt(Te) += 4.0 / 3.0 * Tepara4 * eta_spitzer * Jpar_BS0 * Jpar / Ne0;
      else
        ddt(Te) += 4.0 / 3.0 * Tepara4 * eta_spitzer * J0 * Jpar / Ne0;
    } else {
      if (BScurrent)
    ...
    if (nonlinear) {
      if (spitzer_resist)
        ddt(Te) += 2.0 / 3.0 * Tepara4 * eta_spitzer * Jpar * Jpar / Ne0;
      else
