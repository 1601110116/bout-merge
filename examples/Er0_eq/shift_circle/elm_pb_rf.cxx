...
bool iterative_phi0;
Field2D phi00, phi01, U00, eta_sh, phish0;
Field2D term1, term2, term3, term4, term5, term6;
...
*/
  ...
  if (iterative_phi0)
    {
      phish0 = RF_coef*0.5*log((Mi/Me)/(4.0*PI)*(Te0*Tebar/(Ti0*Tibar+Te0*Tebar)))*(Te0*Tebar*KB*eV_K)/ee;
      phish0 /= Va*Lbar*Bbar;
      SAVE_ONCE(phish0);
...
int physics_run(BoutReal t)
{
  ...
  if (iterative_phi0)
	...
	//phi01 = phi0*B0*sbc_lambda1;
      ...
      ddt(U00) = -(B0/(eta_sh*sbc_lambda1))*Grad2_par2_sh(phi01, phish0*sbc_lambda1)
	+ (Mu_perp/(Va*Lbar))*Delp2(U00)
	+ (Mu_para/(Va*Lbar))*Grad2_par2(U00);
      
      ddt(phi01) = (sbc_lambda0*N0)/(sbc_lambda1*B0)*Delp2(phi01)
	+ (sbc_lambda0*Bbar)/(2.0*MU0*Nbar*density*Va*Lbar*Zeff*ee*B0)*Delp2(P0/2.0)
      	- sbc_lambda0*U00;
      
      phi00=phi01/sbc_lambda1;
      
      term1 = -(B0/(eta_sh*sbc_lambda1))*Grad2_par2_sh(phi01, phish0*sbc_lambda1);
      mesh->communicate(term1);
      term1.applyBoundary();
      
      term2 = (Mu_perp/(Va*Lbar))*Delp2(U00);
      mesh->communicate(term2);
      term2.applyBoundary();
      
      term3 = (Mu_para/(Va*Lbar))*Grad2_par2(U00);
      mesh->communicate(term3);
      term3.applyBoundary();
      
      term4 = (sbc_lambda0*N0)/(sbc_lambda1*B0)*Delp2(phi01);
      mesh->communicate(term4);
      term4.applyBoundary();
      
      term5 = (sbc_lambda0*Bbar)/(2.0*MU0*Nbar*density*Va*Lbar*Zeff*ee*B0)*Delp2(P0/2.0);
