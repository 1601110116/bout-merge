...
bool iterative_phi0;
Vector2D er00, er0, ersh0; 
Field2D phi00, phi01, U00, eta_sh, phish0, phish1, dphi01dx, d2phishdx2;
Field2D term1, term2, term3, term4, term5, term6, term7, term8;
...
BoutReal ehyperviscos;   // electron Hyper-viscosity coefficie
...
void SBC_Dirichlet(Field2D &var, const Field2D &value);
void SBC_yup_eq(Field2D &var, const Field2D &value);
void SBC_ydown_eq(Field2D &var, const Field2D &value);
Field2D Grad2_par2_sh (const Field2D &var, const Field2D &value);
Field2D Laplace_par_filterPF (const Field2D &var);
Field2D Laplace_perp_filterPF (const Field2D &var);
Field2D Laplace_perp_PFboundary (const Field2D &var);
//***********************************************guiend
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, iterative_phi0,          false);
  ...
  */
  ...
  dump.add(mesh->G1, "mg1", 0);
  dump.add(mesh->G2, "mg2", 0);
  dump.add(mesh->g11, "mg11", 0);
  dump.add(mesh->g12, "mg12", 0);
  dump.add(mesh->g13, "mg13", 0);
  dump.add(mesh->g22, "mg22", 0);
  dump.add(mesh->g23, "mg23", 0);
  dump.add(mesh->g33, "mg33", 0);
  dump.add(mesh->J, "mgj", 0);
  dump.add(mesh->g_11, "mg_11", 0);
  dump.add(mesh->g_12, "mg_12", 0);
  dump.add(mesh->g_13, "mg_13", 0);
  dump.add(mesh->g_22, "mg_22", 0);
  dump.add(mesh->g_23, "mg_23", 0);
  dump.add(mesh->g_33, "mg_33", 0);
  ...
  if(!restarting) {
	...
	dphi01dx.setLocation(CELL_CENTRE);
	dphi01dx = DDX(phi01);
	SAVE_ONCE(dphi01dx);
      }
  }
  ...
  if (iterative_phi0)
    {
      phish0 = sbc_lambda1*0.5*log((Mi/Me)/(4.0*PI)*(Te0*Tebar/(Ti0*Tibar+Te0*Tebar)))*(Te0*Tebar*KB*eV_K)/ee;
      phish0 /= Va*Lbar*Bbar;
      //mesh->get(phish0,"Phish0");
      SAVE_ONCE(phish0);

      (er0.x).setLocation(CELL_CENTRE);
      (er0.y).setLocation(CELL_CENTRE);
      er0 = -1.0*Grad(phi0*B0);
      mesh->communicate(er0.x);
      mesh->communicate(er0.y);
      (er0.x).applyBoundary("neumann");
      (er0.y).applyBoundary("neumann");
      er0.toCovariant();
      er0.x *= Rxy*Bpxy;
      ...
      ersh0.toCovariant();
      ersh0.x *= Rxy*Bpxy;
      dump.add(ersh0.x, "ersh0x",0);
      dump.add(ersh0.y, "ersh0y",0);

      phish1 = RF_coef*phish0;
      mesh->communicate(phish1);
      SAVE_ONCE(phish1); 
      
      eta_sh.setLocation(CELL_CENTRE);
      eta_sh=eta.DC();
      SAVE_ONCE(eta_sh);
      
      d2phishdx2.setLocation(CELL_CENTRE);
      //d2phishdx2 = -(Bbar*sbc_lambda1)/(2.0*MU0*N0*Nbar*density*Va*Lbar*Zeff*ee)*Laplace_perp(P0/2.0);
      d2phishdx2 = DDX(phish0); 
      mesh->communicate(d2phishdx2);
      (d2phishdx2).applyBoundary();
      SAVE_ONCE(d2phishdx2);
      ...
      (er00.x).setLocation(CELL_CENTRE);
      (er00.y).setLocation(CELL_CENTRE);
      (er00.x).setBoundary("p");
      (er00.y).setBoundary("p");
      dump.add(er00.x, "er00x",1);
      dump.add(er00.y, "er00y",1);
            
      term1.setLocation(CELL_CENTRE);
      term1.setBoundary("phi");
      dump.add(term1, "term1", 1);
      
      term2.setLocation(CELL_CENTRE);
      term2.setBoundary("U00");
      dump.add(term2, "term2", 1);
      
      term3.setLocation(CELL_CENTRE);
      term3.setBoundary("U00");
      dump.add(term3, "term3", 1);
      
      term4.setLocation(CELL_CENTRE);
      term4.setBoundary("phi");
      dump.add(term4, "term4", 1);
      
      term5.setLocation(CELL_CENTRE);
      term5.setBoundary("P");
      dump.add(term5, "term5", 1);
      
      term6.setLocation(CELL_CENTRE);
      term6.setBoundary("U00");
      dump.add(term6, "term6", 1);
      
      term7.setLocation(CELL_CENTRE);
      term7.setBoundary("P");
      dump.add(term7, "term7", 1);
      
      term8.setLocation(CELL_CENTRE);
      term8.setBoundary("P");
      dump.add(term8, "term8", 1);
    }
...
int physics_run(BoutReal t)
{
  ...
  if (iterative_phi0)
    {
      mesh->communicate(U00);
      U00.applyBoundary();
      
      mesh->communicate(phi01);
      phi01.applyBoundary();
      SBC_Dirichlet(phi01, phish0); 
      
      ddt(U00) = -(B0/(eta_sh*sbc_lambda1))*Grad2_par2_sh(phi01,phish1)
	+ ((Mi*B0)/(2.0*Bbar*Tbar*eta_sh*ee*N0))*Laplace_par(P0/2.0)
	+ ((0.71*KB*eV_K*Tebar*B0)/(Bbar*Va*Lbar*eta_sh*ee))*Laplace_par(Te0)
	+ (Mu_perp/(Va*Lbar))*Laplace_perp(U00)
	+ (Mu_para/(Va*Lbar))*Laplace_par(U00);
      
      ddt(phi01) = (sbc_lambda0*N0)/(sbc_lambda1*B0)*Laplace_perp(phi01)
	+ (sbc_lambda0*Bbar)/(2.0*MU0*Nbar*density*Va*Lbar*Zeff*ee*B0)*Laplace_perp(P0/2.0)
      	- sbc_lambda0*U00;
      ...
      term1 = -(B0/(eta_sh*sbc_lambda1))*Grad2_par2_sh(phi01,phish1);
      mesh->communicate(term1);
      term1.applyBoundary();
      
      term7 = ((Mi*B0)/(2.0*Bbar*Tbar*eta_sh*ee*N0))*Laplace_par(P0/2.0);
      mesh->communicate(term7);
      term7.applyBoundary();
      
      term8 = ((0.71*KB*eV_K*Tebar*B0)/(Bbar*Va*Lbar*eta_sh*ee))*Laplace_par(Te0);
      mesh->communicate(term8);
      term8.applyBoundary();

      term2 = (Mu_perp/(Va*Lbar))*Laplace_perp(U00);
      mesh->communicate(term2);
      term2.applyBoundary();
      
      term3 = (Mu_para/(Va*Lbar))*Laplace_par(U00);
      mesh->communicate(term3);
      term3.applyBoundary();
      
      term4 = (sbc_lambda0*N0)/(sbc_lambda1*B0)*Laplace_perp(phi01);
      mesh->communicate(term4);
      term4.applyBoundary();
      
      term5 = (sbc_lambda0*Bbar)/(2.0*MU0*Nbar*density*Va*Lbar*Zeff*ee*B0)*Laplace_perp(P0/2.0);
      mesh->communicate(term5);
      term5.applyBoundary();
      
      term6 = -sbc_lambda0*U00;
      mesh->communicate(term6);
      term6.applyBoundary();
  ...
  if(evolve_jpar) {
    ...
    ddt(Jpar) = 0.0;// -Grad_parP(B0*U, CELL_YLOW) / B0 + eta*Delp2(Jpar);
    ...
    */
    ...
    if(relax_j_vac) {
      ...
      ddt(Jpar) -=  0.0;// vac_mask * Jpar / relax_j_tconst;
    }
  }else {
    // Vector potential
    
    if(SBC_phi && test_SBC) {
      ddt(Psi) =  0.0;// -Grad_parP_dir(B0*phi,B0*phi_up, B0*phi_dw, CELL_CENTRE) / B0 + eta*Jpar;
    } else 
      {      
	ddt(Psi) =  0.0;// -Grad_parP(B0*phi, CELL_CENTRE) / B0 + eta*Jpar;
	//xqx      ddt(Psi) = -Grad_parP(B0*phi, CELL_YLOW) / B0 + eta*Jpar;
      }
    
    if(eHall) {
      ddt(Psi) +=   0.0;// 0.25*delta_i*(Grad_parP(B0*P, CELL_CENTRE) / B0 
      //                         +b0xGrad_dot_Grad(P0, Psi));   // electron parallel pressure
    }
    
    if(diamag_phi0)
      ddt(Psi) -=  0.0;// b0xGrad_dot_Grad(phi0*B0, Psi)/B0;   // Equilibrium flow
    
    if(withflow)                                //net flow 
      ddt(Psi)-=  0.0;// V_dot_Grad(V0net, Psi);
    ...
    if(diamag_grad_t) {
      ...
      ddt(Psi) +=  0.0;// 1.71 * dnorm * 0.5 * Grad_parP(P, CELL_YLOW) / B0;
    }
	...
	ddt(Psi) -=  0.0;// eta*hyperresist * Delp2(Jpar);
      }else {
    ...
    if(ehyperviscos > 0.0) {
      ddt(Psi) -=  0.0;// eta*ehyperviscos * Delp2(Jpar2);
    }
    ...
    if(diffusion_a4 > 0.0) {
      ...
      tmpA2.applyBoundary();
      ddt(Psi) -=  0.0;// diffusion_a4 * Grad2_par2new(tmpA2);
    }
    ...
    if(relax_j_vac) {
      ...
      ddt(Psi) =  0.0;// ddt(Psi)*(1. - vac_mask) - (Psi - Psitarget)*vac_mask / relax_j_tconst;
    }
  ...
  ddt(U) =  0.0;// (B0^2) * b0xGrad_dot_Grad(Psi + rmp_Psi, J0, CELL_CENTRE); // Grad j term
  
  ddt(U) +=  0.0;// b0xcv*Grad(P);  // curvature term
  
  if(!nogradparj && SBC_phi && test_SBC) {
    // Parallel current term
    ddt(U) -=  0.0;// (B0^2) * Grad_parP_dir(Jpar, jpar_up, jpar_dw, CELL_YLOW); // b dot grad j
    if (K_H_term) ddt(U) -=  0.0;// b0xGrad_dot_Grad(phi,U0); 
  } else {
    // Parallel current term
    ddt(U) -=  0.0;// (B0^2)*Grad_parP(Jpar, CELL_CENTRE); // b dot grad j
    if (K_H_term) ddt(U) -= 0.0;// b0xGrad_dot_Grad(phi,U0);
  }
  
  if(withflow&&K_H_term)                         //K_H_term
    ddt(U) -= 0.0;// b0xGrad_dot_Grad(phi,U0); 
  
  if(diamag_phi0)
    ddt(U) -=  0.0;// b0xGrad_dot_Grad(phi0*B0, U)/B0;   // Equilibrium flow
  
  if(withflow)                            // net flow
    ddt(U) -=  0.0;// V_dot_Grad(V0net, U);

  if(nonlinear) {
    ddt(U) -=  0.0;// bracket(phi, U, bm_exb)*B0;    // Advection
  }
  
  // Viscosity terms 
  if(viscos_par > 0.0)
    ddt(U) +=  0.0;// viscos_par * Grad2_par2(U); // Parallel viscosity
  ...
  if(diffusion_u4 > 0.0) {
    ...
    ddt(U) -=  0.0;// diffusion_u4 * Grad2_par2new(tmpU2);
  }
  
  if(viscos_perp > 0.0)
    ddt(U) +=  0.0;// viscos_perp * Delp2(U);     // Perpendicular viscosity
  ...
  if(hyperviscos > 0.0) {
    ...
    ddt(U) +=  0.0;// hyper_mu_x * mesh->g11*D2DX2(U);
  ...
  if(gyroviscous)
    {
      ...
      ddt(U) -=  0.0;// 0.5*Upara2*bracket(Pi, Dperp2Phi0, bm_exb)/B0;
      ddt(U) -=  0.0;// 0.5*Upara2*bracket(Pi0, Dperp2Phi, bm_exb)/B0;
      ddt(U) +=  0.0;// 0.5*Upara2*bracket(B0*phi, Dperp2Pi0, bm_exb)/B0;
      ddt(U) +=  0.0;// 0.5*Upara2*bracket(B0*phi0, Dperp2Pi, bm_exb)/B0;
      ddt(U) -=  0.0;// 0.5*Upara2*Delp2(bracketPhi0P)/B0;
      ddt(U) -=  0.0;// 0.5*Upara2*Delp2(bracketPhiP0)/B0;
      ...
      if (nonlinear)
	{
	  ...
	  ddt(U) -=  0.0;// 0.5*Upara2*bracket(Pi, Dperp2Phi, bm_exb)/B0;
	  ddt(U) +=  0.0;// 0.5*Upara2*bracket(B0*phi, Dperp2Pi, bm_exb)/B0;
	  ddt(U) -=  0.0;// 0.5*Upara2*Delp2(bracketPhiP)/B0;
	}
    }

  // left edge sink terms 
  if(sink_Ul > 0.0){
    ddt(U) -=   0.0;// sink_Ul*sink_tanhxl(P0,U,su_widthl,su_lengthl); // core sink
  }
  
  // right edge sink terms 
  if(sink_Ur > 0.0){
    ddt(U) -=  0.0;//  sink_Ur*sink_tanhxr(P0,U,su_widthr,su_lengthr); //  sol sink
  }
  ...
  ddt(P) = 0.0;
  if(evolve_pressure) {
    ddt(P) -=  0.0;// b0xGrad_dot_Grad(phi, P0);
    
    if(diamag_phi0)
      ddt(P) -=  0.0;// b0xGrad_dot_Grad(phi0*B0, P)/B0;   // Equilibrium flow
    
    if(withflow)                              //net flow
      ddt(P) -=  0.0;// V_dot_Grad(V0net, P);  
    
    if(nonlinear)
      ddt(P) -=  0.0;// bracket(phi, P, bm_exb)*B0;    // Advection      
  }
  ...
  if(diffusion_par > 0.0)
    {
      ...
      ddt(P) +=  0.0;// 2. / 3. * kappa_par * Grad2_par2(P);
    }
  ...
  if(diffusion_perp > 0.0)
      ...
      if(diffusion_coef_Hmode)
	{
	  ...
	  ddt(P) +=  0.0;// 2. / 3.* kappa_perp * D2DX2(P) + 2. / 3.* DDX(kappa_perp) * DDX(P);
	}
      else
        ddt(P) +=  0.0;// 2. / 3. * kappa_perp * D2DX2(P)+ 2. / 3.* DDX(kappa_perp) * DDX(P);
    }
  ...
  if(diffusion_p4 > 0.0) {
    ...
    tmpP2.applyBoundary();
    ddt(P) =  0.0;// diffusion_p4 * Grad2_par2new(tmpP2);
  }
  
  // heating source terms 
  if(heating_P > 0.0){
    BoutReal pnorm = P0[0][0];
    ddt(P) +=  0.0;// heating_P*source_expx2(P0,2.*hp_width,0.5*hp_length)*(Tbar/pnorm); // heat source
    ddt(P) +=  0.0;// (100.*source_tanhx(P0,hp_width,hp_length)+0.01) * mesh->g11 * D2DX2(P) * (Tbar/Lbar/Lbar) ;     // radial diffusion
  }
  
  // sink terms 
  if(sink_P > 0.0){
    ddt(P) -=  0.0;// sink_P*sink_tanhxr(P0,P,sp_width,sp_length)*Tbar; // sink
  }
  ...
  if(compress0) {
    ...
    ddt(P) -=  0.0;// beta*Div_par_CtoL(Vpar);
    
    if(phi_curv) {
      ddt(P) -=  0.0;// 2.*beta*b0xcv*Grad(phi);
    }
    
    // Vpar equation
    
    //ddt(Vpar) = -0.5*Grad_parP(P + P0, CELL_YLOW);
    ddt(Vpar) =  0.0;// -0.5*Grad_par_LtoC(P + P0);
    
    if(nonlinear)
      ddt(Vpar) -=  0.0;// bracket(phi, Vpar, bm_exb)*B0; // Advection
  }
  ...
  if(low_pass_z > 0) {
    ...
    if(evolve_jpar) {
      ddt(Jpar) =  0.0;// lowPass(ddt(Jpar), low_pass_z, zonal_field);
    }else
      ddt(Psi) =  0.0;// lowPass(ddt(Psi), low_pass_z, zonal_field);
    
    ddt(U) =  0.0;// lowPass(ddt(U), low_pass_z, zonal_flow);
    ddt(P) =  0.0;// lowPass(ddt(P), low_pass_z, zonal_bkgd);
  }
...
void SBC_Dirichlet(Field2D &var, const Field2D &value) //let the boundary equall to the value next to the boundary
{
  ...
  for (; !xrup.isDone(); xrup++)  
      ...
      if (x[xind][0] >= xloc)
	{
	  for(int jy=mesh->yend+1; jy<mesh->ngy; jy++)
	    {
	      var[xind][jy] = value[xind][mesh->yend];
	    }
	}
    }
}
  ...
  for (; !xrdn.isDone(); xrdn++)
      ...
      if (x[xind][0] >= xloc)
	{
	  for(int jy=mesh->ystart-1; jy>=0; jy--)
	    {
	      var[xind][jy] = value[xind][mesh->ystart];
	    }
	}
    }
}
  ...
  for(int jy=0;jy<mesh->ngy;jy++)
      ...
      if ((indy <= jysep1) || (indy > jysep2))
	{
	  for(int jx=0;jx<mesh->ngx;jx++)
		...
		{
		  result[jx+1][jy] = 2.0*result[jx+2][jy]-result[jx+3][jy];
		  result[jx][jy] = 2.0*result[jx+1][jy]-result[jx+2][jy];
		  
		  result[jx-2][jy] = 2.0*result[jx-3][jy]-result[jx-4][jy];
		  result[jx-1][jy] = 2.0*result[jx-2][jy]-result[jx-3][jy];
		}
	    }
	}
	  ...
	  for (int jy=mesh->ystart; jy<=mesh->yend; jy++)
	    {
	      ...
	      if (mgy == yloc-1)
		result[jx][jy] = -var[jx][jy-2]/12.0 + 4.0*var[jx][jy-1]/3.0 - 2.5*var[jx][jy] + 4.0*var[jx][jy+1]/3.0 - value[jx][jx+2]/12.0;
	      
	      if (mgy == yloc)
		result[jx][jy] = -var[jx][jy-2]/12.0 + 4.0*var[jx][jy-1]/3.0 - 2.5*var[jx][jy] + 4.0*value[jx][jy+1]/3.0 - value[jx][jx+2]/12.0;

	      if (mgy == yloc+1)
		result[jx][jy] = -value[jx][jy-2]/12.0 + 4.0*value[jx][jy-1]/3.0 - 2.5*var[jx][jy] + 4.0*var[jx][jy+1]/3.0 - var[jx][jx+2]/12.0;

	      if (mgy == yloc+2)
		result[jx][jy] = -value[jx][jy-2]/12.0 + 4.0*var[jx][jy-1]/3.0 - 2.5*var[jx][jy] + 4.0*var[jx][jy+1]/3.0 - var[jx][jx+2]/12.0;
	    }
	}
    }
