...
//  Ti_exp *=1000;
  mesh->get(Ne_exp, "Neexp");    // hl2a 10^19/m^3
//  Ne_exp *=10;
  mesh->get(Ni_exp, "Niexp");    // hl2a 10^19/m^3
//  Ni_exp *=10;
...
//////////// CALCULATE PARAMETERS ///////////////////
 ...
 if(!include_curvature)
  ...
  Te_grid /= Te_x;
  Ti_grid /= Te_x;
  Ni_grid =Ni_grid/Ni_x;  //hl2a grid in unit 10^19/m^3 
...
//  ddt(U00)=0.0;
    ddt(phi01) = (sbc_lambda0*Ni)/(sbc_lambda1*B0)*Laplace_perp(phi01)
             + (sbc_lambda0*Bbar)/(2.0*MU0*Ni_x*density_unit*Lbar/tbar*Lbar*Zi*ee*B0)*Laplace_perp(Pi)
...
//  output.write("Now updata Vi \n");
  ddt(U00) =  -(B0/(eta*sbc_lambda1))*Laplace_par(phi01)
              + ((Mi2*B0)/(2.0*Bbar*tbar*eta*ee*Ni))*Laplace_par(Pe)
              + ((0.71*KB*eV_K*Te_x*B0)/(Bbar*Lbar/tbar*Lbar*eta*ee))*Laplace_par(Te)
              + (Mu_perp/(Lbar/Tbar*Lbar))*Laplace_perp(U00)
              + (Mu_para/(Lbar/tbar*Lbar))*Laplace_par(U00);

  ddt(U00) += 2.0*KB*Te_x*eV_K/(Mi*Lbar*Lbar/(tbar*tbar))*b0xcv*Grad(pei);  // curvature term

      er0 = -1.0*Bbar/(2.0*MU0*Ni*Ni_x*density_unit*Lbar/tbar*Lbar*Zi*ee)*Grad(Pi);
 ...
 //     SBC_Dirichlet(phi01, phish0);
      SBC_yup_eq(phi01, phish0);    
      SBC_ydown_eq(phi01,phish0);  
  //    Grad2_par2_sh(phi01,phish1);
//*************************************************************liend   
