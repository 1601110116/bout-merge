...
Field2D psixy;
// 3D evolving fields
Field3D  Te, Ni, Vi, Ti, Nn,Tn, Vn, Nli ,Vlix,Nli_1,Vli_1,Tli_1;             // Vi and Vn are paralell velocity
Field3D temp_Ni,temp_Nn,temp_Nli,temp_Ti,temp_Tli1,temp_Te,temp_Nli_1;         // temporary variables for minivalue protection

// 3D initial profiles Hmode
Field3D Ni0_Hmode,Ti0_Hmode,Te0_Hmode;

// Non-linear coefficientsx
Field3D kappa_Te, kappa_Ti,kappa_Tli1;
Field3D kappa_Te_fl,kappa_Ti_fl,kappa_Tli1_fl;                         // heat flux limit coefficients 

// Thermal Ion and Electron Speed
Field3D V_th_i,V_th_e,V_th_n,V_th_li1;

//ionization/CX/rates for Plasmas and Neutrals
Field3D nu_ionz,nu_CX,nu_ionz_n,nu_CX_n,nu_ionli1,nu_rec,nu_recli1;

// Collisional time between electron and ions
Field3D tau_ei,tau_eli1,tau_ili1;

//ion viscosity
Field3D eta0_i,eta0_n,eta0_li1;
...
// Source Terms
Field3D Si_p,Si_CX,Si_li1,S_rec,S_recli1;
...
// pressures
Field3D  Pe,Pi,pei,pn,pm,pli1,pi;

// parallel pressure gradient
Field3D Grad_par_pei,Grad_par_pli1,Grad_par_pi,Grad_par_pn;
...
// parameters
BoutReal Te_x, Ti_x,Tm_x, Ni_x, Vi_x, bmag, rho_s, fmei,fmei_li1, AA,ll, ZZ,Zli;
BoutReal Zi;                  // charge number of ion same as ZZ
BoutReal minimum_val;
BoutReal Mli,Mi,Mn;
BoutReal W_ionz,W_ionli1,W_diss,W_bind,W_rec;
BoutReal Lbar,tbar,Lp,Lp_crit,Lnn_min;
BoutReal lambda_ei, lambda_ii,lambda_eli,lambda_ili;
BoutReal N_cubic_Lbar;           //particle numbers in cubic of Lbar^3
BoutReal nu_hat, mui_hat, wci, nueix, nuiix,nuilix,nuelix;
...
int physics_init(bool restarting)
{
  ...
  output.write("Solving transport equations for Plasmas: Ni, Vi, Ti, Te and li ions: Nli,Nli_1,Tli_1,Vli,Vli_1  \n");
...
// Load 2D profiles
  ...
  OPTION(options, ll, 7.0);
  OPTION(options, ZZ, 1.0);
  OPTION(options, Zli, 3.0);
  Zi=ZZ;                                   // Zi is applied in BS_current from Tianyang

  OPTION(options, Mi, 1.0);        // Read in Mi
  OPTION(options, Mn, 1.0);        // Read in Mi
  OPTION(options, Mli, 7.0);        // Read in Mi
  OPTION(options, W_ionz, 4.3);    // Read in eV
  OPTION(options, W_ionli1, 4.3);    // Read in eV
  ...
  OPTION(options, Vli0,   -500.0);    // Read in m/s
  //Vm0 = 0.0;
  OPTION(options, Nli0,  1.e7);      // Read in in 1.e^19 m^-3
 ...
 // bmag in SI unit Tesla multiply by 1.e4 changes to Gaussian unit gauss 
  ...
  output.write("\tparameters:  AA=mi/mp %e,  ZZ %e ,zli %e,ll %e \n",AA,ZZ,Zli,ll);
  ...
  fmei_li1  = 1./1836.2/AA*0.2857;                                          // no unit
  lambda_ei = 24.-log(sqrt(Ni_x)/Te_x);                          // in unit cm
  lambda_ii = 23.-log(ZZ*ZZ*ZZ*sqrt(2.*Ni_x)/pow(Ti_x, 1.5));    // unit cm
  lambda_eli = 24.-log(sqrt(Ni_x)/Te_x);                          // in unit cm
  lambda_ili = 23.-log(ZZ*ZZ*ZZ*(AA+ll)/(AA*Ti_x+ll*Ti_x)*sqrt(Ni_x/Ti_x+Ni_x/Ti_x));    // unit cm
  ...
  nuelix     = 2.91e-6*Ni_x*lambda_eli/pow(Ti_x, 1.5);         // unit 1/s
  nuilix     = 4.78e-8*Ni_x*lambda_ili/pow(Ti_x, 1.5)/sqrt(ll);         // unit 1/s
 ...
 if(!include_curvature)
  ...
  Nli_1.setLocation(CELL_CENTRE);
  Tli_1.setLocation(CELL_CENTRE);
  Vli_1.setLocation(CELL_CENTRE);
  ...
  Nli.setLocation(CELL_CENTRE);
  Vlix.setLocation(CELL_CENTRE); 
  ...
  //
   ...
   Ni.setBoundary("Ni");
   Nli_1.setBoundary("Nli_1");
   Ti.setBoundary("Ti");
   Tli_1.setBoundary("Tli_1");
   Te.setBoundary("Te");
   Vi.setBoundary("Vi");
   Vli_1.setBoundary("Vi");
   Nn.setBoundary("Nn");
   Tn.setBoundary("Tn");
   Vn.setBoundary("Vn");
   Nli.setBoundary("Nli");
   ...
   tau_ili1.setBoundary("Tn");
   tau_eli1.setBoundary("Tn");
   ...
   Grad_par_pli1.setBoundary("Tn");
   Grad_par_pi.setBoundary("Tn");
 ...
 //  frac.setBoundary("Tn");
  ...
  if(profiles_lasttimestep)
    {
     ...
     BoutReal lstimeNn[NX][NY][NZ],lstimeNli[NX][NY][NZ],lstimeVlix[NX][NY][NZ];  // Vn Tn are not evloved in recent version
	...
	for(int jx=0;jx<NX;jx++) 
	 for(int jy=0;jy<NY;jy++) 
	   for(int jz=0;jz<NZ;jz++) 
	       ...
	       pFile6 >> lstimeNli[jx][jy][jz]; 
	       pFile7 >> lstimeVlix[jx][jy][jz]; 
	...
	for(int jz=0;jz<mesh->ngz;jz++) 
	  ...
	  Nli[jx][jy][jz] = lstimeNli[mesh->XGLOBAL (jx)][mesh->YGLOBAL (jy)][jz];
	  Vlix[jx][jy][jz] = lstimeVlix[mesh->XGLOBAL (jx)][mesh->YGLOBAL (jy)][jz];
	}
 	...
 	for (int jz=0;jz<mesh->ngz;jz++)
	    ...
	    if(!load_grid_profiles)
	      {
                ...
                if(initial_profile_exp)
	          {
                ...
                if(initial_profile_Hmode) 
                  ...
                  // Nli_1[jx][jy][jz] = Ni_edge+Ni_core*(Hmode_tanh - 1.);
                   Ti[jx][jy][jz] = Ti_edge+Ti_core*(Hmode_tanh - 1.);
                  // Tli_1[jx][jy][jz] = Ti_edge+Ti_core*(Hmode_tanh - 1.);
		...
		if(initial_SOL_edgeval)
	     ...
	     Vi[jx][jy][jz]=0.0;                                               //~~~~~~~~~~~~~~~~~~
             ...
             Tli_1[jx][jy][jz] = minimum_val;

             Nn[jx][jy][jz] = minimum_val;
             Nli[jx][jy][jz] = minimum_val;
             Nli_1[jx][jy][jz] = minimum_val;
  ...
  if (diffusion_coef_Hmode && initial_profile_Hmode)        //TRUE
    {
  ...
  if(extsrcs_balance_diffusion)       //FALSE
    {
      D2DX2_Ni = D2DX2(Ni);
  ...
  V_th_li1= 9.79e3*sqrt(Tli_1*Te_x/ll);
  ...
  output.write("\tli1 thermal velocity: %e -> %e [m/s]\n", min(V_th_li1), max(V_th_li1));
  V_th_e /= Lbar/tbar;
  V_th_i /= Lbar/tbar;
  V_th_li1 /= Lbar/tbar;
  ...
  output.write("\tNormalized li1 thermal velocity: %e -> %e [Lbar/tbar]\n", min(V_th_li1), max(V_th_li1));

  kappa_Te = 3.2*(1./fmei)*(1./tbar/nueix)*(Te^2.5);     // power operator '^' works only for Fields
  kappa_Ti = 3.9*(1./tbar/nuiix)*(Ti^2.5);  
  kappa_Tli1 = 3.9*(1./tbar/nuilix)*(Tli_1^2.5);  
  Field3D kappa_Te_realunits, kappa_Ti_realunits,kappa_Tli1_realunits;
  ...
  kappa_Tli1_realunits= kappa_Tli1*Ni_x*pow(Lbar,2.)/tbar;
  ...
  output.write("\tNormalzied li1 para thermal conductivity: %e -> %e [N0 Lbar^2/tbar]\n", min(kappa_Tli1_realunits), max(kappa_Tli1_realunits));
  output.write("\telectron para thermal conductivity: %e -> %e [N0 m^2/s]\n", min(kappa_Te_realunits), max(kappa_Te_realunits));   
  output.write("\tNormalized electron para thermal conductivity: %e -> %e [N0 Lbar^2/tbar]\n", min(kappa_Te), max(kappa_Te));  
  output.write("\tNormalized li1 para thermal conductivity: %e -> %e [N0 Lbar^2/tbar]\n", min(kappa_Tli1), max(kappa_Tli1));  

  kappa_Te_fl = V_th_e*q95_input*Ni;    // Ne=Ni quasineutral
  kappa_Ti_fl = V_th_i*q95_input*Ni;
  kappa_Tli1_fl = V_th_li1*q95_input*Ni;

  kappa_Te *= kappa_Te_fl/(kappa_Te_fl+kappa_Te);
  kappa_Ti *= kappa_Ti_fl/(kappa_Ti_fl+kappa_Ti);
  kappa_Tli1 *= kappa_Tli1_fl/(kappa_Tli1_fl+kappa_Tli1);
  ...
  output.write("\tUsed li1 electron para thermal conductivity: %e -> %e [N0 Lbar^2/tbar]\n", min(kappa_Tli1), max(kappa_Tli1));  
 ...
 //Dissociation rate of molecules
  nu_ionli1 = 9.5e3*Nli*Ni_x*Te*Te*Te_x*Te_x/(4.6+0.1*Te*Te*Te_x*Te_x);     //need be corrected
  ...
  // recombination rate of ions and electrons
  nu_recli1 = Nli_1*Ni_x*5.197e-1*Zli*sqrt(lambda_rec)*(0.4288+0.5*log(lambda_rec)+0.469/(lambda_rec)^0.333); //Seaton M F 1959 'Radiative recombination of hydrogenic ions'
  output.write("\tionization rate: %e -> %e [1/s]\n", min(nu_ionz), max(nu_ionz));
  output.write("\tcharge exchange rate: %e -> %e [1/s]\n", min(nu_CX), max(nu_CX)); 
  output.write("\tli ionization rate: %e -> %e [1/s]\n", min(nu_ionli1), max(nu_ionli1));
 ...
 if (BScurrent)
  ...
  dump.add(kappa_Tli1,"kappa_Tli1",1);
  ...
  dump.add(nu_ionli1,"nu_ionli1",1);
  dump.add(nu_rec,"nu_rec",1);
  dump.add(nu_recli1,"nu_recli1",1);

  dump.add(Si_p,"Si_p",1);
  dump.add(Si_CX,"Si_CX",1);
  dump.add(Si_li1,"Si_li1",1);
  dump.add(S_rec,"S_rec",1);
  dump.add(S_recli1,"S_recli1",1);
  dump.add(Grad_par_pei,"Grad_par_pei",1);
  dump.add(Grad_par_pli1,"Grad_par_pli1",1);
  dump.add(Grad_par_pi,"Grad_par_pi",1);
  dump.add(Grad_par_pn,"Grad_par_pn",1);
  dump.add(Grad_par_logNn,"Grad_par_logNn",1);

  dump.add(tau_ei,"tau_ei",1);
  dump.add(tau_ili1,"tau_ili1",1);
  dump.add(tau_eli1,"tau_eli1",1);
//  dump.add(frac,"frac",1);
...
int physics_run(BoutReal t)
{
  ...
  mesh->communicate(Nli_1, Vli_1, Tli_1 );
  //mesh->communicate(Nn,Tn,Vn);
  mesh->communicate(Nn);
  mesh->communicate(Nli,Vlix,Vli);
  ...
  if(SMBI_LFS)
  {
  Nli=ret_const_flux_BC(Nli, Nli0);
  Vlix=ret_const_flux_BC(Vlix, Vli0); 
  }
  else
 {
  Nli=ret_const_flux_BC(Nli,minimum_val);
  Vlix=ret_const_flux_BC(Vlix,0.0);
  }
...
//  c_se = sqrt((temp_Te+temp_Ti+temp_Tli1)/Mi); 
  ...
  if(Sheath_BC)                    //TRUE
    {
      ...
      SBC_Dirichlet(Vli_1, c_se);       //LI
      SBC_Gradpar(Ni, 0.);
      SBC_Gradpar(Nli_1, 0.);           //LI
      SBC_Gradpar(Te, q_se_kappa);
      SBC_Gradpar(Ti, q_si_kappa);
      SBC_Gradpar(Tli_1, q_si_kappa);   //LI
     }
  ...
  // Collisional time ion-electrons and D-li1 and e-li1
  tau_ei = (1./tbar/nueix)*(temp_Ni+temp_Nli_1)/(temp_Te^1.5);
 //tau_ili1 = (1./tbar/nuilix)*temp_Ni/(temp_Ti^1.5);
  tau_ili1 = (1./tbar)*1.5e13*Mli*temp_Tli1/((temp_Ti/Mi)^1.5);
  tau_eli1 = (1./tbar/nuelix)*(temp_Ni+temp_Nli_1)/(temp_Te^1.5);
 //// frac= (Vli_1-Vi)/tau_ili1;
  //ion viscosity
  eta0_i=0.96*(1./tbar/nuiix)*temp_Ni*temp_Ti;
  eta0_li1=0.96*(1./tbar/nuilix)*temp_Nli_1*temp_Tli1;
 ...
 //Dissociation rate of molecules
  nu_ionli1 = 9.5e3*tbar*temp_Nli*Ni_x*temp_Te*temp_Te*Te_x*Te_x/(4.6+0.1*temp_Te*temp_Te*Te_x*Te_x);     //need be corrected
  //nu_diss =0.;
  ...
  // recombination rate of ions and electrons
  nu_recli1 = tbar*temp_Nli_1*Ni_x*5.197e-1*Zli*sqrt(lambda_rec)*(0.4288+0.5*log(lambda_rec)+0.469/(lambda_rec)^0.333); //Seaton M F 1959 'Radiative recombination of hydrogenic ions'
  // source terms
  Si_p = (temp_Ni+temp_Nli_1)*nu_ionz;
  Si_CX = temp_Ni*nu_CX;
  Si_li1 = (temp_Ni+temp_Nli_1)*nu_ionli1;  
  S_rec = (temp_Ni+temp_Nli_1)*nu_rec;
  S_recli1=(temp_Ni+temp_Nli_1)*nu_recli1;
  //Set Boundary and Apply Boundary for output variables
  tau_ei.applyBoundary();
  tau_ili1.applyBoundary();  
  tau_eli1.applyBoundary();
  ...
  Si_p.applyBoundary();
  Si_CX.applyBoundary();
  Si_li1.applyBoundary();

  if(t==0.)  output.write("\n **** t %e  nu_ionli1 %e  Si_li1 %e tau_ei %e    tau_ili1 %e tau_eli1 %e  ****\n",t,max(nu_ionli1),max(Si_li1),max(tau_ei),max(tau_ili1),max(tau_eli1)); 
  ...
  Diffc_nn_perp = Tn/Mn/(nu_CX_n+2.*Si_p/temp_Nn);
  if(terms_recombination) Diffc_nn_perp = Tn/Mn/(nu_CX_n+2.*Si_p/temp_Nn+S_rec/temp_Nn);
  V_th_n=sqrt(Tn/Mn);
  ...
  Diffc_nn_par = Tn/Mn/(nu_CX_n+2.*Si_p/temp_Nn);
  if(terms_recombination) Diffc_nn_par = Tn/Mn/(nu_CX_n+2.*Si_p/temp_Nn+S_rec/temp_Nn);
  ...
  if(Wall_particle_recycle)             //TRUE
   ...
   //  WallBC_Xout_GradX_len(Nli_1, -1./Lni_wall);                                                   //fixed Gradient length (real unit m) at wall
     WallBC_Xout_GradX(Nn,Gamma_nnw_Diffc);                                                     // '+' means in-flowing from wall
  //   WallBC_Xout_GradX(Nli,Gamma_nnw_Diffc);                                                     // '+' means in-flowing from wall

     } 

  if(SBC_particle_recycle)               //TRUE
     {      
       /*
   ...
   //    SBC_Gradpar(Nli, Gamma_nn/Diffc_nn_par);
...
//************************************
  // DENSITY OF LI EQUATION
//************************************
  // output.write("Now updata Ni \n"); 
  //ddt(Ni) = 0.; 

  ddt(Nli_1) =   Diffc_ni_perp*Delp2(Nli_1)
            - Nli_1*Grad_par(Vli_1)
            - Vpar_Grad_par(Vli_1,Nli_1)
            + Si_li1; 
//  if(terms_recombination) ddt(Nli_1) -= S_recli1;
  if(external_sources) ddt(Nli_1) += S_pi_ext;
  if(terms_Gradperp_diffcoefs) 
    {
      grad_perp_Diffi=Grad_perp(Diff_ni_perp);
      grad_perp_Diffi.applyBoundary();
      mesh->communicate(grad_perp_Diffi);  
      ddt(Nli_1) +=  V_dot_Grad(grad_perp_Diffi,Nli_1); 
    }
...
//************************************
  ...
  //ddt(Ti)=0.0;
   ...
   ddt(Ti) = - Vpar_Grad_par(Vi,Ti)
           ...
           //  - nu_ionz*Ti
             -((temp_Ni+temp_Nli_1)/temp_Ni)*nu_ionz*Ti
             - 0.6667*nu_CX*(Ti-Tn)
            // + 2.*fmei*(Te-Ti)/tau_ei
            // + (Mli/Mi)*(Tli_1-Ti)/tau_ili1
            + 2.*((temp_Ni+temp_Nli_1)/temp_Ni)*fmei*(Te-Ti)/tau_ei 
            +(Mi*temp_Ni/(Mli*temp_Nli_1))*(Tli_1-Ti)/tau_ili1
          ;
   if(terms_recombination)  ddt(Ti) += ((temp_Ni+temp_Nli_1)/temp_Ni)*nu_rec*Ti;
...
//************************************
  ...
  //ddt(Ti)=0.0;
   ...
   ddt(Tli_1) = - Vpar_Grad_par(Vli_1,Tli_1)
             - 0.6667*Tli_1*Grad_par(Vli_1)
             + 0.6667*kappa_Tli1*Grad2_par2(Tli_1)/temp_Nli_1
             + 0.6667*Grad_par(kappa_Tli1)*Grad_par(Tli_1)/temp_Nli_1
             + 0.6667*chi_i_perp*Delp2(Tli_1)
             //- nu_ionli1*Tli_1
             -((temp_Ni+temp_Nli_1)/temp_Nli_1)*nu_ionli1*Tli_1
            // - 0.6667*nu_CX*(Tli_1-Tn)
             //+ 2.*fmei_li1*(Te-Tli_1)/tau_eli1
             + 2.*((temp_Nli_1+temp_Ni)/temp_Nli_1)*fmei_li1*(Te-Tli_1)/tau_eli1
            // + 2.*fmei_li1*(Ti-Tli_1)/tau_ili1
            // + (Mi/Mli)*(Ti-Tli_1)/tau_ili1
             +(temp_Ni/temp_Nli_1)*(Mi/Mli)*(Ti-Tli_1)/tau_ili1
                   ;
  // if(terms_recombination)  ddt(Tli_1) += nu_recli1*Tli_1;
   if(external_sources) ddt(Tli_1) += 0.6667*S_Ei_ext/temp_Nli_1;
   if(terms_Gradperp_diffcoefs) 
    {
      grad_perp_chii=Grad_perp(chi_i_perp);
      grad_perp_chii.applyBoundary();
      mesh->communicate(grad_perp_chii);  
      ddt(Tli_1) +=  0.6667*V_dot_Grad(grad_perp_chii,Tli_1); 
    }
...
//************************************
  ...
  //output.write("Now updata Vi \n");
     ...
     Grad_par_pli1=Grad_par(pli1);
     Grad_par_pi=Grad_par(pi);
     Grad_par_pei.applyBoundary();
     Grad_par_pli1.applyBoundary();
     Grad_par_pi.applyBoundary();
     mesh->communicate(Grad_par_pei);
     mesh->communicate(Grad_par_pli1);
     mesh->communicate(Grad_par_pi);

   //ddt(Vi)=0.0;
   
   ddt(Vi) = - Vpar_Grad_par(Vi,Vi)
       //      - Grad_par(pei)/temp_Ni/Mi
         //    - Grad_par(pli1)/temp_Ni/Mi
             - Grad_par(pi)/temp_Ni/Mi
             + 2.*0.6667*Grad_par(eta0_i)*Grad_par(Vi)/temp_Ni/Mi
             + 2.*0.6667*eta0_i*Grad2_par2(Vi)/temp_Ni/Mi
             + (Mli*temp_Nli_1*Vli_1-Mi*temp_Ni*Vi)/(Mi*temp_Ni)/tau_ili1
             + (temp_Ni+temp_Nli_1)/temp_Ni*nu_ionz*Vi
            // + (Vli_1-Vi)/tau_ili1
             // - (nu_ionz+nu_CX)*(Vi-Vn)
             ;             
   
//  output.write("tau_ili1 = %e\n", (Vli_1-Vi)/tau_ili1);
//************************************
  // ION Paralell VELOCITY  ---Vi---
//************************************
...
//  pli1 = (Te+Tli_1)*Nli_1;
  //output.write("Now updata Vi \n");
  // Grad_par_pei=Grad_par(pli1);
  // Grad_par_pei.applyBoundary();
   //mesh->communicate(Grad_par_pei);

   //ddt(Vi)=0.0;
   
   ddt(Vli_1) = - Vpar_Grad_par(Vli_1,Vli_1)
             - Grad_par(pli1)/temp_Nli_1/Mli
   //          - Grad_par(pei)/temp_Nli_1/Mli
     //        - Grad_par(pi)/temp_Nli_1/Mli
             + 2.*0.6667*Grad_par(eta0_li1)*Grad_par(Vli_1)/temp_Nli_1/Mli
             + 2.*0.6667*eta0_li1*Grad2_par2(Vli_1)/temp_Nli_1/Mli
             -((temp_Nli_1+temp_Ni)/temp_Nli_1)*nu_ionli1*Vli_1
             +(temp_Ni*Mi*Vi-temp_Nli_1*Mli*Vli_1)/(temp_Ni*Mi)/tau_ili1
      //       - nu_ionli1*Vli_1
       //      +(Vi-Vli_1)/tau_ili1
		;             
...
//************************************
  ...
  */
//************************************
  //Lithium Perpendicular Velocity in X ---Vlix---
 //************************************
...
//   ddt(Vmx) =0.;
 ...
 Vli.y=0.;
 Vli.z=0.;
 ddt(Vli) = - V_dot_Grad(Vli,Vli) 
           - Grad(pm)/temp_Nli/Mli
          ;
...
//************************************
 ...
 ddt(Nli)=-V_dot_Grad(Vli,Nli)-Nli*Div(Vli) - Si_li1;//+S_recli1;
