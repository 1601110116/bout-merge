...
// 2D inital profiles
Field2D J0, P0, n0; // Current and pressure
...
BoutReal T0, omegai; //temperature,Ti_perp=Ti_parallel=Te_perp=Te_parallel, ion gyroradius and frequency
...
Field2D gyroa,gyrod; //invert parameters
int gyron_flags, gyrophi_flags; //invert flags
BoutReal gyrodensity, constantp;  //constant density in vorticity
Vector2D sour1;
Vector3D sour2;
bool parallel_current;  //parallel current in pressure equation
bool constant_B;
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, gyrofluid,           true);  //=======================
  OPTION(options, electron_pressure,   true);
  OPTION(options, FLR_terms,           true);
  OPTION(options, constant_density,    true);
  OPTION(options, constantp,         0.0077);  //normalized pressure at maximum gradient
  OPTION(options, T0,                  1000);   //temperature in eV
  OPTION(options, parallel_current,   false);   //parallel current in pressure equation
  OPTION(options, constant_B,         false);

  OPTION(options, electron_inertial,  false);  //keep electron inertial ============================================================
  ...
  OPTION(options, withflow,          true);    //withflow or not
  ...
  T0=T0*1.602e-19;                           //using J not eV

  if(constant_B)
  rhoi = Mi*sqrt(T0/Mi)/(1.602e-19*1.33);   //constant rhoi
  else
  ...
  V0=V0/Va;
  Dphi0*=Tbar;
  n0 = 0.5*P0/T0;                    //normalized n0 =====================================================
  gyrodensity = 0.5*constantp/T0;    //normalized constant density in vorticity ===========================
  output.write("gyrodensity = %e\n",gyrodensity);
  ...
  if(gyrofluid){
    ...
    if(constant_density){
      ns = T0*(gyron-ni1-U/(gynorm*B0))/gyrodensity;
      ...
      sour1 = Grad(n0, CELL_YLOW);
      sour2 = Grad(ns, CELL_YLOW);
      gyrosour = -ns/invertpar-sour1*sour2/n0;
      gyropar = invert_laplace(gyrosour, phi_flags, NULL, &n0, NULL);
    }
    phi=0.5*(gyropar+ns)/(gynorm*B0);
    ...
    if(diamag_phi0){
    gyrophi0 = invert_laplace(phi0,gyrophi_flags,&gyroa,NULL);
    ...
    if(constant_density){
     ...
     if(diamag){
       phi -= 0.5*dnorm * P / (B0*gyrodensity);
      }
    }
    else{
     Un = U/n0;
     diasour=Un;
     if(diamag){
       diasour-=0.5*dnorm*Delp2(P)/(B0*n0);
     }
     mesh->communicate(diasour);
     diasour.applyBoundary();
...
int physics_run(BoutReal t)
{
  ...
  if(electron_pressure){
    Pe1 = 0.5*(P+T0*U/(gynorm*B0));
    mesh->communicate(Pe1);
  }
  if(gyrofluid){

    ni1 = 0.5*(P/T0-U/(gynorm*B0));
    mesh->communicate(ni1);
    ...
    gyrophi_d = gyrophi-phi;
    mesh->communicate(gyrophi_d);

    if(diamag_phi0){
    gyrophi0 = invert_laplace(phi0,gyrophi_flags,&gyroa,NULL);
    gyrophi0.applyBoundary();
    mesh->communicate(gyrophi0);
    gyrophi0_d = gyrophi0-phi0;
    mesh->communicate(gyrophi0_d);
     }
    }
}
else{
  if(constant_density){
    ...
    diasour=Un;
    if(diamag){
     diasour-=0.5*dnorm*Delp2(P)/(n0*B0);
    }
    mesh->communicate(diasour);
    diasour.applyBoundary();
    phi=invert_laplace(diasour, phi_flags, NULL, &n0, NULL);
...
// Apply a boundary condition on phi for target plates
  ...
  if(electron_inertial) {
    ...
    ddt(myLambda) = 0;
    ddt(myLambda) = -massratio/(gynorm*gynorm)*b0xGrad_dot_Grad(J0/n0,phi) -Grad_parP0(phi*B0, CELL_CENTRE)/B0 + eta*Jpar;

    if(nonlinear)
      ddt(myLambda) -= bracket(phi, myLambda, bm_exb)*B0;

    //=====================================================================================================
    if(electron_pressure){
       ddt(myLambda)+=1.0/(4.0*gynorm*n0)*b0xGrad_dot_Grad(P0/B0,Psi);
       ddt(myLambda)+=1.0/(2.0*gynorm*n0)*Grad_parP_back(Pe1/B0, CELL_YLOW);
    }
    ...
    //=====================================================================================================
    if(electron_pressure){
       ddt(Psi)+=1.0/(4.0*gynorm*n0)*b0xGrad_dot_Grad(P0/B0,Psi);
       ddt(Psi)+=1.0/(2.0*gynorm*n0)*Grad_parP_back(Pe1/B0, CELL_YLOW);
    }
  ...
  if(gyrofluid&&FLR_terms)
   ddt(U)-=gynorm*B0*b0xGrad_dot_Grad(n0,gyrophi_d);

  if(gyrofluid&&diamag_phi0&&FLR_terms)
   ddt(U)+=gynorm*B0*b0xGrad_dot_Grad(gyrophi0_d,ni1);

  if(gyrofluid&&nonlinear&&FLR_terms)
    ddt(U)+=gynorm*B0*bracket(gyrophi_d,ni1)*B0;
  ...
  if(evolve_pressure) {
    ...
    //=============================================================================
    if(gyrofluid&&FLR_terms)
      ddt(P) += T0*b0xGrad_dot_Grad(n0,gyrophi_d);

    if(gyrofluid&&diamag_phi0&&FLR_terms)
      ddt(P) -= T0*b0xGrad_dot_Grad(gyrophi0_d, ni1);

    if(gyrofluid&&nonlinear&&FLR_terms)
      ddt(P) -= T0*bracket(gyrophi_d, ni1, bm_exb)*B0;

    if(parallel_current){
      ddt(P) -= B0/gynorm*Grad_parP(T0*Jpar, CELL_YLOW);
      ddt(P) -= B0/gynorm*b0xGrad_dot_Grad(T0*J0,Psi);
    }
...
{
  Field3D U1;
  Field3D P2, Psi2, U2, myLambda2;
  Field3D P3, Psi3, U3, myLambda3;

  //output.write("precon t = %e, gamma = %e\n", t, gamma);
  if(electron_inertial){
    mesh->communicate(ddt(P), ddt(myLambda), ddt(U));
  // First matrix. Only modifies vorticity

  U1 = ddt(U) + gamma*(Lp(ddt(P)) + Lpsi(ddt(myLambda)));
  ...
  myLambda3 = myLambda2 + gamma*Grad_par(B0*phitmp) / B0;
  U3 = U2;
...
int jacobian(BoutReal t)
{
  ...
  Field3D JmyLambda = -Grad_par(B0*phi, CELL_YLOW) / B0;
  JmyLambda.setBoundary("myLambda"); JmyLambda.applyBoundary();

  Field3D JU = b0xcv*Grad(ddt(P))
    - (B0^2)*Grad_par(Jpar, CELL_CENTRE)
    + (B0^2) * b0xGrad_dot_Grad(ddt(myLambda), J0, CELL_CENTRE);
