...
const char CXXVERSION[] = "v0.4.1_02062018_J.G.Chen";
#include "bout.hxx"
#include "derivs.hxx"
...
BoutReal T0_height, T0_ave, T0_width, T0_center, T0_bottom_x;                            // the total height, average width and center of profile of T0
...
bool local_q;
BoutReal q_alpha;
bool n0_fake_prof, n0_p0_0p3, T0_fake_prof, Ti0_fake_prof;
BoutReal Zi; // charge number of ion

/***************** profile control *********************/

bool profile_control, fit_pressure, constant_density, T0_profile;
BoutReal n0_const, T0_const;
BoutReal P_0, s_p, P_min, PDped, Pdel, P_x0, Pbottom, Pa;
BoutReal T_0, s_t;
int Equilibrium_case;
BoutReal n0_cyclone, cyclone_ti0_ave, cyclone_te0_ave;
bool cyclone_const_ti0, cyclone_const_te0;
...
Field3D U, ubyn, Psi, P, Pi, Pe;
Field3D Ni, Te, Ti, Ne;
Field3D Vipar, Vepar;
Field3D qepar, qipar;
Field3D Ti_gradphi, Ti_diamag, Ti_energy_flux, Ti_gradvepar, Ti_advection, Ti_energy_exch, Ti_resist;
Field3D Te_gradphi, Te_diamag, Te_energy_flux, Te_gradvepar, Te_advection, Te_energy_exch, Te_resist;
Field3D Te_vegradte0, Te_ve0gradte, Te_vegradte;
Vector3D Vexb, Vexb0;

bool output_vradial, output_Teterms, output_Titerms, output_Tevegradte, output_qparcompare, output_kappa;
...
BoutReal Nbar, Tibar, Tebar, Pibar, Pebar;
BoutReal dia_fact; // Multiply diamagnetic term by this

BoutReal diffusion_par;                        // Parallel thermal conductivity
bool Landau, thermalforceinohmslaw, fluxlimit; // Use Gyro Landau Fluid closure
...
BoutReal k0;                                   // collisionless Landau damping coefficient
Field3D kappa_i;                               // ion collisional Landau damping coefficient (0.5*nu_i/vth_i)
Field3D kappa_e;                               // electron collisional Landau damping coefficient (0.5nu_e/vthe_e)
...
BoutReal D_Nix;          // radial transport coefficient of density
BoutReal Xi_Tix, Xi_Tex; // radial transport coefficient of temperature
...
BoutReal sink_Til;    // left edge sink in vorticity
BoutReal sTi_widthl;  // left edge sink profile radial width in vorticity
BoutReal sTi_lengthl; // left edge sink radial domain in vorticity

BoutReal sink_Tir;    // right edge sink in vorticity
BoutReal sTi_widthr;  // right edge sink profile radial width in vorticity
BoutReal sTi_lengthr; // right edge sink radial domain in vorticity

BoutReal sink_Tel;    // left edge sink in vorticity
BoutReal sTe_widthl;  // left edge sink profile radial width in vorticity
BoutReal sTe_lengthl; // left edge sink radial domain in vorticity

BoutReal sink_Ter;    // right edge sink in vorticity
BoutReal sTe_widthr;  // right edge sink profile radial width in vorticity
BoutReal sTe_lengthr; // right edge sink radial domain in vorticity
...
bool update_N_T;                  // update density and temperature in eta_spitzer and diffusion
...
int physics_init(bool restarting) {
  ...
  output.write("Finished read bxcv\n");
  ...
  OPTION(options, Equilibrium_case,          0);
  OPTION(options, n0_cyclone,           1.0e19);
  OPTION(options, cyclone_const_ti0,     false);
  OPTION(options, cyclone_const_te0,     false);
  OPTION(options, cyclone_ti0_ave,          0.);
  OPTION(options, cyclone_te0_ave,          0.);

  OPTION(options, T0_profile,            false);  // non-constant temperature
  OPTION(options, T_0,                       0);
  OPTION(options, s_t,                       0);
  ...
  OPTION(options, n0_p0_0p3,             false);  // use n0=n0_height*(P0/P00)^0.3*density
  OPTION(options, Ti0_fake_prof,         false);  // use the hyperbolic profile of Ti0, calculate Te0 with other equilibrium profiles
  ...
  OPTION(options, T0_height,               0.4);  // the total height of profile of T0, in percentage of Ni_x
  OPTION(options, T0_ave,                 0.01);  // the center or average of T0, in percentage of Ni_x
  OPTION(options, T0_width,                0.1);  // the width of the gradient of T0,in percentage of x
  OPTION(options, T0_center,             0.633);  // the grid number of the center of T0, in percentage of x
  OPTION(options, T0_bottom_x,            0.81);  // the start of flat region of T0 on SOL side, in percentage of x
  ...
  OPTION(options, update_N_T,             true);  // Update density & temperature in eta_spitzer & diffusion
  ...
  OPTION(options, diff_par_flutter,      false);
  ...
  OPTION(options, thermalforceinohmslaw,  true);
  OPTION(options, fluxlimit,              true);
  ...
  OPTION(options, D_Nix,                  -1.0);  // radial transport coefficient of ion density
  OPTION(options, Xi_Tix,                 -1.0);  // radial transport coefficient of Ti
  OPTION(options, Xi_Tex,                 -1.0);  // radial transport coefficient of Te
  ...
  OPTION(options, output_kappa,          false);
  ...
  // left edge sink factor in vorticity
  OPTION(options, sink_Til,               -1.0);  // left edge sink in vorticity
  OPTION(options, sTi_widthl,             0.06);  // the percentage of left edge radial grid points for sink profile radial width in vorticity
  OPTION(options, sTi_lengthl,            0.15);  // the percentage of left edge radial grid points for sink profile radial domain in vorticity

  // right edge sink factor in vorticity
  OPTION(options, sink_Tir,               -1.0);  // right edge sink in vorticity
  OPTION(options, sTi_widthr,             0.06);  // the percentage of right edge radial grid points for sink profile radial width in vorticity
  OPTION(options, sTi_lengthr,            0.15);  // the percentage of right edge radial grid points for sink profile radial domain in vorticity

  // left edge sink factor in vorticity
  OPTION(options, sink_Tel,               -1.0);  // left edge sink in vorticity
  OPTION(options, sTe_widthl,             0.06);  // the percentage of left edge radial grid points for sink profile radial width in vorticity
  OPTION(options, sTe_lengthl,            0.15);  // the percentage of left edge radial grid points for sink profile radial domain in vorticity
  ...
  if (diffusion_par < 0 || !Landau) {
    output_qparcompare = false;
    output_kappa = false;
    output.write("No parallel thermal conduction or Landau closure\n");
    output.write("    Set 'output_qparcomare' and 'output_kappa' to be false.\n");
  }
  ...
  if (mesh->get(Nbar, "Nixexp")) // Typical ion density scale
    // Failed to get Nixexp: density unit in grid is `density` in BOUT.inp
    Nbar = 1.0;
  else
    // Got Nixexp: density unit in grid is 10^20 [m^-3]
    Nbar *= 1.e20 / density;

  Va = sqrt(Bbar * Bbar / (MU0 * Mi * Nbar * density));

  if (profile_control && Equilibrium_case == 2) {
    mesh->get(Bbar, "Bbar");
    Lbar = 0.585;
    Va = sqrt(2000. * 1.602e-19 / (2 * 1.6726e-27));
    Tibar = 2000.;
    Tebar = 2000.;
  }
  ...
  output.write("                Pibar = %e Pa\n", ee * Tibar * Nbar * density);
  output.write("                Pebar = %e Pa\n", ee * Tebar * Nbar * density);
  ...
  if (profile_control) {
    ...
    case 1:
      ...
      if (constant_density) {
        ...
        if (T0_profile) {
          ...
          Ti0 =
              T_0 *
              exp(s_t * (P_min * (exp(-(P_x0 - x) / PDped) + (1.0 + Pa * (P_x0 - x) / PDped) * exp((P_x0 - x) / PDped) * Pdel) / (exp(-(P_x0 - x) / PDped) + exp((P_x0 - x) / PDped)) + Pbottom)); // eV
          Te0 = Ti0;
    ...
    case 2:
      ...
      mesh->get(Te0, "TeHat"); // normalized
      N0 *= n0_cyclone / (Nbar * density);
      Ti0 = Te0;
      P0 = 2. * N0 * Te0;

      if (cyclone_const_ti0)
        Ti0 = cyclone_ti0_ave;
      if (cyclone_const_te0)
        Te0 = cyclone_te0_ave;
    ...
    default:
      output.write("need to give equilibrium_case 1, 2\n");
      return 1;
    }
  } else if ((!T0_fake_prof) && n0_fake_prof) {
    if (fit_pressure) { // only for cbm grids
      P0 = P_0 * exp(s_p * (P_min * (exp(-(P_x0 - x) / PDped) + (1.0 + Pa * (P_x0 - x) / PDped) * exp((P_x0 - x) / PDped) * Pdel) / (exp(-(P_x0 - x) / PDped) + exp((P_x0 - x) / PDped)) + Pbottom));
    ...
    if (n0_p0_0p3) {
      ...
      N0 = N0tanh(n0_height * Nbar, n0_ave * Nbar, n0_width, n0_center, n0_bottom_x, n0_topgrad, n0_bottomgrad);

    if (Ti0_fake_prof) {
      Ti0 = N0tanh(T0_height, T0_ave, T0_width, T0_center, T0_bottom_x);
      Te0 = P0 / N0 - Ti0;
    } else {
      Ti0 = P0 / N0 / (1. + Zi);
      Te0 = Ti0;
  ...
  if (diffusion_par > 0.0 || diffusion_perp > 0.0) {
    ...
    if (Landau) {
      ...
      if (Landau_coll) {
        ...
        if (output_kappa) {
          dump.add(kappa_i, "kappa_i", 1);
          dump.add(kappa_e, "kappa_e", 1);
        }
      }
    }
  }
  ...
  if (diffusion_par > 0.0) {
    ...
    if (Landau) {
      if (Landau_coll) {
        ...
        mesh->communicate(kappa_i, kappa_e);
        kappa_i.applyBoundary();
        kappa_e.applyBoundary();
      } else {
        output.write("Collisionless Landau damping closure used!\n");
        k0 = 2. / (20. * (2 * PI * q95_input));
        output.write("\tk0 = %e\n", k0);
      }
    }
  }
  ...
  if (output_Titerms) {
    dump.add(Ti_gradphi, "Ti_gradphi", 1);
    dump.add(Ti_diamag, "Ti_diamag", 1);
    dump.add(Ti_energy_flux, "Ti_energy_flux", 1);
    dump.add(Ti_gradvepar, "Ti_gradvepar", 1);
    dump.add(Ti_advection, "Ti_advection", 1);
    dump.add(Ti_energy_exch, "Ti_energy_exch", 1);
    dump.add(Ti_resist, "Ti_resist", 1);
  }

  if (output_Teterms) {
    dump.add(Te_gradphi, "Te_gradphi", 1);
    dump.add(Te_diamag, "Te_diamag", 1);
    dump.add(Te_energy_flux, "Te_energy_flux", 1);
    dump.add(Te_gradvepar, "Te_gradvepar", 1);
    dump.add(Te_advection, "Te_advection", 1);
    dump.add(Te_energy_exch, "Te_energy_exch", 1);
    dump.add(Te_resist, "Te_resist", 1);
  }

  if (output_Tevegradte) {
    dump.add(Te_vegradte0, "Te_vegradte0", 1);
    dump.add(Te_ve0gradte, "Te_ve0gradte", 1);
    dump.add(Te_vegradte, "Te_vegradte", 1);
  }
  ...
  if (output_vradial) {
    ...
    Vexb0.covariant = false;
    Vexb0.setLocation(CELL_YLOW);
    Vexb0.setBoundary("Vipar");
    dump.add(Vexb0.x, "Vexb0_x", 1);
  }
  ...
  if (diamag && diamag_phi0) {
    ...
    if (n0_p0_0p3)
      phi0 = -1 / 0.7 * Upara0 * Pi0 / N0 / B0;
    else
      phi0 = -Upara0 * Pi0 / N0 / B0;
    // Field2D pth;
...
int physics_run(BoutReal t) {
  ...
  if (compress0 && (diffusion_par > 0.0) && (gamma_i_BC > 0.0) && (gamma_e_BC > 0.0)) {
    ...
    Jpar_sh = Zi * N_tmp * Nbar * density * ee;
...
q_si *= - (Nbar*density*KB) / kappa_par_i;
  ...
  if (compress0) {
    if (nonlinear)
      Vepar = Vipar - B0 * (Jpar) / (N_tmp * Zi) * Vepara;
    else
      Vepar = Vipar - B0 * (Jpar) / Ne0 * Vepara;
    Vepar.applyBoundary();
  ...
  if (Landau) {
    if (Landau_coll) {
      q_par_i = -Landau_coeff * N0 * 2. * sqrt(2. / PI) * vth_i / Va * iSign_kpar_wcoll(Ti, kappa_i, nLorentzian);
      q_par_e = -Landau_coeff * Ne0 * 2. * sqrt(2. / PI) * vth_e / Va * iSign_kpar_wcoll(Te, kappa_e, nLorentzian);
    } else {
      q_par_i = -Landau_coeff * N0 * 2. * sqrt(2. / PI) * vth_i / Va * iSign_kpar(Ti, k0, nLorentzian);
      q_par_e = -Landau_coeff * Ne0 * 2. * sqrt(2. / PI) * vth_e / Va * iSign_kpar(Te, k0, nLorentzian);
    }
  ...
  if (!emass) {
    ...
    if (viscos_par > 0.0)
      ddt(Psi) += viscos_par * Grad2_par2(Psi); // Parallel viscosity
  } else {
    ddt(Ajpar) = 0.0;
    ...
    if (viscos_par > 0.0)
      ddt(Ajpar) += viscos_par * Grad2_par2(Ajpar); // Parallel viscosity
  }
  ...
  if (output_Titerms)
    Ti_advection -= bracket(B0 * phi, Ti0, bm_exb);

  if (continuity) {
    ddt(Ti) -= 4.0 / 3.0 * Ti0 / B0 * b0xcv * Grad(phi * B0);
    if (output_Titerms)
      Ti_gradphi -= 4.0 / 3.0 * Ti0 / B0 * b0xcv * Grad(phi * B0);
    if (diamag) {
      ddt(Ti) -= 4.0 / 3.0 * Tipara2 * Ti0 / N0 * b0xcv * Grad(Pi) / B0;
      if (output_Titerms)
        Ti_diamag -= 4.0 / 3.0 * Tipara2 * Ti0 / N0 * b0xcv * Grad(Pi) / B0;
    }
    if (nonlinear) {
      ddt(Ti) -= 4.0 / 3.0 * Ti / B0 * b0xcv * Grad(phi * B0);
      if (output_Titerms)
        Ti_gradphi -= 4.0 / 3.0 * Ti / B0 * b0xcv * Grad(phi * B0);
      if (diamag) {
        ddt(Ti) -= 4.0 / 3.0 * Tipara2 * Ti / N0 * b0xcv * Grad(Pi) / B0;
        if (output_Titerms)
          Ti_diamag += 4.0 / 3.0 * Tipara2 * Ti / N0 * b0xcv * Grad(Pi) / B0;
      }
    }
  }
  ...
  if (energy_flux) {
    ...
    if (output_Titerms) {
      Ti_energy_flux -= 10.0 / 3.0 * Tipara2 / B0 * V_dot_Grad(Ti0 * b0xcv, Ti);
      Ti_energy_flux -= 10.0 / 3.0 * Tipara2 * Ti / B0 * b0xcv * Grad(Ti0);
    }
    if (nonlinear) {
      // ddt(Ti) -= 10.0/3.0 * Tipara2 * Ti/B0 * b0xcv*Grad(Ti);
      ddt(Ti) -= 10.0 / 3.0 * Tipara2 / B0 * V_dot_Grad(Ti * b0xcv, Ti);
      if (output_Titerms)
        Ti_energy_flux -= 10.0 / 3.0 * Tipara2 / B0 * V_dot_Grad(Ti * b0xcv, Ti);
    }
  }
  ...
  if (diamag) {
    ...
    if (output_Titerms)
      Ti_advection -= bracket(phi0 * B0, Ti, bm_exb); // Equilibrium flow
  }

  if (nonlinear) {
    ddt(Ti) -= bracket(phi * B0, Ti, bm_exb); // Advection
    if (output_Titerms)
      Ti_advection -= bracket(phi * B0, Ti, bm_exb); // Advection
  }
  ...
  if (compress0) {
    ...
    if (continuity) {
      ...
      if (output_Titerms)
        Ti_gradvepar -= 2.0 / 3.0 * Ti0 * B0 * Grad_parP(Vipar / B0, CELL_CENTRE);
    }
    if (nonlinear) {
      ...
      if (continuity) {
        ...
        if (output_Titerms)
          Ti_gradvepar -= 2.0 / 3.0 * Ti * B0 * Grad_par(Vipar / B0, CELL_CENTRE);
      }
    }
  }
  ...
  if (energy_exch) {
    ...
    if (output_Titerms)
      Ti_energy_exch += 2.0 * Zi * Tbar * nu_e / (ratio_pe * AA) * (Te - Ti);
  }

  if (Xi_Tix > 0.0)
    ddt(Ti) += Xi_Tix * D2DX2(Ti.DC());
  ...
  if (diffusion_par > 0.0) {
    if (!Landau) {
      ...
      ddt(Ti) -= Grad_par(q_par_i) * 2. / (3. * N0);
      if (diff_par_flutter)
        ddt(Ti) -= -bracket(Psi, q_par_i, bm_mag) * B0 * 2. / (3. * N0);
    }
  }
  ...
  // left edge sink terms
  if (sink_Til > 0.0) {
    ddt(Ti) -= sink_Til * sink_tanhxl(P0, Ti, sTi_widthl, sTi_lengthl); // core sink
  }

  // right edge sink terms
  if (sink_Tir > 0.0) {
    ddt(Ti) -= sink_Tir * sink_tanhxr(P0, Ti, sTi_widthr, sTi_lengthr); // sol sink
  }
  ...
  if (output_vradial) {
    Vexb = (B0vec ^ Grad(phi)) / B0;
    mesh->communicate(Vexb);
    Vexb.applyBoundary();
    Vexb0 = (B0vec ^ Grad(phi0)) / B0;
    mesh->communicate(Vexb0);
    Vexb0.applyBoundary();
  }

  ddt(Te) -= bracket(B0 * phi, Te0, bm_exb);
  if (output_Teterms)
    Te_advection -= bracket(B0 * phi, Te0, bm_exb);
  if (output_Tevegradte)
    Te_vegradte0 -= bracket(B0 * phi, Te0, bm_exb);

  if (continuity) {
    ddt(Te) -= 4.0 / 3.0 * Te0 / B0 * b0xcv * Grad(phi * B0);
    if (output_Teterms)
      Te_gradphi -= 4.0 / 3.0 * Te0 / B0 * b0xcv * Grad(phi * B0);
    if (diamag) {
      ddt(Te) += 4.0 / 3.0 * Tepara2 * Te0 / Ne0 * b0xcv * Grad(Pe) / B0;
      if (output_Teterms)
        Te_diamag += 4.0 / 3.0 * Tepara2 * Te0 / Ne0 * b0xcv * Grad(Pe) / B0;
    }

    if (nonlinear) {
      ddt(Te) -= 4.0 / 3.0 * Te / B0 * b0xcv * Grad(phi * B0);
      if (output_Teterms)
        Te_gradphi -= 4.0 / 3.0 * Te / B0 * b0xcv * Grad(phi * B0);
      if (diamag) {
        ddt(Te) += 4.0 / 3.0 * Tepara2 * Te / N0 * b0xcv * Grad(Pe) / B0;
        if (output_Teterms)
          Te_diamag += 4.0 / 3.0 * Tepara2 * Te / N0 * b0xcv * Grad(Pe) / B0;
      }
    }
  }
  ...
  if (energy_flux) {
    ...
    if (output_Teterms) {
      Te_energy_flux -= 10.0 / 3.0 * Tepara2 / B0 * V_dot_Grad(-Te0 * b0xcv, Te);
      Te_energy_flux += 10.0 / 3.0 * Tepara2 * Te / B0 * b0xcv * Grad(Te0);
    }
    if (thermal_force) {
    ...
    if (nonlinear) {
      ...
      if (output_Teterms)
        Te_energy_flux -= 10.0 / 3.0 * Tepara2 / B0 * V_dot_Grad(-Te * b0xcv, Te);
      if (thermal_force)
  ...
  if (diamag) {
    ...
    if (output_Teterms)
      Te_advection -= bracket(B0 * phi0, Te, bm_exb); // Equilibrium flow
    if (output_Tevegradte)
      Te_ve0gradte -= bracket(B0 * phi0, Te, bm_exb); // Equilibrium flow
  }

  if (nonlinear) {
    ddt(Te) -= bracket(B0 * phi, Te, bm_exb); // Advection
    if (output_Teterms)
      Te_advection -= bracket(B0 * phi, Te, bm_exb); // Advection
    if (output_Tevegradte)
      Te_vegradte -= bracket(B0 * phi, Te, bm_exb); // Advection
  }
  ...
  if (compress0) {
    ...
    if (continuity) {
      ...
      if (output_Teterms)
        Te_gradvepar -= 2.0 / 3.0 * Te0 * B0 * Grad_parP(Vepar / B0, CELL_CENTRE);
    }
    if (nonlinear) {
      ...
      if (continuity) {
        ...
        if (output_Teterms)
          Te_gradvepar -= 2.0 / 3.0 * Te * B0 * Grad_par(Vepar / B0, CELL_CENTRE);
      }
    }
  }
  ...
  if (energy_exch) {
    ...
    if (output_Teterms)
      Te_energy_exch -= 2.0 * Tbar * nu_e / (ratio_pe * AA) * (Te - Ti);
    ...
    if (spitzer_resist) {
      ...
      if (output_Teterms)
        if (BScurrent)
          Te_resist += 4.0 / 3.0 * Tepara4 * eta_spitzer * B0 * B0 * Jpar_BS0 * Jpar / Ne0;
        else
          Te_resist += 4.0 / 3.0 * Tepara4 * eta_spitzer * B0 * B0 * J0 * Jpar / Ne0;
    } else {
      if (BScurrent)
      ...
      if (output_Teterms)
        if (BScurrent)
          Te_resist += 4.0 / 3.0 * Tepara4 * eta * B0 * B0 * Jpar_BS0 * Jpar / Ne0;
        else
          Te_resist += 4.0 / 3.0 * Tepara4 * eta * B0 * B0 * J0 * Jpar / Ne0;
    }
    if (nonlinear) {
      if (spitzer_resist)
      ...
      if (output_Teterms)
        if (spitzer_resist)
          Te_resist += 2.0 / 3.0 * Tepara4 * eta_spitzer * B0 * B0 * Jpar * Jpar / Ne0;
        else
          Te_resist += 2.0 / 3.0 * Tepara4 * eta * B0 * B0 * Jpar * Jpar / Ne0;
    }
  }

  if (Xi_Tex > 0.0)
    ddt(Te) += Xi_Tex * D2DX2(Te.DC());
  ...
  if (diffusion_par > 0.0) {
    if (!Landau) {
      ...
      ddt(Te) -= Grad_par(q_par_e) * 2. / (3. * Ne0);
      if (diff_par_flutter)
        ddt(Te) -= -bracket(Psi, q_par_e, bm_mag) * B0 * 2. / (3. * Ne0);
    }
  }
