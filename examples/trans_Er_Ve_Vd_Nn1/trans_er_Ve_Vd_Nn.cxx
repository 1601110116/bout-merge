...
Field2D psixy;
// 3D evolving fields
Field3D  Te, Ne,Ni, Vi, Ve, Ti, Nn,Tn, Vn, Nm ,Vmx;         
Field3D temp_Ni,temp_Nn,temp_Nm,temp_Ti,temp_Te;    
Field3D Nn_grad,Nn_grad1,Nn_grad2,Nn_grad3;
...
Field3D S_pi_ext,S_Ee_ext,S_Ei_ext; 
...
bool noshear, include_curvature,include_Nn,curvature_phi,diamag,energy_flux,Turb_Diff_on;
...
bool terms_recombination,terms_Gradpar_pn,terms_Gradpar_eta0n,terms_cross,terms_exb,include_J0,J1_all,J1_no_density,terms_Ve,terms_radial_conv,thermal_force;
...
void WallBC_GradY(Field3D &var, const Field3D &value);
void WallBC_GradY_up(Field3D &var, const Field3D &value);
void WallBC_GradY_down(Field3D &var, const Field3D &value);
...
BoutReal sbc_lambda0, sbc_lambda1, D_perp,Mu_perp, Mu_para, RF_coef;
Vector3D er00, er0, ersh0,ersh1,Vexb,Vdia,Vdia_e;
Field3D phi0,phi00, phi01, U00, eta_sh, phish0, phish1, dphi01dx, d2phishdx2;
Field3D term_Mu_perp,term_Mu_par,term_J1,term_J0,term_curvature,term_exb,J1_para,J1_phi,J1_Te,J1_Pe;

//---------------------------for testing all the terms in all Eqs-------------------------------
Field3D ni01,ni02,ni03,ni04,ni05,ni06,ni07,ni08,ni09,ni10;
Field3D te01,te02,te03,te04,te05,te06,te07,te08,te09,te10,te11,te12,te13,te14;
Field3D ti01,ti02,ti03,ti04,ti05,ti06,ti07,ti08,ti09,ti10,ti11,ti12,ti13,ti14,ti15,ti16;
Field3D vi01,vi02,vi03,vi04,vi05,vi06,vi07,vi08;
Field3D ph01,ph02,ph03,ph04;
Field3D uu01,uu02,uu03,uu04,uu05,uu06,uu07;
Field3D nn01,nn02,nn03,nn04,nn05;
...
// Load 2D profiles
  ...
  OPTION(options, minimum_val,  1.e-10);    // minimum value limit for densities
  ...
  OPTION(options, include_Nn, false);
  ...
  OPTION(options, J1_no_density, false);  
  ...
  OPTION(options, C_fe_sheat, 7.0);
  OPTION(options, C_fi_sheat, 2.5);
 ...
 if(!include_curvature)
  ...
  // Set BOUNDARiES first here, and then apply them every time in physics run/////
   ...
   Nn_grad.setBoundary("Tn");
   Nn_grad1.setBoundary("Tn");
   Nn_grad2.setBoundary("Tn");
   Nn_grad3.setBoundary("Tn");
 ...
 if (BScurrent)
  ...
  dump.add(nu_ionz_1, "nu_ionz_1",1);
  dump.add(Nn_grad, "Nn_grad",1);
  dump.add(Nn_grad1, "Nn_grad1",1);
  dump.add(Nn_grad2, "Nn_grad2",1);
  dump.add(Nn_grad3, "Nn_grad3",1);
  dump.add(ni01, "ni01",1);
  dump.add(ni02, "ni02",1);
  dump.add(ni03, "ni03",1);
  dump.add(ni04, "ni04",1);
  dump.add(ni05, "ni05",1);
  dump.add(ni06, "ni06",1);
  dump.add(ni07, "ni07",1);
  dump.add(ni08, "ni08",1);
  dump.add(ni09, "ni09",1);
  dump.add(ni10, "ni10",1);
  dump.add(te01, "te01",1);
  dump.add(te02, "te02",1);
  dump.add(te03, "te03",1);
  dump.add(te04, "te04",1);
  dump.add(te05, "te05",1);
  dump.add(te06, "te06",1);
  dump.add(te07, "te07",1);
  dump.add(te08, "te08",1);
  dump.add(te09, "te09",1);
  dump.add(te10, "te10",1);
  dump.add(te11, "te11",1);
  dump.add(te12, "te12",1);
  dump.add(te13, "te13",1);
  dump.add(te14, "te14",1);
  dump.add(ti01, "ti01",1);
  dump.add(ti02, "ti02",1);
  dump.add(ti03, "ti03",1);
  dump.add(ti04, "ti04",1);
  dump.add(ti05, "ti05",1);
  dump.add(ti06, "ti06",1);
  dump.add(ti07, "ti07",1);
  dump.add(ti08, "ti08",1);
  dump.add(ti09, "ti09",1);
  dump.add(ti10, "ti10",1);
  dump.add(ti11, "ti11",1);
  dump.add(ti12, "ti12",1);
  dump.add(ti13, "ti13",1);
  dump.add(ti14, "ti14",1);
  dump.add(ti15, "ti15",1);
  dump.add(ti16, "ti16",1);
  dump.add(vi01, "vi01",1);
  dump.add(vi02, "vi02",1);
  dump.add(vi03, "vi03",1);
  dump.add(vi04, "vi04",1);
  dump.add(vi05, "vi05",1);
  dump.add(vi06, "vi06",1);
  dump.add(vi07, "vi07",1);
  dump.add(vi08, "vi08",1);
  dump.add(ph01, "ph01",1);
  dump.add(ph02, "ph02",1);
  dump.add(ph03, "ph03",1);
  dump.add(ph04, "ph04",1);
  dump.add(uu01, "uu01",1);
  dump.add(uu02, "uu02",1);
  dump.add(uu03, "uu03",1);
  dump.add(uu04, "uu04",1);
  dump.add(uu05, "uu05",1);
  dump.add(uu06, "uu06",1);
  dump.add(uu07, "uu07",1);
  dump.add(nn01, "nn01",1);
  dump.add(nn02, "nn02",1);
  dump.add(nn03, "nn03",1);
  dump.add(nn04, "nn04",1);
  dump.add(nn05, "nn05",1);
  return(0);
}
...
// Thermal Speed normalized in ion thermal speed V_thi
  ...
  if(Secondary_electron_emission) 
    {q_se_kappa=-c_se*Ni*Te*(2.0/(1.0-delta)+1.0/2.0*log((1.-delta)*(1.-delta)*Mi2/Me/2./PI/(1.+Ti/Te)))/kappa_Te;
  q_si_kappa=-c_se*Ni*2.0*Ti/kappa_Ti;}
  else {q_se_kappa = -C_fe_sheat*Ni*Te*c_se/kappa_Te;                                     // '-' means out-flowing to Sheath
  ...
  if(J1_all){J1_para =  -1.0/(eta*sbc_lambda1*B0)*Grad_par(phi01)+((KB*eV_K*Te_x*tbar)/(eta*Lbar*Lbar*ee*Ni*B0*Bbar))*Grad_par((Te*Ni))+((0.71*KB*eV_K*Te_x*tbar)/(eta*Lbar*Lbar*B0*Bbar*ee))*Grad_par(Te);
  ...
  if (iterative_phi0)
      {  phish0 = sbc_lambda1*0.5*log((Mi2/Me)/(4.0*PI)*(2*temp_Te*Te_x/(temp_Ti*Ti_x+temp_Te*Te_x)))*(temp_Te*Te_x*KB*eV_K)/ee;
        if(Secondary_electron_emission) {phish0 = sbc_lambda1*0.5*log((Mi2/Me)*(1-delta)*(1-delta)/(4.0*PI)*(2*temp_Te*Te_x/(temp_Ti*Ti_x+temp_Te*Te_x)))*(temp_Te*Te_x*KB*eV_K)/ee;}
        if (J_para0) phish0 -= sbc_lambda1*(temp_Te*Te_x*KB*eV_K/ee)*log(1-J1_para*B0*Bbar/(MU0*Lbar)/(temp_Ni*Ni_x*density_unit*ee*sqrt((temp_Ti*Ti_x+temp_Te*Te_x)*eV_K/Mi2)));
...
for (int jx=0;jx<mesh->ngx;jx++)
     ...
     for (int jy=0;jy<mesh->ngy;jy++)
         {
          for (int jz=0;jz<mesh->ngz;jz++)
              { 
  nu_ionz[jx][jy][jz] = 38.5*temp_Nn[jx][jy][jz]*Ni_x*exp(1.272e-5-(temp_Ti[jx][jy][jz]*Te_x+3.13)/2403.9)*erfc(0.00356-(temp_Ti[jx][jy][jz]*Te_x+3.13)/47.745);   //Ni_x in unit 1e^20 
              }
         }
    }  

  nu_ionz_1=nu_ionz/temp_Nn;  
 ...
 //Dissociation rate of molecules
  ...
  if(terms_recombination) Diffc_nn_perp = Tn/Mn/(nu_CX_n+S_rec/temp_Nn);
  V_th_n=sqrt(Tn/Mn);
  Diffc_nn_perp_fl=V_th_n*Lnn_min/Lbar;
  Diffc_nn_perp *= Diffc_nn_perp_fl/(15.0*(Diffc_nn_perp+Diffc_nn_perp_fl));                 // Diffc_nn_perp calculated once for SBC and again later
  Diffc_nn_par = Tn/Mn/nu_CX_n;
  if(terms_recombination) Diffc_nn_par = Tn/Mn/(nu_CX_n+S_rec/temp_Nn);
  Diffc_nn_par *= Diffc_nn_perp_fl/(8.0*(Diffc_nn_par+Diffc_nn_perp_fl)); 
  ...
  Nn_grad=Gamma_nn/Diffc_nn_par;
  Nn_grad1=Grad_par(Nn);
...
//   output.write("Now updata Ni \n"); 
 // ddt(Ni) = 0.; 
  Pe = Te*Ni;
  Pi = Ti*Ni;
  ddt(Ni) =  (1.0-F_iol1)*(D_perp/(Lbar/tbar*Lbar)+Diff_ni_perp)*Delp2(Ni)
            - Ni*Grad_par(Vi)
            - Vpar_Grad_par(Vi,Ni)
            + Si_p; 
  if(terms_recombination) ddt(Ni) -= S_rec;
  if(terms_exb) ddt(Ni) -= (1.0-F_iol1)*bracket((phi01/sbc_lambda1), Ni, bm_exb);
  if(curvature_phi) ddt(Ni) -= test_f1*(1.0-F_iol1)*2.0*Ni*b0xcv*Grad((phi01/sbc_lambda1))/B0;  // curvature term
  if(diamag)  ddt(Ni) -= test_f2*(1.0-F_iol1)*2.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*b0xcv*Grad(Pi)/B0;  // curvature term
...
ni01 = (1.0-F_iol1)*(D_perp/(Lbar/tbar*Lbar)+Diff_ni_perp)*Delp2(Ni);
ni02 = -Ni*Grad_par(Vi);
ni03 = -Vpar_Grad_par(Vi,Ni);
ni04 = Si_p;
ni05 = -S_rec;
ni06 = -(1.0-F_iol1)*bracket((phi01/sbc_lambda1), Ni, bm_exb);
ni07 = -test_f1*(1.0-F_iol1)*2.0*Ni*b0xcv*Grad((phi01/sbc_lambda1))/B0;
ni08 = -test_f2*(1.0-F_iol1)*2.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*b0xcv*Grad(Pi)/B0;
ni09 = S_pi_ext;
ni10 = (1.0-F_iol1)*V_dot_Grad(grad_perp_Diffi,Ni);


  // ELECTRON TEMPERATURE   ---Te---
   //output.write("Now updata Te \n");
  Vector3D V_diff = Diff_ni_perp*Grad_perp(Ni)/Ni;
  ...
  ddt(Te) =   0.6667*kappa_Te*Grad2_par2(Te)/temp_Ni
             ...
             - 2.*fmei*(Te-Ti)/tau_ei
             ;
   if(terms_Ve) ddt(Te) -= (Vpar_Grad_par(Vi,Te) + 0.6667*Te*Grad_par(Vi));
   if(terms_radial_conv) ddt(Te) +=0.6667*Te*Div(V_diff)+Diff_ni_perp*Grad_perp(Ni)*Grad_perp(Te)/Ni;
//  if(spitzer_resist) ddt(Te) += 2.0/3.0*(Bbar*Bbar/(MU0*Ni_x*density_unit*Te_x*KB*eV_K))*eta*B0*B0*J1_para*J1_para/Ni;
//  if(thermal_force) ddt(Te) += 0.71*2.0/3.0*(tbar*Bbar/(Ni_x*density_unit*ee*Lbar*Lbar*MU0))*Te*B0/Ni*Grad_par(J1_para);
   ...
   if(terms_exb) ddt(Te) -= bracket((phi01/sbc_lambda1), Te, bm_exb);
   if(curvature_phi) ddt(Te) -= 4.0/3.0*Te*b0xcv*Grad(phi00)/B0;  // curvature term
   if(diamag)  ddt(Te) += 4.0/3.0*(KB*Te_x*eV_K/(ee*Bbar*Lbar*Lbar/tbar))*Te/temp_Ni*b0xcv*Grad(Pe)/B0;  // curvature term
   if(energy_flux)  ddt(Te) += 10.0/3.0*(KB*Te_x*eV_K/(ee*Bbar*Lbar*Lbar/tbar))*Te*b0xcv*Grad(Te)/B0;  // curvature term
...
te01 = 0.6667*kappa_Te*Grad2_par2(Te)/temp_Ni;
te02 = 0.6667*Grad_par(kappa_Te)*Grad_par(Te)/temp_Ni;
te03 = 0.6667*chi_e_perp*Delp2(Te);
te04 = -nu_ionz*(Te+0.6667*W_ionz); 
te05 = -2.*fmei*(Te-Ti)/tau_ei;
te06 = -(Vpar_Grad_par(Vi,Te) + 0.6667*Te*Grad_par(Vi));
te07 = 0.6667*Te*Div(V_diff)+Diff_ni_perp*Grad_perp(Ni)*Grad_perp(Te)/Ni;
te08 = nu_rec*W_rec;
te09 = -bracket((phi01/sbc_lambda1), Te, bm_exb);
te10 = -4.0/3.0*Te*b0xcv*Grad(phi00)/B0;
te11 = 4.0/3.0*(KB*Te_x*eV_K/(ee*Bbar*Lbar*Lbar/tbar))*Te/temp_Ni*b0xcv*Grad(Pe)/B0;
te12 = 10.0/3.0*(KB*Te_x*eV_K/(ee*Bbar*Lbar*Lbar/tbar))*Te*b0xcv*Grad(Te)/B0;
te13 = 0.6667*S_Ee_ext/temp_Ni;
te14 = 0.6667*V_dot_Grad(grad_perp_chie,Te);
...
//   output.write("Now updata Ti \n");
   ...
   if(terms_exb) ddt(Ti) -= (1.0-E_iol1)*bracket((phi01/sbc_lambda1), Ti, bm_exb);
   if(curvature_phi) ddt(Ti) -= (1.0-E_iol1)*4.0/3.0*Ti*b0xcv*Grad(phi00)/B0;  // curvature term
   if(diamag)  ddt(Ti) -= (1.0-E_iol1)*4.0/3.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*Ti/temp_Ni*b0xcv*Grad(Pi)/B0;  // curvature term
   if(energy_flux)  ddt(Ti) -= (1.0-E_iol1)*10.0/3.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*Ti*b0xcv*Grad(Ti)/B0;  // curvature term
...
ti01 = -Vpar_Grad_par(Vi,Ti);
ti02 = -0.6667*Ti*Grad_par(Vi);
ti03 = (1.0-E_iol1)*0.6667*kappa_Ti*Grad2_par2(Ti)/temp_Ni;
ti04 = (1.0-E_iol1)*0.6667*Grad_par(kappa_Ti)*Grad_par(Ti)/temp_Ni;
ti05 = (1.0-E_iol1)*0.6667*chi_i_perp*Delp2(Ti);
ti06 = -nu_ionz*Ti;
ti07 = -0.6667*nu_CX*(Ti-Tn);
ti08 = 2.*fmei*(Te-Ti)/tau_ei;
ti09 = 0.6667*Ti*Div(V_diff)+Diff_ni_perp*Grad_perp(Ni)*Grad_perp(Ti)/Ni;
ti10 = nu_rec*Ti;
ti11 = -(1.0-E_iol1)*bracket((phi01/sbc_lambda1), Ti, bm_exb);
ti12 = -(1.0-E_iol1)*4.0/3.0*Ti*b0xcv*Grad(phi00)/B0;
ti13 = -(1.0-E_iol1)*4.0/3.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*Ti/temp_Ni*b0xcv*Grad(Pi)/B0;
ti14 = -(1.0-E_iol1)*10.0/3.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*Ti*b0xcv*Grad(Ti)/B0;
ti15 = 0.6667*S_Ei_ext/temp_Ni;
ti16 = (1.0-E_iol1)*0.6667*V_dot_Grad(grad_perp_chii,Ti);
...
//  output.write("Now updata Vi \n");
   ...
   if(terms_exb) ddt(Vi) -= bracket((phi01/sbc_lambda1), Vi, bm_exb);
   if(terms_IOL) ddt(Vi) +=V_iol1;

vi01 = -Vpar_Grad_par(Vi,Vi);
vi02 = -Grad_par(pei)/temp_Ni/Mi;
vi03 = 2.*0.6667*Grad_par(eta0_i)*Grad_par(Vi)/temp_Ni/Mi;
vi04 = 2.*0.6667*eta0_i*Grad2_par2(Vi)/temp_Ni/Mi;
vi05 = -(nu_ionz+nu_CX)*(Vi-Vn);
vi06 = Diff_ni_perp*Grad_perp(Ni)*Grad(Vi)/Ni;
vi07 = -bracket((phi01/sbc_lambda1), Vi, bm_exb);
vi08 = V_iol1;
    ...
    if (terms_cross) ddt(phi01) += sbc_lambda0/(B0*sbc_lambda1)*Grad(Ni)*Grad(phi01);  
      ...
      phi00=phi01/sbc_lambda1;
ph01 = (sbc_lambda0*Ni)/(sbc_lambda1*B0)*Laplace_perp(phi01);
ph02 = (sbc_lambda0*KB*eV_K*Ti_x)/(Lbar/tbar*Lbar*Bbar*Zi*ee*B0)*Laplace_perp(Pi);
ph03 = -sbc_lambda0*U00;
ph04 = sbc_lambda0/(B0*sbc_lambda1)*Grad(Ni)*Grad(phi01);
    ...
    if(include_curvature)  ddt(U00) += test_f3*2.0*KB*Te_x*eV_K/(Mi2*Lbar*Lbar/(tbar*tbar))*Div(pei*b0xcv);  // curvature term
    if(terms_exb) ddt(U00) -=bracket((phi01/sbc_lambda1), U00, bm_exb);
    if(terms_radial_conv) ddt(U00) +=Diff_ni_perp*Grad_perp(Ni)*Grad(U00)/Ni;

uu01 = (B0*B0*Bbar*Bbar/(MU0*Mi2*Ni_x*density_unit*Lbar*Lbar/tbar/tbar))*Grad_par(J1_para);
uu02 = (Mu_perp/(Lbar/tbar*Lbar))*Laplace_perp(U00);
uu03 = (Mu_para/(Lbar/tbar*Lbar))*Laplace_par(U00);
uu04 = (B0*B0*Bbar*Bbar/(MU0*Mi2*Ni_x*density_unit*Lbar*Lbar/tbar/tbar))*Grad_par(J0);
uu05 = test_f3*2.0*KB*Te_x*eV_K/(Mi2*Lbar*Lbar/(tbar*tbar))*Div(pei*b0xcv);
uu06 = -bracket((phi01/sbc_lambda1), U00, bm_exb);
uu07 = Diff_ni_perp*Grad_perp(Ni)*Grad(U00)/Ni;
...
//******** parallel heat flux *******************//
      ...
      heatf_conv_e1 = 2.5*Ni*Te*Vi*Lbar/tbar*Te_x*ee*Ni_x*density_unit;
...
*/
  if(terms_NnGradpar_Vn) ddt(Nn) -= ( Nn*Grad_par(Vi)+Vpar_Grad_par(Vi,Nn));              //added by gzdeng
 
  if(terms_Diffcnn_par)  ddt(Nn) += ( Diffc_nn_par*Nn*Grad_par(Grad_par_logNn)
                                      +Nn*Grad_par_logNn*Grad_par(Diffc_nn_par)
                                     );
  }
nn01 = Diffc_nn_perp*Delp2(Nn);
nn02 = -Si_p;
nn03 = S_rec;
nn04 = -( Nn*Grad_par(Vi)+Vpar_Grad_par(Vi,Nn));
nn05 = ( Diffc_nn_par*Nn*Grad_par(Grad_par_logNn)
                                      +Nn*Grad_par_logNn*Grad_par(Diffc_nn_par)
                                     );
...

{
  ...
  WallBC_GradY_up(var, value);

  WallBC_GradY_down(var, value); // Fluxes go towards X point and hit on plates, thus SBC at ydown or theta=0 should be different
...
void WallBC_GradY_up(Field3D &var, const Field3D &value)
{
  ...
  for(int jy=0;jy<mesh->ngy;jy++)
    {
     if ( mesh->YGLOBAL (jy) < 3)
       {
             for(int jz=0;jz<mesh->ngz;jz++)
                {
                  var[xrup.ind][jy][jz] = var[xrup.ind][jy-1][jz] - value[xrup.ind][jy][jz]*mesh->dy[xrup.ind][jy]*sqrt(mesh->g_22[xrup.ind][jy]);  // calculated in BOUT psi coordinate
                 }
       }
     }
  } 
...
void WallBC_GradY_down(Field3D &var, const Field3D &value)
{
  ...
  for(int jy=mesh->ngy;jy>0;jy--)
    {
     if ( mesh->YGLOBAL (jy) > NY - 3 )
       {
             for(int jz=0;jz<mesh->ngz;jz++)
                {
                  var[xrup.ind][jy][jz] = var[xrup.ind][jy-1][jz] + value[xrup.ind][jy][jz]*mesh->dy[xrup.ind][jy]*sqrt(mesh->g_22[xrup.ind][jy]);  // calculated in BOUT psi coordinate
