...
// filter 
BoutReal filter_para,filter_perp;
...
bool Diffcoefs_couple_dirichletBC;     // set zero dirichlet BC. for Diffusion coefficients
bool Diagnose;
bool Evolving_Eqn_Ni,Evolving_Eqn_Ti,Evolving_Eqn_Te,Evolving_Eqn_Vi;
...
bool nlfilter_Gradperp_diffcoefs;
...
// Load 2D profiles
// N.B. there should be any "if(bool_ctrl)" control for the "mesh->get" quantities since bool_ctrl is "false" as default
// 
  mesh->get(Te_grid, "Te0");    // eV          // not time dependent for Diffcoefs_Hmode calculation
  ...
  OPTION(options, nlfilter_Gradperp_diffcoefs, false);
  OPTION(options, filter_perp, 0.20);

  OPTION(options, Diagnose, false);  // diagnose control for more output informations 
  ...
  OPTION(options, aveY_g11J_xin,  0.005);  // Read in a.u., NB: value changed according to grid file, it together with dNidx_xin_au for Flux_xi
  ...
  OPTION(options, Diffcoefs_couple_dirichletBC,  false);  // set zero dirichlet BC. for Diffusion coefficients
 ...
 // bmag in SI unit Tesla multiply by 1.e4 changes to Gaussian unit gauss 
  ...
  Ni_x *= (density_unit/1.0e6);    // in unit cm^-3 now
 ...
 // bmag in Gaussian unit gauss divided by 1.e4 changes back to Gaussian SI unit Tesla
  ...
  Ni_x /= (density_unit/1.0e6);           // back to unit  m^-3 mow
  bmag /=1.0e4;                           // back to unit tesla now
  Vi_x /= 1.e2;                           // back to unit m/s

  tbar = Lbar/Vi_x;                       // in unit s

  N_cubic_Lbar = Ni_x*density_unit*Lbar*Lbar*Lbar;  // unit 1
 ...
 if(!include_curvature)
  ...
  if(Diagnose) output.write("Diagnosing: Normalizing the Psi quantities \n");
  //mesh->dx /= Lbar;
  I   *= Lbar*Lbar*bmag;
  ...
  if(Diagnose) output.write("Diagnosing: Normalizing the Magnetic field quantities \n");
  Bpxy /= bmag;
  Btxy /= bmag;
  mesh->Bxy  /= bmag;
  
  // Normalise coefficients
  if(Diagnose) output.write("Diagnosing: Normalizing the some coefficients  \n");
  Lni_wall /= Lbar;
  Lp_crit /= Lbar;
  W_ionz /= Te_x;
  W_diss /= Te_x;
  ...
  if(Diagnose) output.write("Diagnosing: Normalizing the evolving quantities \n");
  Te_grid /= Te_x;
  Ti_grid /= Te_x;
  Ni_grid /= Ni_x;  //hl2a grid in unit 10^19/m^3 
  if(couple_to_6field)
  {
  Te_grid_couple /= Te_x;
  Ti_grid_couple /= Te_x;
  ...
  if(Diagnose) output.write("Diagnosing: Normalizing the Diffusion Coefficients quantities \n");
  ...
  if(Diagnose) output.write("Diagnosing: Normalizing the external sources quantities \n");
       ...
       if (load_grid_profiles)
        {
         ...
         Ni0_Hmode = Ni_grid;      // the initial Hmode grid profile no time dependent except coupling to EFIT later  
         Ti0_Hmode = Ti_grid;      // TiNiTe_Hmode are used for Hmode coefficients only
         Te0_Hmode = Te_grid;
         Te = Te_grid_couple;
         Ti = Ti_grid_couple;
         Ni = Ni_grid_couple;
  ...
  //if (diffusion_coef_Hmode )
   ...
   BoutReal Gamma_ni_xin = -diffusion_coef_Hmode0*aveY_g11J_xin*dNidx_xin_au;
   BoutReal Qi_xin = -diffusion_coef_Hmode0*aveY_g11J_xin*dTidx_xin_au;
   BoutReal Qe_xin = -diffusion_coef_Hmode0*aveY_g11J_xin*dTedx_xin_au;

   output.write(" Gamma_ni_xin %e,  diffusion_coef_Hmode0 %e , aveY_g11J_xin %e, dNidx_xin_au %e \n", Gamma_ni_xin ,diffusion_coef_Hmode0,aveY_g11J_xin,dNidx_xin_au);
  for (int jx=0;jx<mesh->ngx;jx++)
     {
 ...
 //    if((jx_global<=2) & (jy_global ==32) )
 //    {
 //  output.write(" jx %d, diffusion_coef_Hmode %e  vs. %e \n",jx_global, diffusion_coef_Hmode0,Diffc_ni_Hmode[jx][jy][jz]);
 //  output.write(" jx %d, aveY_g11J_xin %e vs %e \n", jx_global,aveY_g11J_xin,aveY_g11J[jx][jy]);
 //  output.write(" jx %d, dNidx_xin_au %e vs %e \n", jx_global,dNidx_xin_au,DDX_Ni[jx][jy][jz]);
  //  } 
		...
		if(diffusion_PF_constant)
			...
			Diffc_ni_Hmode[jx][jy][jz]=diffusion_coef_Hmode0;
              ...
              if(couple_to_6field && Diffcoefs_couple_dirichletBC);  // set zero dirichlet BC. for Diffusion coefficients
              {
                if(jx_global <=1 || jx_global >= NX-2)
                 {
                  Diff_perp_couple[jx][jy][jz]=minimum_val*minimum_val;
                  chi_i_perp_couple[jx][jy][jz]=minimum_val*minimum_val;
                  chi_e_perp_couple[jx][jy][jz]=minimum_val*minimum_val;
                 }
              }
 ...
 // Ionization rate  depending on Nn for Plasmas 
  ...
  nu_CX =  1.e5*Nn*Ni_x*(1.7+1.9*(((1.5*Ti*Te_x)^0.333)-2.466)/(((150.*Ti*Te_x)^0.333)-2.466));      // empirical formula 
 ...
 if (BScurrent)
  ...
  {
   if(Evolving_Eqn_Ni) dump.add(grad_perp_Diffi,"grad_perp_Diffi",1);
   if(Evolving_Eqn_Ti) dump.add(grad_perp_chii,"grad_perp_chii",1);
   if(Evolving_Eqn_Te) dump.add(grad_perp_chie,"grad_perp_chie",1);
  }
  return(0);
}
...
// Set flux limit for kappa
 ...
 // eta0_n=0.96*(1./tbar/nuiix)*(Tn^2.5)*Nn/temp_Nn;
  ...
  nu_CX =  1.e5*tbar*temp_Nn*Ni_x*(1.7+1.9*(((1.5*temp_Ti*Te_x)^0.333)-2.466)/(((150.*temp_Ti*Te_x)^0.333)-2.466));      // empirical formula 
  ...
  nu_CX_n =  1.e5*tbar*temp_Ni*Ni_x*(1.7+1.9*(((1.5*temp_Ti*Te_x)^0.333)-2.466)/(((150.*temp_Ti*Te_x)^0.333)-2.466));      // empirical formula 
...
//************************************
  // DENSITY EQUATION
//************************************
  if(Diagnose) output.write("Now updata Ni \n"); 
  if(!Evolving_Eqn_Ni)
  {
  ddt(Ni) = 0.; 
  }
  ...
  if(terms_Gradperp_diffcoefs) 
    {
      ...
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_Diffi.x,filter_perp);
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_Diffi.y,filter_perp);
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_Diffi.z,filter_perp);
...
//************************************
   ...
   if(Diagnose)output.write("Now updata Te \n");
   ...
   if(terms_Gradperp_diffcoefs) 
    {
      ...
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_chie.x,filter_perp);
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_chie.y,filter_perp);
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_chie.z,filter_perp);
...
//************************************
   ...
   if(Diagnose)output.write("Now updata Ti \n");
   ...
   if(terms_Gradperp_diffcoefs) 
    {
      ...
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_chii.x,filter_perp);
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_chii.y,filter_perp);
      if(nlfilter_Gradperp_diffcoefs)nl_filter(grad_perp_chii.z,filter_perp);
...
//************************************
  ...
  if(Diagnose)output.write("Now updata Vi \n");
