...
Field3D Di_couple, kaii_couple, kaie_couple;
BoutReal epsilon;
...
bool output_transport_couple; //output the results of transport coefficient
...
//  output.write("%i, %i, %i, %i,\n",jysep1, jysep1_2, jysep2_1, jysep2);
  ...
  OPTION(options, output_vradial,       true);
  OPTION(options, output_transport_couple, true);
  ...
  if (nonlinear)
    {
      ...
      if (impurity_prof)
	Ne_tmp = field_larger(Ne0+Ni, N_tmp1);
      else
	Ne_tmp = N_tmp;
    }
      ...
      if (output_transport_couple)
        {
         ...
         if (diffusion_par > 0.0 && diff_par_flutter)
           {
            Di_couple = Lbar*Va*(((Ni+N0)*Vexb.x*sqrt(mesh->g11)*Lbar*Bbar/(B0^2)).DC()+((Ni+N0)*Vipar*Vbtilde.x*sqrt(mesh->g11)*Lbar*Bbar).DC())/(abs(DDX(Ni.DC()+N0))+Low_limit);
            kaii_couple =(((Ti+Ti0)*Vexb.x*Lbar*Va*sqrt(mesh->g11)*Lbar*Bbar/(B0^2)).DC()-(kappa_par_i*bracket1i/Tepara1/Nbar).DC())/(abs(DDX(Ti.DC()+Ti0))+Low_limit);
            kaie_couple =(((Te+Te0)*Vexb.x*Lbar*Va*sqrt(mesh->g11)*Lbar*Bbar/(B0^2)).DC()-(kappa_par_e*bracket1i/Tepara1/Nbar).DC())/(abs(DDX(Te.DC()+Te0))+Low_limit);
           }
         else
           {
