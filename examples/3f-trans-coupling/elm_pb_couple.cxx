 ...
 *
 * 2017/08/31 modified VExB calculation by ZHWang
 *******************************************************************************/
...
// 2D inital profiles
Field2D J0, P0, Pt, P00; // Current and pressure
...
bool change_fakeN0_gridNi0;     // change fakeN0 to Ni0 in the grid
...
// calculate turbulent transport coefficients from (VExB)_r for TNT (TiNiTe) transport simulations
Field3D Di_couple, kaii_couple, kaie_couple;  // turbulent coefficients for Ni,Ti,Te; NB: they are output in the nc file in the REAL SI units
Field3D VeXp;    //Divergence flux for coupling
Field2D VeXp_zonal,Div_Flux_couple;  // zonal part for coupling
Field3D Ni, Ti, Te;              // fluctuated density and temperatures
Vector3D Er0;                    // radial E0
Vector3D Ve0,Vexb;               //  ExB Velocity
Vector3D Vbtilde;                // equal to -B0vec^Grad(Psi), magmetic fluctuation related 
BoutReal Upara0;                 // Temporary normalization constants for E/phi caculation
BoutReal Low_limit;              //To limit the negative value of total density and temperatures
BoutReal Ratio_tilde_TioverNi;   // Ratiao of (Tilde Ti)/(Tilde Ni) for Tidle Ti(Ni) calculation from P 

bool transport_couple;   //calculate the zonal pressure in the transport code
bool output_vradial;  // output the results of radial velocity, induced by ExB and magnetic flutter
bool output_transport_couple;   //output the results of turbulent transport coefficients 
bool chicouple_calc_withP;      // (k)chii_couple and (k)chie_couple calculated with P
...
//bool diamag_er;     // switch phi0 to Er0 for the calculation
...
const BoutReal Mi = 1.6726e-27; // Ion mass
...
int physics_init(bool restarting)
{
  ...
  mesh->get(Pt, "pressure_couple"); // Pascals
  ...
  OPTION(options, change_fakeN0_gridNi0, false);    // change fakeN0 to Ni0 in the grid
  ...
  // Turbulent transport coefficientr
  OPTION(options, Low_limit,         1.0e-10); //limit the negative value of total quantities
  OPTION(options, output_vradial,       false);   // output radial velocity of VExB
//  OPTION(options, diamag_er,       false);        // Using E0 instead of phi0 if true
  OPTION(options, output_transport_couple, false);
  OPTION(options, transport_couple, false);
  OPTION(options, chicouple_calc_withP,false);      // (k)chii_couple and (k)chie_couple calculated with P
  OPTION(options, Ratio_tilde_TioverNi, 1.0); // Ratiao of (Tilde Ti)/(Tilde Ni) for extracting them from P 
  ...
  if (!nonlinear)   // if linearing run, no need for turbulent transport coef. output
   {
      if (output_vradial)
        {
     ...
     if (output_transport_couple)
       {
         output_transport_couple = false;
         output.write("Linear simulation! Set output_transport_couple to false.\n");
       }
   }
  ...
  Va = sqrt(Bbar*Bbar / (MU0*density*AA*Mi));

  Tbar = Lbar / Va;

  dnorm = dia_fact * AA*Mi / (2.0*Zeff*1.602e-19*Bbar*Tbar);

  delta_i = 60.67*5.31e5/sqrt(density/1e6/AA)/(Lbar*100.0)/Zeff;
  ...
  if (constn0)
      ...
      if( (!T0_fake_prof) && n0_fake_prof )
	{
	  N0 = N0tanh(n0_height*Nbar, n0_ave*Nbar, n0_width, n0_center, n0_bottom_x);
          if(change_fakeN0_gridNi0 && output_transport_couple)mesh->get(N0, "Ni0_couple"); // in unit of 1.e20/m^3 in cbm18_den8 
	  Ti0 = P0/N0/2.0;
	  Te0 = Ti0;
	}
...
///// set boundary and location of VExB and output it
  if (output_vradial) 
    {
  ...
  if (output_transport_couple)     // output turbulent transport coef. for TNT transport (NB: they are dump.add in the nc file in the REAL SI units)
    {
      Di_couple.setLocation(CELL_YLOW);
      Di_couple.setBoundary("Dicouple");
      kaii_couple.setLocation(CELL_YLOW);
      kaii_couple.setBoundary("Dicouple");
      kaie_couple.setLocation(CELL_YLOW);
      kaie_couple.setBoundary("Dicouple");
      VeXp.setLocation(CELL_CENTRE);
      VeXp.setBoundary("P");
      VeXp_zonal.setLocation(CELL_CENTRE);
      VeXp_zonal.setBoundary("P");
      Div_Flux_couple.setLocation(CELL_CENTRE);
      Div_Flux_couple.setBoundary("P");

      dump.add(Di_couple, "Di_couple", 1);
      dump.add(kaii_couple, "kaii_couple", 1);
      dump.add(kaie_couple, "kaie_couple", 1);
      dump.add(VeXp, "VeXp", 1);
      dump.add(VeXp_zonal, "VeXp_zonal", 1);
 ...
 /*   if(transport_couple)
     {
      ...
      if(diamag_phi0)
       {VeXp += b0xGrad_dot_Grad(phi0, P+P00);}   // Equilibrium flow
      if(nonlinear)
       {VeXp += bracket(phi, P+P00, bm_exb)*B0;}    // Advection      
      mesh->communicate(VeXp);
      VeXp.applyBoundary();
      VeXp_zonal = 0.;
      
      VeXp_zonal = VeXp.DC();
      mesh->communicate(VeXp_zonal);
      VeXp_zonal.applyBoundary();

      Div_Flux_couple = VeXp_zonal*Va/Lbar*Bbar*Bbar/2.0/MU0;  //real value for transport code
      mesh->communicate(Div_Flux_couple);
      Div_Flux_couple.applyBoundary();

      ddt(P) -= VeXp - VeXp_zonal;
     }
    else
    */
    if(!transport_couple){
  ...
  if(compress0) {
   ...
   ///////////////////////////////////////////////////
   // Calculate perturbed Vexb and then 
   // use its radial component to calculated turbulent Coef.s for TNT transport
  ...
  if (output_vradial)
    {
      ...
      if (output_transport_couple)
        {
        if(chicouple_calc_withP)      // chii_couple and chie_couple calculated with P
          {
           kaii_couple = (((P+P0)*Vexb.x*Lbar*Va).DC())/(abs(DDX(P.DC()+P0))+Low_limit);
           kaie_couple = kaii_couple;
           Di_couple = 0.4*kaii_couple;   // supposed 2.5*Di = Chii which is usually the case
          }
          else
          { 
          // supposed 0.5*P = Ni*Ti0 +N0*Ti, while Ti=Te and Ti*Ni ignored
          // then Ni = P/(N0*Ratio_tilde_TioverNi+Ti0)
           Ni = P/(N0*Ratio_tilde_TioverNi+Ti0);  
           Ti = Ratio_tilde_TioverNi*Ni;
           Te = Ti;
           Di_couple =Lbar*Va*(((Ni+N0)*Vexb.x).DC())/(abs(DDX(Ni.DC()+N0))+Low_limit);
           kaii_couple = (((Ti+Ti0)*Vexb.x*Lbar*Va).DC())/(abs(DDX(Ti.DC()+Ti0))+Low_limit);
           kaie_couple = (((Te+Te0)*Vexb.x*Lbar*Va).DC())/(abs(DDX(Te.DC()+Te0))+Low_limit);
          }
         mesh->communicate(Di_couple);
         Di_couple.applyBoundary();
         mesh->communicate(kaii_couple);
         kaii_couple.applyBoundary();
         mesh->communicate(kaie_couple);
         kaie_couple.applyBoundary();
        }  // end for if (output_transport_couple)
    }  // end for if(output_vradial)
