...
Field2D Te_grid_couple,Ti_grid_couple,Ni_grid_couple;
...
Field2D Di_couple, chii_couple, chie_couple;
Field3D Diff_perp_couple, chi_i_perp_couple, chi_e_perp_couple;
Field3D Flux_couple,Div_Flux_couple;
...
//logical paramter
bool Evolving_Eqn_Ni,Evolving_Eqn_Ti,Evolving_Eqn_Te,Evolving_Eqn_Vi,Evolving_Eqn_phi,Evolving_Eqn_pressure;
...
// Load 2D profiles
  ...
  mesh->get(Te_grid_couple, "Te0_couple");    // eV   the coupled quantities are time dependent
  mesh->get(Ti_grid_couple, "Ti0_couple");    // eV
  mesh->get(Ni_grid_couple, "Ni0_couple");    // hl2a 10^21/m^3
  mesh->get(phi01_grid, "PHI_0");    // hl2a 10^21/m^3

  mesh->get(Diff_grid, "diff");    // hl2a 10^21/m^3
  mesh->get(Chi_grid, "chi_i");    // hl2a 10^21/m^3
  mesh->get(Che_grid, "chi_e");    // hl2a 10^21/m^3

  mesh->get(Flux_couple, "Flux_couple");
  mesh->get(Div_Flux_couple, "Div_Flux_couple");
  mesh->get(Di_couple, "Di_couple");
  mesh->get(chii_couple, "kaii_couple");
  mesh->get(chie_couple, "kaie_couple");
  Diff_perp_couple= Di_couple;
  chi_i_perp_couple=chii_couple;
  chi_e_perp_couple=chie_couple;
...
//*********************************************************liend 
  ...
  OPTION(options, Evolving_Eqn_Ni, true);  // Evolving Equations control 
  OPTION(options, Evolving_Eqn_Ti, true); 
  OPTION(options, Evolving_Eqn_Vi, true); 
  OPTION(options, Evolving_Eqn_Te, true); 
  OPTION(options, Evolving_Eqn_phi, true);  // Evolving Equations control 
  OPTION(options, Evolving_Eqn_pressure, true);  // Evolving Equations control 
  ...
  OPTION(options, couple_to_6field,  false);
...
//////////// CALCULATE PARAMETERS ///////////////////
 ...
 // bmag in Gaussian unit gauss divided by 1.e4 changes back to Gaussian SI unit Tesla
  ...
  //tbar = Lbar/sqrt(Bbar*Bbar / (MU0*density_unit*Mi2));
 ...
 if(!include_curvature)
  ...
  Te_grid /= Te_x;
  Ti_grid /= Te_x;
  Ni_grid =Ni_grid/Ni_x;  //hl2a grid in unit 10^20/m^3 
  ...
  Te_grid_couple /= Te_x;
  Ti_grid_couple /= Te_x;
  Ni_grid_couple /= Ni_x;  //hl2a grid in unit 10^19/m^3 
  Diff_perp_couple /= Lbar*Lbar/tbar;
  chi_i_perp_couple /= Lbar*Lbar/tbar;
  chi_e_perp_couple /= Lbar*Lbar/tbar;
  Div_Flux_couple /=(Vi_x/Lbar*Ni_x*density_unit*Ti_x*ee); 
  Flux_couple /=Ni_x*density_unit*Ti_x*ee*Lbar/tbar; 
  }
  Te_exp /= Te_x;
  Ti_exp /= Te_x;
  Ni_exp /= Ni_x;  
  Ne_exp /= Ni_x;  
  P0 /= Ni_x*Te_x*ee*density_unit;  
  P_dc /= Ni_x*Te_x*ee*density_unit;  
  //P0 = P0*2.*MU0/(Bbar*Bbar); 
...
//**********************************************li2016
//    eta = 0.51*1.03e-4*Zi*20.*((Te0*Te_x)^(-1.5)); // eta in Ohm-m. NOTE: ln(Lambda) = 20
//    output.write("\tSpitzer resistivity: %e -> %e [Ohm m]\n", min(eta), max(eta));
//    eta /= MU0 * Va * Lbar;
...
//**********************************************liend
 ...
 if (BScurrent)
  ...
  if(couple_to_6field)
    {
     dump.add(Diff_perp_couple,   "Diff_perp_couple",1);           // adding D_turb to dump files
     dump.add(chi_i_perp_couple, "chi_i_perp_couple",1);
     dump.add(chi_e_perp_couple, "chi_e_perp_couple",1);
    }
...
//  dump.add(q95,"q95",0);
  ...
  dump.add(Div_Flux_couple, "Div_Flux_couple",1);
  dump.add(Flux_couple, "Flux_couple",1);
...
//***************************************liend
 ...
 //Dissociation rate of molecules
  ...
  if(couple_to_6field)
    {
      Diff_ni_perp += Diff_perp_couple;           // Total diffusion coefficients = D_neo(or D_Hmode) + D_turb
      chi_i_perp += chi_i_perp_couple;
      chi_e_perp += chi_e_perp_couple;
    }
...
//   output.write("Now updata Ni \n"); 
  ...
  Pi = Ti*Ni;
  //pei = Te*Ni+Ti*Ni;
  if(Evolving_Eqn_pressure)
  {
  ddt(pei) = -Div_Flux_couple;
          // -Div(Flux_couple); 
  }
  if(!Evolving_Eqn_Ni)
  {
...
//*************************************************************li2016
//************************************
  //  potential and vorticity equations ---phi01,U00---
//************************************
   if(!Evolving_Eqn_phi)
    {
    ddt(phi01)=0.0;
...
//             + sbc_lambda0/(B0*sbc_lambda1)*Grad(Ni)*Grad(phi01)  
             - sbc_lambda0*U00;
    if (terms_cross) ddt(phi01) += sbc_lambda0/(B0*sbc_lambda1)*Grad(Ni)*Grad(phi01);  

      phi00=phi01/sbc_lambda1;

     /* if(J1_all) {J1_para =  -1.0/(eta*sbc_lambda1*B0)*Grad_par(phi01)+((KB*eV_K*Te_x*tbar)/(eta*Lbar*Lbar*ee*Ni*B0*Bbar))*Grad_par(Pe)+((0.71*KB*eV_K*Te_x*tbar)/(eta*Lbar*Lbar*B0*Bbar*ee))*Grad_par(Te);}
      mesh->communicate(J1_para);
      J1_para.applyBoundary("neumann");
   */
   ...
   } //end of Evolving Eq.phi01 and U00
