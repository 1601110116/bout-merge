...
Field2D phi0P0;  //added by tn for convenience for vorticity
...
Field2D betahat;   //used for compressibility   tn
bool flr;         // finite Larmor radius effect  tn
bool flr_U;         // finite Larmor radius effect  tn
bool flr_e;         // finite Larmor radius effect  tn
bool vpar_conv;         // convective term for parallel component in nonlinear
bool diamag3;       // third term of FLR in the vorticity equation
BoutReal delta_flr_i; // Normalized ion skin depth by minor radius tn
BoutReal delta_flr_e; // Normalized electron skin depth by minor radius tn
BoutReal delta_flr; // inverse normalized electron skin depth by minor radius tn
BoutReal Ti2Te;//tn
...
const Field3D Grad_parP_LtoC(const Field3D &f) {
  ...
  if(parallel_lr_diff) {
    ...
    if(nonlinear)
      result -= bracket(Psi, f, BRACKET_CTU)*B0;
  }else {
    if(nonlinear) {
      result = Grad_parP(Psi, f)*B0;
    }else {
      result = Grad_par(f);
    }
  }
...
const Field3D Grad_parP_CtoL(const Field3D &f) {
  ...
  if(parallel_lr_diff) {
    ...
    if(nonlinear)
      result -= bracket(Psi, f, BRACKET_CTU);
  }else {
    if(nonlinear) {
      result = Grad_parP(Psi, f);
    }else {
      result = Grad_par(f);
    }
  }
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, flr,           false); // finite Larmor radius  tn
  OPTION(options, flr_U,         false); // finite Larmor radius  tn
  OPTION(options, flr_e,         false); // finite Larmor radius  tn
  OPTION(options, vpar_conv,     false); // finite Larmor radius  tn
  OPTION(options, diamag3,         false); // finite Larmor radius  tn
  OPTION(options, Ti2Te,             1.0e1); // finite Larmor radius  tn
  ...
  OPTION(options, AA,               1.0);    // ion mass in units of proton mass default is deutrium
  ...
  dnorm = dia_fact * Mi / (2.*1.602e-19*Bbar*Tbar);
   //This is same with delta_flr

  delta_i = 2.28e7/sqrt(density/1e6)/(Lbar*100.0)*sqrt(AA);
  delta_e = 5.31e5/sqrt(density/1e6)/(Lbar*100.0)*emass_inv;
  delta_e_inv = 1.e0/delta_e/delta_e;
  gyroAlv = 1.602e-19*Bbar*Tbar/ Mi;

  delta_flr = delta_i*0.5e0;  // /sqrt(density_prof);   //normalized skin depth tn
  delta_flr_i =  delta_flr * Ti2Te / ( 1.0+Ti2Te);   //delta_i tn
  delta_flr_e =  delta_flr / ( 1.0 + Ti2Te);         //delta_e tn


  coef_jpar = -Bbar*Tbar/MU0/Lbar/Lbar/(density*1.602e-19);

  output.write("Normalisations: Bbar = %e T   Lbar = %e m\n", Bbar, Lbar);
  output.write("                Va = %e m/s   Tbar = %e s\n", Va, Tbar);
  output.write("                dnorm = %e\n", dnorm);
  output.write("                delta_i = %e\n", delta_i);
  output.write("                delta_flr = %e\n", delta_flr);
  output.write("                delta_flr_i = %e\n", delta_flr_i);
  output.write("                delta_flr_ei = %e\n", delta_flr_e);
  ...
  if(emass)
    output.write("                delta_e = %e    wci*tA = %e \n", delta_e, gyroAlv);
  ...
  if(compress0) {
    ...
    betahat =1.* B0*B0/ (0.5 + ( B0*B0/P0) );   //beta hat representing compressibility tn
    gradparB = Grad_par(B0) / B0;
    dump.add(beta, "heta", 0);
    dump.add(betahat, "hetahat", 0);


    output.write("Beta in range %e -> %e\n",
      min(beta), max(beta));
    output.write("Beta hat in range %e -> %e\n",
      min(betahat),max(betahat));
  }
  ...
  if(diamag && diamag_phi0) {
    ...
    phi0 = -delta_flr_i*P0/B0;    // change by tn
    SAVE_ONCE(phi0);
  }
    phi0P0 = phi0+delta_flr_i*P0;
  ...
  if(!restarting) {
    ...
    if(diamag) {
      phi -= delta_flr_i * P / B0 ;    // change by tn
    }
  }
     phiP = phi+delta_flr_i*P;
  ...
  phiP.setBoundary("phi"); // Set boundary conditions
  tmpU3.setBoundary("tmpU3");
...
int physics_run(BoutReal t)
{
  ...
  if(evolve_jpar) {
	...
	aset = true;
	if(compress0) {
	  Psi = invert_laplace(acoeff*Ajpar-gyroAlv*Vpar, apar_flags, &acoeff);
	} else {
  ...
  if(evolve_jpar) {
    ...
    ddt(Ajpar) = -Grad_parP(B0*phi, CELL_CENTRE) / B0 + eta*Jpar; // modified by tn

    // Ajpar 2
    if(diamag && diamag_phi0)
      //ddt(Ajpar) -= b0xGrad_dot_Grad(phi0, Ajpar);   // modified dby tn
      ddt(Ajpar) -= b0xGrad_dot_Grad(B0*phi0, Ajpar) / B0; // Equilibrium flow

    // Ajpar 3
    // Hyper-resistivity
    if(hyperresist > 0.0) {
      ddt(Ajpar) -= eta*hyperresist * Delp2(Jpar);
    }
   // FLR effect tn
    if(flr_e){
      //ddt(Ajpar) += delta_flr_e * Grad_parP(P+P0, CELL_CENTRE);//modified by tn
    // Ajpar 4
      ddt(Ajpar) += delta_flr_e * Grad_parP(P, CELL_CENTRE)/B0;  //modified by tn
    // Ajpar 5
      ddt(Ajpar) -= delta_flr_e * b0xGrad_dot_Grad(Psi,P0)/B0;
    }


    if(diamag_grad_t) {
      // grad_par(T_e) correction
      //ddt(Ajpar) += 1.71 * dnorm * 0.5 * Grad_parP(P, CELL_YLOW) / B0;
    }
    ...
    if(ehyperviscos > 0.0) {
      ddt(Ajpar) -= eta*ehyperviscos * Delp2(Jpar2);
    }

    // Ajpar 7
    if(Vt0!=0 || Vp0!=0)
      ddt(Ajpar) += 1.0*V_dot_Grad(V0eff, Ajpar);

    //xia: adding hyper diffusion of psi
    if(diffusion_psiP > 0.0) {
      Field3D tmp = Grad2_par2new(Ajpar);
      mesh->communicate(tmp);
      tmp.applyBoundary("neumann");
      ddt(Ajpar) -= diffusion_psiP * Grad2_par2new(tmp);
    }
    ...
    if(relax_j_vac) {
      ...
      ddt(Ajpar) = ddt(Ajpar)*(1. - vac_mask) - (Psi - Psitarget)*vac_mask / relax_j_tconst;
    }
  ...
  //Vorticity 4
   ...
   //Vorticity 5
     if(flr && flr_U){    //added by tn
      //finite Larmor effect + diamangnetic potential
      ddt(U) -= 0.5*delta_flr_i*b0xGrad_dot_Grad(P0, U);   // Equilibrium flow
      ddt(U)+= delta_flr_i/ B0*0.5*(b0xGrad_dot_Grad(phi0P0,Delp2(P)));
      if( diamag3 ){
        tmpU3 = b0xGrad_dot_Grad(phi0,P);
        tmpU3.applyBoundary();
        mesh->communicate(tmpU3);
        tmpU3 = -delta_flr_i/ B0*0.5*Delp2(tmpU3);
        ddt(U)+=tmpU3; }
           }
    }
   ...
   //Vorticity 6
  if(flr && flr_U){    //added by tn
       //finite Larmor effect
     ddt(U) += delta_flr_i/ B0 *0.5*(b0xGrad_dot_Grad(phiP,Delp2(P0)));
     if( diamag3 ){
       tmpU3 = b0xGrad_dot_Grad(phi,P0);
       tmpU3.applyBoundary();
       mesh->communicate(tmpU3);
       tmpU3 = -delta_flr_i/ B0 *0.5*Delp2(tmpU3);
       ddt(U) += tmpU3;}
  ...
  if(nonlinear) {
   ...
   //Vorticity 9
    if(flr && flr_U){    //added by tn
       //finite Larmor effect  added by TN Rhee of WCI center for fusion theory in NFRI
     ddt(U) -= 0.5*delta_flr_i*b0xGrad_dot_Grad(P, U);    // Advection
     ddt(U) += delta_flr_i/ B0 *0.5*(b0xGrad_dot_Grad(phiP,Delp2(P)));
     if( diamag3 ){
       tmpU3 = b0xGrad_dot_Grad(phi,P);
       tmpU3.applyBoundary();
       mesh->communicate(tmpU3);
       tmpU3 = -delta_flr_i/ B0*0.5*Delp2(tmpU3);
       ddt(U) += tmpU3;}
  ...
  if(diffusion_U4 > 0.0){
    Field3D tmp = Grad2_par2new(U);
    mesh->communicate(tmp);
    tmp.applyBoundary("neumann");
    ddt(U) -= diffusion_U4 * Grad2_par2new(tmp);}
  ...
  if(diffusion_p4 > 0.0){
    Field3D tmp = Grad2_par2new(P);
    mesh->communicate(tmp);
    tmp.applyBoundary("neumann");
    ddt(P) -= diffusion_p4 * Grad2_par2new(tmp);}
  ...
  // Pressure 7
    ...
    ddt(P) += betahat*( - Grad_parP(Vpar, CELL_CENTRE) + Vpar*gradparB );//modified by tn
  // Pressure 8
    if(flr){
       ddt(P) -= 2.*betahat*B0*delta_flr*Grad_parP(Jpar, CELL_CENTRE);
       ddt(P) += 2.*betahat*B0*delta_flr*b0xGrad_dot_Grad(Psi,J0,CELL_CENTRE);
       // FLR effect on the pressure tn
    }

//    if (emass)
//      ddt(P) -= coef_jpar*beta*( Grad_parP(Jpar*B0, CELL_CENTRE) )/B0;

  // Pressure 9,10
    if(phi_curv) {
 //     ddt(P) -= 2.*beta*b0xcv*Grad(phi);
      ddt(P) -= 2.*betahat*b0xcv*Grad(phi*B0)/B0;  //modified by tn
      if(flr_e)
      ddt(P) += 2.*betahat * delta_flr_e *b0xcv*Grad(P)/B0;  //modified by tn
    ...
    // Vpar 1
    ddt(Vpar) = -0.5*Grad_parP(P, CELL_CENTRE);
    ddt(Vpar) += 0.5*b0xGrad_dot_Grad(Psi,P0,CELL_CENTRE);

    // Vpar 2
    if(diamag && diamag_phi0)
      ddt(Vpar) -= b0xGrad_dot_Grad(phi0, Vpar); // Advection
    ...
    if(diffusion_v4 > 0.0){
      Field3D tmp = Grad2_par2new(Vpar);
      mesh->communicate(tmp);
      tmp.applyBoundary("neumann");
      ddt(Vpar) -= diffusion_v4 * Grad2_par2new(tmp);}
	...
	ddt(Ajpar) -= Vpar_Grad_par(Vpar, Ajpar);
...
int precon(BoutReal t, BoutReal gamma, BoutReal delta)
{
  ...
  mesh->communicate(ddt(P), ddt(Ajpar), ddt(U));

  // First matrix. Only modifies vorticity

  U1 = ddt(U) + gamma*(Lp(ddt(P)) + Lpsi(ddt(Ajpar)));
...
int jacobian(BoutReal t)
{
  ...
  Field3D JU = b0xcv*Grad(ddt(P))
    ...
    + (B0^2) * b0xGrad_dot_Grad(ddt(Ajpar), J0, CELL_CENTRE);
