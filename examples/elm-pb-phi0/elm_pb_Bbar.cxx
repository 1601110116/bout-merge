...
const char CXXVERSION[] = "20170907_J.G.Chen";
...
bool nogradparj; // not including gradpar Jpar?
bool filter_z;
int filter_z_mode;
int low_pass_z;
int zonal_flow;
...
int physics_init(bool restarting)
{
  ...
  if(include_rmp) {
	...
	Jpar = 0.0;
        // following one line working for master branch
	//rmp_Psi0 = invert_laplace(Jpar/B0, INVERT_4TH_ORDER | INVERT_OUT_SET, NULL);
	// Currently only implemented for 4th-order serial inversion (NXPE = 1)
        // following 5 lines working for merge-branch
  ...
  // ion inertial length: c/omega_pi = 2dnorm
...
// NOTE: Te0 = Ti0, and Te0 only used in Spitzer-Resistivity formula
  ...
  if(diamag_phi0) {
    ...
    //	phi0 = -1.0/0.7 *dnorm * P0/N0; // newphi0
    	phi0 = -1.0/1.4 *dnorm * P0/N0; // **final expression**
    //	phi0 = -1.0/2.0 *dnorm * P0/N0; // xu_2014_POP
    else if (T0_fake_prof) {
    // Temperature normalized by 0.5*Mi*Va^2
  	Tconst = Tconst * 1.602e-19/ (0.5*Mi*Va*Va);
	phi0 = - dnorm * Tconst * log(P0);
    }
    else {
  ...
  // Diamagnetic VExB
  // V0dia = (B0vec^Grad(phi0)) / B0;
  V0dia = - 0.5 * dnorm * (B0vec^Grad(P0)) / (N0*B0*B0);
  mesh->communicate(V0dia);
  V0dia.x.setBoundary("Vpar");
  V0dia.y.setBoundary("Vpar");
  V0dia.z.setBoundary("Vpar");
  V0dia.x.applyBoundary();
  V0dia.y.applyBoundary();
  V0dia.z.applyBoundary();
...
//  comms.add(phi);
  ...
  if(fakerun){
    ...
    for(count=0; ; count++){  // loop until at the end of BOUT.dmp file
      ...
      VExB = (B0vec ^ Grad(phi))/ B0;       // ExB velocity
...
int physics_run(BoutReal t)
{
  ...
  if(evolve_jpar) {
    ...
    Psi = invert_laplace(Jpar/B0, apar_flags, NULL);
  }
  ...
  if(phi_constraint) {
    ...
    // TODO: diamag?
    C_phi = Delp2(phi) - U*B0/N0; // Error in the bulk
    ...
    if (constn0)
      {
	phi = invert_laplace(U*B0/N0, phi_flags, NULL);
  ...
  if(evolve_jpar) {
    // Jpar
    // WARNING: deprecated
    ddt(Jpar) = -Grad_parP(U*B0/N0, CELL_YLOW) + B0*Delp2(eta*Jpar/B0);
    ...
    */
    ...
    if(diamag_phi0)
    // ddt(Psi) -= b0xGrad_dot_Grad(phi0, Psi)/B0;   // Equilibrium flow
      ddt(Psi) -= V_dot_Grad(V0dia, Psi);
    ...
    // Hyper-resistivity
    if(hyperresist > 0.0) {
      ddt(Psi) -= eta*hyperresist * Delp2(Jpar/B0);
    }
    ...
    if(relax_j_vac) {
      ...
      // Invert laplacian for Psi
      Psitarget = invert_laplace(Jtarget/B0, apar_flags, NULL);
  ...
  ddt(U) = peeling_coef * (B0^2) * b0xGrad_dot_Grad(Psi + rmp_Psi, J0/B0, CELL_CENTRE); // Grad j term
  ...
  if(withflow&&K_H_term)                         //K_H_term
    ddt(U) -= b0xGrad_dot_Grad(phi,U0)/B0;

  if(diamag_phi0)
    //ddt(U) -= b0xGrad_dot_Grad(phi0, U)/B0;   // Equilibrium flow
    ddt(U) -= V_dot_Grad(V0dia, U);

  if(withflow)                            // net flow
    ddt(U) -= V_dot_Grad(V0net, U);

  if(nonlinear) {
    ddt(U) -= nonlinear_coef * bracket(phi, U, bm_exb);    // Advection
  }
  ...
  if(gyroviscous)
    {
      ...
      Dperp2Phi0 = Field3D(Div(V0dia^B0vec));
      if (withflow)
      ...
      bracketPhi0P = V_dot_Grad(V0dia, Pi);
      if (withflow)
        bracketPhi0P += V_dot_Grad(V0net, Pi);
      ...
      ddt(U) += 0.5*Upara2*V_dot_Grad(V0dia, Dperp2Pi)/B0;
      if (withflow)
  ...
  ddt(P) = 0.0;
  if(evolve_pressure) {
    ddt(P) -= b0xGrad_dot_Grad(phi, P0)/B0;

    if(diamag_phi0)
      //ddt(P) -= b0xGrad_dot_Grad(phi0, P)/B0;   // Equilibrium flow
      ddt(P) -= V_dot_Grad(V0dia, P);
  ...
  if(compress0) {
    ...
    if(nonlinear)
      ddt(Vpar) -= bracket(phi, Vpar, bm_exb); // Advection
  }
...
{
  Field3D jpre = B0*Delp2(f);
  jpre.setBoundary("J");
  jpre.applyBoundary();

  mesh->communicate(jpre);

  Field3D result = b0xGrad_dot_Grad(f, J0/B0);

  return (B0^2)*(result - Grad_parP(jpre/B0) );
}

const Field3D Pschur(const Field3D &f)
{
  Field3D phitmp = invert_laplace(f*B0/N0, phitmp, phi_flags, NULL);
  // Need to communicate phi
  mesh->communicate(phitmp);

  Field3D dP = b0xGrad_dot_Grad(phitmp, P0)/B0;
  Field3D dPsi = Grad_parP(phitmp) / B0;
  dP.setBoundary("P"); dP.applyBoundary();
  dPsi.setBoundary("Psi"); dPsi.applyBoundary();

  Field3D dJ = B0*Delp2(dPsi);
  dJ.setBoundary("J"); dJ.applyBoundary();

  mesh->communicate(dP, dPsi, dJ);

  Field3D result = b0xcv*Grad(dP) + (B0^2)*(Grad_par(dJ/B0) - b0xGrad_dot_Grad(dPsi, J0/B0));
...
int precon(BoutReal t, BoutReal gamma, BoutReal delta)
{
  ...
  U2 = U1 + gamma*gamma*Pschur(U1*B0/N0); // Binomial expansion of P^-1

  // Third matrix

  Field3D phitmp = invert_laplace(U2*B0/N0, phi_flags, NULL);
  mesh->communicate(phitmp);

  P3 = P2 + gamma*b0xGrad_dot_Grad(phitmp, P0) /B0;
  Psi3 = Psi2 + gamma*Grad_par(phitmp) / B0;
  U3 = U2;
...
int jacobian(BoutReal t)
{
  ...
  phi = invert_laplace(ddt(U)*B0/N0, phi_flags, NULL);

  Jpar = B0*Delp2(ddt(Psi));

  mesh->communicate(phi, Jpar);

  Field3D JP = -b0xGrad_dot_Grad(phi, P0) / B0;
  JP.setBoundary("P"); JP.applyBoundary();

  Field3D JPsi = -Grad_par(phi, CELL_YLOW) / B0;
  JPsi.setBoundary("Psi"); JPsi.applyBoundary();

  Field3D JU = b0xcv*Grad(ddt(P))
    - (B0^2)*Grad_par(Jpar/B0, CELL_CENTRE)
    + (B0^2) * b0xGrad_dot_Grad(ddt(Psi), J0/B0, CELL_CENTRE);
...
int precon_phi(BoutReal t, BoutReal cj, BoutReal delta)
{
  ...
  phi = C_phi + phi;
  */
  phi = invert_laplace(C_phi - ddt(U)*B0/N0, phi_flags, NULL);
  U = ddt(U);
  return 0;
}
