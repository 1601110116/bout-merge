...
Field2D psixy;
// 3D evolving fields
Field3D  Te, Ne,Ni, Vi, Ti, Nn,Tn, Vn, Nm ,Vmx;             // Vi and Vn are paralell velocity
...
Field2D Diff_grid,Chi_grid_i,Chi_grid_e;
...
Field2D Diff_perp_couple, chi_i_perp_couple, chi_e_perp_couple;
...
bool terms_recombination,terms_Gradpar_pn,terms_Gradpar_eta0n,terms_cross,terms_exb,include_J0,J1_all,J1_no_density;
...
Vector3D er00, er0, ersh0,ersh1,Vexb,Vdia;
...
//******** parallel heat flux *******************//
Field3D heatf_cond_i,heatf_cond_e,heatf_conv_i,heatf_conv_e,heatf_cond_i_x,heatf_cond_e_x ,heatf_exb_i_x,heatf_exb_e_x,heatf_dia_i_x,heatf_dia_e_x;
...
// Load 2D profiles
  ...
  mesh->get(Chi_grid_i, "chi_i");    // hl2a 10^21/m^3
  mesh->get(Chi_grid_e, "chi_e");    // hl2a 10^21/m^3
...
//  Ni_exp *=10;
  ...
  mesh->get(Diff_perp_couple, "Di_couple");
  mesh->get(chi_i_perp_couple, "kaii_couple");
  mesh->get(chi_e_perp_couple, "kaie_couple");
...
//***********************************************************li2016
 ...
 //if(mesh->get(Nbar, "Nixexp")) // Typical ion density scale
  ...
  Va = sqrt(Bbar*Bbar / (MU0*density_unit*Mi2));

  Tbar = tbar;

  dnorm = dia_fact * Mi2 / (2.*1.602e-19*Bbar*Tbar);
...
//*********************************************************liend 
  ...
  OPTION(options, couple_to_6field,  true);
...
//////////// CALCULATE PARAMETERS ///////////////////
 ...
 if(!include_curvature)
  ...
  Chi_grid_i /= Lbar*Lbar/tbar;
  Chi_grid_e /= Lbar*Lbar/tbar;
  ...
  if (constn0)
    {
      ...
      N0 = 1. / Ni_x;
      N0 = 2.0;
      Te = P0 / (2.0*N0*Ni_x*density_unit * 1.602e-19); // Temperature in eV
      Ti0 = Te/Ti_x;
      Te0 = Te/Te_x;
  ...
  else
    {
      ...
      if( (!T0_fake_prof) && n0_fake_prof )
      ...
      else if (T0_fake_prof)
        {
          ...
          N0 = P0/((Ti0+Te)*Ni_x*density_unit*1.602e-19);// N0 after normalized
        }
...
//  mesh->get(Te0,  "TEEXP1");
  ...
  P0 = (Ti0+Te0)*N0*Ni_x*density_unit*ee;
...
//    output.write("\t -> Lundquist %e -> %e\n", 1.0/max(eta), 1.0/min(eta));
  ...
  //else {
    // transition from 0 for large P0 to resistivity for smP0
...
//  dump.add(q95,"q95",0);
  ...
  dump.add(Vexb.y, "vexb_y", 1);
  dump.add(Vexb.z, "vexb_z", 1);
  dump.add(Vdia.x, "vdia_x", 1);
  dump.add(Vdia.y, "vdia_y", 1);
  dump.add(Vdia.z, "vdia_z", 1);
...
//  dump.add(c_se,"c_se",0); 
  ...
  if (iterative_phi0)
      {  phish0 = sbc_lambda1*0.5*log((Mi2/Me)/(4.0*PI)*(2*temp_Te*Te_x/(temp_Ti*Ti_x+temp_Te*Te_x)))*(temp_Te*Te_x*KB*eV_K)/ee;
        if (J_para0) phish0 -= sbc_lambda1*(temp_Te*Te_x*KB*eV_K/ee)*log(1-J0*B0*Bbar/(MU0*Lbar)/(temp_Ni*Ni_x*density_unit*ee*sqrt((temp_Ti*Ti_x+temp_Te*Te_x)*eV_K/Mi2)));
  ...
  if(load_grid_trans)
    {
      ...
      chi_i_perp = Chi_grid_i;
      chi_e_perp = Chi_grid_e;
     }
  if(couple_to_6field)
    {
      Diff_ni_perp = Diff_perp_couple;
...
//   output.write("Now updata Ni \n"); 
 // ddt(Ni) = 0.; 
  ...
  Pe = Te*Ni;
  Pi = Ti*Ni;
  ddt(Ni) =  (D_perp/(Lbar/tbar*Lbar)+Diff_ni_perp)*Delp2(Ni)
 // ddt(Ni) = Diff_grid*Delp2(Ni)
            - Ni*Grad_par(Vi)
            - Vpar_Grad_par(Vi,Ni)
            + Si_p; 
  if(terms_recombination) ddt(Ni) -= S_rec;
  if(terms_exb) ddt(Ni) -= bracket((phi01/sbc_lambda1), Ni, bm_exb);
  if(include_curvature)
      {
        if(curvature_phi) ddt(Ni) -= 2.0*Ni*b0xcv*Grad((phi01/sbc_lambda1))/B0;  // curvature term
        if(diamag)  ddt(Ni) -= 2.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*b0xcv*Grad(Pi)/B0;  // curvature term
       }
   ...
   if(terms_Gradperp_diffcoefs) 
    {
      ...
      ddt(Te) +=  0.6667*V_dot_Grad(grad_perp_chie,Te)/temp_Ni; 
    }
//************************************
  // ION TEMPERATURE   ---Ti---
//************************************
...
//  ddt(Ti)=0.0;
   ...
   if(terms_exb) ddt(Ti) -= bracket((phi01/sbc_lambda1), Ti, bm_exb);
   if(include_curvature)
      {
        if(curvature_phi) ddt(Ti) -= 4.0/3.0*Ti*b0xcv*Grad(phi00)/B0;  // curvature term
        if(diamag)  ddt(Ti) -= 4.0/3.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*Ti/temp_Ni*b0xcv*Grad(Pi)/B0;  // curvature term
        if(energy_flux)  ddt(Ti) -= 10.0/3.0*(KB*Ti_x*eV_K/(Zi*ee*Bbar*Lbar*Lbar/tbar))*Ti*b0xcv*Grad(Ti)/B0;  // curvature term
       }
   ...
   if(terms_Gradperp_diffcoefs) 
    {
      ...
      ddt(Ti) +=  0.6667*V_dot_Grad(grad_perp_chii,Ti)/temp_Ni; 
    }
...
//             + sbc_lambda0/(B0*sbc_lambda1)*Grad(Ni)*Grad(phi01)  
    ...
    if (terms_cross) ddt(phi01) += sbc_lambda0/(B0*sbc_lambda1)*Grad(Ni)*Grad(phi01);  
      ...
      if(J1_no_density) {J1_para =  -1.0/(eta*sbc_lambda1*B0)*Grad_par(phi01)+((1.71*KB*eV_K*Te_x*tbar)/(eta*Lbar*Lbar*B0*Bbar*ee))*Grad_par(Te);}  //just true when test the density affect
...
//******** parallel heat flux *******************//
      heatf_cond_i = -kappa_Ti*Grad_par(Ti)*Lbar/tbar*Te_x*ee*Ni*Ni_x*density_unit;  //unit W/(m^-2)
      (heatf_cond_i).applyBoundary("neumann");
      heatf_cond_e = -kappa_Te*Grad_par(Te)*Lbar/tbar*Te_x*ee*Ni*Ni_x*density_unit;
      (heatf_cond_e).applyBoundary("neumann");
      heatf_conv_i = Ni*Ti*Vi*Lbar/tbar*Ti_x*ee*Ni_x*density_unit;
      heatf_conv_e = Ni*Te*c_se*Lbar/tbar*Te_x*ee*Ni_x*density_unit;
      heatf_cond_i_x = -chi_i_perp*sqrt(mesh->g11)*(Grad(Ti)).x*Lbar/tbar*Ti_x*ee*Ni*Ni_x*density_unit;
      (heatf_cond_i_x).applyBoundary("neumann");
      heatf_cond_e_x = -chi_e_perp*sqrt(mesh->g11)*(Grad(Te)).x*Lbar/tbar*Te_x*ee*Ni*Ni_x*density_unit;
      (heatf_cond_e_x).applyBoundary("neumann");
      heatf_exb_i_x = Ni*Ti*Vexb.x*Rxy*Bpxy*Lbar/tbar*Ti_x*ee*Ni*Ni_x*density_unit;
      heatf_exb_e_x = Ni*Te*Vexb.x*Rxy*Bpxy*Lbar/tbar*Te_x*ee*Ni*Ni_x*density_unit;
      heatf_dia_i_x = Ni*Ti*Vdia.x*Rxy*Bpxy*Lbar/tbar*Ti_x*ee*Ni*Ni_x*density_unit;
      heatf_dia_e_x = Ni*Te*Vdia.x*Rxy*Bpxy*Lbar/tbar*Te_x*ee*Ni*Ni_x*density_unit;
//******* radial particle flux ****************//
      gamma_cond_i_x = -Diff_ni_perp*sqrt(mesh->g11)*(Grad(Ni)).x*Ni_x*density_unit; //for this term Vi_x is needed for the normalization of the output
