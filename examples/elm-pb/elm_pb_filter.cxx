...
int physics_init(bool restarting)
{
  ...
  OPTION(options, filter_position,      100);  // radial index of the filter. Zonal component of P in the x range of 0 - filter_position will be filtered.

  if ( pos_filter2 )
    {
      if (filter_position<=0)
        {
          filter_position = ixsep;
          output << "\tWarning: filter_position has a negtive value!\n";
          output << "\tfilter_position is forced to be isxeps1="<<ixsep<<".\n";
        }
    }
...
int physics_run(BoutReal t)
{
  ...
  if(low_pass_z > 0) {
    ...
    if (pos_filter2)
      {
        ddt(P) = lowPass_pos(ddt(P), filter_position);
      }
  }
