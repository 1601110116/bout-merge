...
int physics_init(bool restarting)
{
   ...
   SAVE_ONCE(qpar_fs);
   qpar_eff = (qpar_Brag * qpar_fs) / (qpar_Brag + qpar_fs + 1e-20);
   SAVE_ONCE(qpar_eff);
   ...
   output.write("Flag1\n");
   tstart = std::clock();
   qpar_nonlocal_0 = - 1.6 * n0 * vthe * iSign_kpar_wcoll(1.6e-12 * te, k0, 1, 20);
   output.write("2d, 0: %f\n", ( std::clock() - tstart ) / (double) CLOCKS_PER_SEC);
   /* tstart = std::clock();
   output.write("Flag2\n");
   qpar_nonlocal_1 = - 1.6 * n0 * vthe * iSign_kpar_wcoll(1.6e-12 * te, k0, 1, 20);
   output.write("N = 12: %f\n", ( std::clock() - tstart ) / (double) CLOCKS_PER_SEC);
   tstart = std::clock();
   output.write("Flag3\n");
   qpar_nonlocal_3 = - 1.6 * n0 * vthe * iSign_kpar_wcoll(1.6e-12 * te, k0, 3, 20);
   output.write("N = 20: %f\n", ( std::clock() - tstart ) / (double) CLOCKS_PER_SEC);
   qpar_nonlocal_4 = - 1.6 * n0 * vthe * iSign_kpar_wcoll(1.6e-12 * te, k0, 4, 20);
   */
   ...
   */
   ...
   output.write("Flag5\n");
   qpar_nonlocal_3d_wocoll = - 1.6 * n0 * vthe * iSign_kpar_wcoll(1.6e-12 * te3d, k03d * 0.01);
