...
bool Hmode_rc, time_average_loop;
Field3D VeXp, VeXp_ave, P00;
Field2D Vexp_ave_tmp;
BoutReal Hmode_rc_lowlimit, ****Vexp_ave_interval;;
int sheath_position, P00_flags, P00_sep_val, calc_interval, time_average, time_average_count;
int Grid_NX;
BoutReal P0_min;
...
bool smooth_j_x, smooth_p0_x;  // Smooth Jpar in the x direction
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, calc_interval,      10);      //timestep to calculate new P0
  OPTION(options, time_average,       10);      // timestep for aberage on source term
  OPTION(options, Hmode_rc_lowlimit,  1e-5);  // limit the negative value of inverted pressure
 ...
 // Radial smoothing
  ...
  if (Hmode_rc)
    {
      ...
      if (time_average > 0)
	{
	  Vexp_ave_interval = (double****)calloc(mesh->ngx, sizeof(BoutReal));
	  if (!Vexp_ave_interval)
	    {
	      output.write("Cannot create array for time average (x)!\n");
	      exit(1);
	    }
	  for (int i=0;i<mesh->ngx;i++)
	    {
	      Vexp_ave_interval[i] = (double***)calloc(mesh->ngy, sizeof(BoutReal));
	      if (!Vexp_ave_interval[i])
		{
		  output.write("Cannot create array for time average (y)!\n");
		  exit(1);
		}
	      for (int j=0;j<mesh->ngy;j++)
		{
		  Vexp_ave_interval[i][j] = (double**)calloc(mesh->ngz, sizeof(BoutReal));
		  if (!Vexp_ave_interval[i][j])
		    {
		      output.write("Cannot create array for time average (z)!\n");
		      exit(1);
		    }
		  for (int k=0;k<mesh->ngz;k++)
		    {
		      Vexp_ave_interval[i][j][k] = (double*)calloc(time_average, sizeof(BoutReal));
		      if (!Vexp_ave_interval[i][j][k])
			{
			  output.write("Cannot create array for time average (t)!\n");
			  exit(1);
			}
		    }
		}
  ...
  /*  Field2D b0tmp;
  b0tmp = B0vec*B0vec;
  output.write("The normalized B is: %d -> %d \n",min(b0tmp,true),min(b0tmp,true));*/
...
int physics_run(BoutReal t)
{
  ...
  /*xiaty******************************************/
    if (Hmode_rc)
    {
      ...
      VeXp = b0xGrad_dot_Grad(phi, P)*mesh->averageY(mesh->J)/(kappa_perp_Hmode*aveY_g11J);
	...
	for(int j=0;j<mesh->ngy;j++)
	  {
	    for(int k=0;k<mesh->ngz-1;k++)
	      Vexp_ave_tmp[i][j] = Vexp_ave_tmp[i][j]+VeXp[i][j][k];
	    Vexp_ave_tmp[i][j] /= mesh->ngz-1;
	  }
      VeXp_ave = mesh->averageY(Vexp_ave_tmp);
      ...
      if ( (time_average>0) && (!first_run) && (t == int(t)) )
	{
	  for(int i=0;i<mesh->ngx;i++)
	    for(int j=0;j<mesh->ngy;j++)
	      for(int k=0;k<mesh->ngz;k++)
		{
		  Vexp_ave_interval[i][j][k][time_average_count] = VeXp_ave[i][j][k];
		}
	  ...
	  //output.write("The inversion of P0 is on, time is %d.\n",t);
	  ...
	  if (first_run || (time_average<0) || ((time_average_count == 0)&&(!time_average_loop)) )
	      ...
	      if (time_average_loop)
		{
		  ...
		  for(int i=0;i<mesh->ngx;i++)
			...
			for (int ttim=0;ttim<time_average;ttim++)
			  {
			    Vexp_tave_tmp[i][j][k] += Vexp_ave_interval[i][j][k][ttim];
			  }
		  VeXp_ave = Vexp_tave_tmp/time_average;
	      ...
	      else
		{
		  ...
		  VeXp_ave = Vexp_tave_tmp/(time_average_count);
	  ...
	  // for adding specific values at two boundaries
/*	  if(mesh->lastX())
	    {
	      for(int i=mesh->ngx-2;i<mesh->ngx;i++)
		for(int j=0;j<mesh->ngy;j++)
		  for(int k=0;k<mesh->ngz;k++)
		P00[i][j][k] = P0[i][j];
	    }
	  if(mesh->firstX())
		...
		for(int j=0;j<mesh->ngy;j++)
		  for(int k=0;k<mesh->ngz;k++)
		    P00[i][j][k] = -P0[i+1][j]+P0[i][j];
	    }
*/

	  int ret;
	  ret = invert_laplace(VeXp_ave, P00, P00_flags, NULL, &kappa_perp_tmp, NULL);
	  ...
	  if (outer_mid_plane > 0)
	    {
	      P0_tmp2 = mesh->averageY(P0_tmp)*(mesh->yend-mesh->ystart+1)*mesh->getNYPE();
	    }
	  else
	    P0_tmp2 = mesh->averageY(P0_tmp);
