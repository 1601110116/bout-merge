...
bool Hmode_rc, Hmode_rc_par, Hmode_rc_p0, Hmode_rc_vexp, zero_transp_bndry, Hmode_rc_phi00, iterate_phi0, Hmode_rc_U00, Hmode_rc_psi00, include_zonal_psi, include_jpar00, include_bs_jpar;
Field3D VeXp, VeXp_ave;//, phi00_tmp3;
Vector3D ve_tmp;
Field2D d2dx2p0, ddtPdc, phi00, phi0_origin, U00, psi00, Jpar00;
Field2D phi00_tmp1, phi00_tmp2, P00, phi00_inv;
Field2D phi0_cal;
int phi00_flags;
BoutReal phi0_ratio, phi00_ratio;
BoutReal aspect_ratio, bs_const, bs_constv;

bool time_average_loop;
BoutReal ***E_flux_interval, ***U_flux_interval, ***M_flux_interval, t_prev;
int time_average, time_average_count, calc_interval;
Field2D E_flux_ave, U_flux_ave, M_flux_ave;
BoutReal diffusion_p004, diffusion_u004, diffusion_ps004;
Field2D tmpP002, tmpU002, tmpPs002;
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, include_jpar00,  false);  //switch to calculate Jpar00
  OPTION(options, include_bs_jpar,  false); // include the BS current in the calculation based on the Callen's model (Eq (38) in Callen PoP2009)
  OPTION(options, aspect_ratio,      0.55); // value of the aspect ratio

  OPTION(options, calc_interval,      -1);      //timestep to calculate new P00
  OPTION(options, time_average,       -1);      // timestep for aberage on source (E_flux) term
  ...
  OPTION(options, output_flux,          false);  // output zonal fluxes for vorticity, energy and magnetic
 ...
 // Radial smoothing
  ...
  OPTION(options, diffusion_p004,        -1.0);   //xty: parallel hyper-viscous diffusion for p00
  OPTION(options, diffusion_u004,        -1.0);   //xty: parallel hyper-viscous diffusion for U00
  OPTION(options, diffusion_ps004,        -1.0);   //xty: parallel hyper-viscous diffusion for psi00
  ...
  if (Hmode_rc2)
    {
      ...
      VeXp.setBoundary("p");
      E_flux.setLocation(CELL_CENTRE);
      E_flux.setBoundary("p");
      //dump.add(VeXp, "VeXp", 1);
      dump.add(E_flux, "E_flux", 1);
      E_flux_ave.setLocation(CELL_CENTRE);
      E_flux_ave.setBoundary("p");
      dump.add(E_flux_ave, "E_flux_ave", 1);
      ...
      if (calc_interval > 0)
	{
	  if (time_average <= 0)
	    {
	      time_average = calc_interval;
	      output.write("calc_interval is %d.\n", calc_interval);
	      output.write("time_average is set to be equal to calc_interval, is %d.\n", time_average);
	      output_flux = true;
	      output.write("output_flux is set to be equal to be true.\n");
	    }
	}

      if (time_average > 0)
        {
          E_flux_interval = (double***)calloc(mesh->ngx, sizeof(BoutReal));
          U_flux_interval = (double***)calloc(mesh->ngx, sizeof(BoutReal));
          M_flux_interval = (double***)calloc(mesh->ngx, sizeof(BoutReal));
          if (!E_flux_interval)
            {
              output.write("Cannot create array for time average (x) of P!\n");
              exit(1);
            }
          if (!U_flux_interval)
            {
              output.write("Cannot create array for time average (x) of U!\n");
              exit(1);
            }
          if (!M_flux_interval)
            {
              output.write("Cannot create array for time average (x) of M!\n");
              exit(1);
            }

          for (int i=0;i<mesh->ngx;i++)
            {
              E_flux_interval[i] = (double**)calloc(mesh->ngy, sizeof(BoutReal));
              U_flux_interval[i] = (double**)calloc(mesh->ngy, sizeof(BoutReal));
              M_flux_interval[i] = (double**)calloc(mesh->ngy, sizeof(BoutReal));
              if (!E_flux_interval[i])
                {
                  output.write("Cannot create array for time average (y) of P!\n");
                  exit(1);
                }
              if (!U_flux_interval[i])
                {
                  output.write("Cannot create array for time average (y) of U!\n");
                  exit(1);
                }
              if (!M_flux_interval[i])
                {
                  output.write("Cannot create array for time average (y) of M!\n");
                  exit(1);
                }

              for (int j=0;j<mesh->ngy;j++)
                {
                  E_flux_interval[i][j] = (double*)calloc(time_average, sizeof(BoutReal));
                  U_flux_interval[i][j] = (double*)calloc(time_average, sizeof(BoutReal));
                  M_flux_interval[i][j] = (double*)calloc(time_average, sizeof(BoutReal));
                  if (!E_flux_interval[i][j])
                    {
                      output.write("Cannot create array for time average (z) of P!\n");
                      exit(1);
                    }
                  if (!U_flux_interval[i][j])
                    {
                      output.write("Cannot create array for time average (z) of U!\n");
                      exit(1);
                    }
                  if (!M_flux_interval[i][j])
                    {
                      output.write("Cannot create array for time average (z) of M!\n");
                      exit(1);
                    }
                }

          time_average_count = 0;
          time_average_loop = false;
        }
      }
  ...
  if (output_flux)
    {
      U_flux.setLocation(CELL_CENTRE);
      U_flux.setBoundary("U");
      dump.add(U_flux, "U_flux", 1);
      M_flux.setLocation(CELL_CENTRE);
      M_flux.setBoundary("Psi");
      dump.add(M_flux, "M_flux", 1);
      if (calc_interval)
	{
          U_flux_ave.setLocation(CELL_CENTRE);
          U_flux_ave.setBoundary("U");
          M_flux_ave.setLocation(CELL_CENTRE);
          M_flux_ave.setBoundary("Psi");
	}
    }
  ...
  if (Hmode_rc2)
    {
      if(diffusion_p004 > 0.0) {
	output.write("    diffusion_p004: %e\n", diffusion_p004);
	dump.add(diffusion_p004, "diffusion_p004", 1);
      }
      if(diffusion_u004 > 0.0) {
	output.write("    diffusion_u004: %e\n", diffusion_u004);
	dump.add(diffusion_u004, "diffusion_u004", 1);
      }
      if(diffusion_ps004 > 0.0) {
	output.write("    diffusion_ps004: %e\n", diffusion_ps004);
	dump.add(diffusion_ps004, "diffusion_ps004", 1);
      }
    }
...
//	    U00 = zonal_part(U, low_pass_z);
	    if (Hmode_rc2 && Hmode_rc_phi00 && Hmode_rc_psi00)
	      ...
	      if (include_bs_jpar)
		bs_const = 4.39 * sqrt(aspect_ratio)*Bbar*Lbar/2.;
		bs_constv = 4.39 * sqrt(aspect_ratio)*MU0*1.602e-19*density*Lbar*Va;
	  }
      }
  }
  else
...
//  comms.add(phi);
  if (Hmode_rc2)
      ...
      if (Hmode_rc_phi00)
	{
      ...
      if ( (diffusion_p004 > 0) && (calc_interval > 1) )
	tmpP002.setBoundary("P");
      if ( (diffusion_p004 > 0) && (calc_interval > 1) )
      	tmpU002.setBoundary("U");
      if ( (diffusion_ps004 > 0) && (calc_interval > 1) )
        tmpPs002.setBoundary("Psi");
    }
  if (iterate_phi0)
    comms.add(phi0_cal);
  ...
  if(transport_only)
    {
      ...
      E_flux.allocate();
      file->read_rec(*(E_flux.getData()), "E_flux", mesh->ngx, mesh->ngy);

      mesh->communicate(E_flux);
      E_flux.applyBoundary();
...
//output.write("The inversion of P00 is on, time is %f. Previous time is %f.\n",t,t_prev);

  bool time_crt1 = false;  //flag for flux average
  bool time_crt2 = false;  //flag for transport calculation
  if (calc_interval > 0)
    {
      if (calc_interval ==1) {
	time_crt1 = true;
  ...
  if(!evolve_jpar) {
    if (Hmode_rc2 && Hmode_rc_psi00 && Hmode_rc_phi00 && include_jpar00)
    {
/*      if (smooth_j_x)
	{
	  psi00 = smooth_x(psi00);
          psi00 = smooth_x(psi00);
          psi00 = smooth_y(psi00);
	  psi00 = smooth_y(psi00);
	}*/
      Jpar00 = Delp2(psi00);
      if (include_bs_jpar)
	{
          Jpar00 += bs_const*Rxy*Btxy/B0*DDX(P00);
	  Jpar00 -= bs_constv*N0*(B0vec^Grad(B0*(phi00)))*y_vec/B0/Bpxy;
	}
...
//	  Jpar00 = smooth_y(Jpar00);
	  //Jpar00 = zonal_part(nl_filter(Jpar00, 1), low_pass_z);
          //Jpar00 = smooth_x(Jpar00);
          //Jpar00 = smooth_x(Jpar00);
      }
...
//************************************************************xiaty*/
  ...
  if (Hmode_rc2 && Hmode_rc_phi00)
    {
      if (!constn0)
        {
          Field2D cross_tmp;
          //cross_tmp = zonal_part(Grad_perp(N0)*Grad_perp(phi00), low_pass_z)/N0;
	  cross_tmp = phi00_ratio*zonal_part(Grad_perp(N0)*Grad_perp(phi00_inv), low_pass_z)/N0;
          mesh->communicate(cross_tmp);

          phi00_tmp1 = (phi00_ratio*Delp2(phi00_inv) + 0.5*dnorm/(N0*B0) * Delp2(P00) + cross_tmp);
          ddt(phi00_inv) = phi00_tmp1;
          if (Hmode_rc_U00)
//            ddt(phi00_inv) -= U00/N0+zonal_part(U,low_pass_z)/N0;
	    ddt(phi00_inv) -= zonal_part(U+U00,low_pass_z)/N0;
          else
  ...
  if(evolve_jpar) {
    ...
    if (Hmode_rc2)
	  ...
	  if (Hmode_rc_psi00)
            ddt(Psi) += zonal_part(Grad_par(B0*(phi+phi00), CELL_CENTRE)/B0, low_pass_z); }
        else
        ...
        if (nonlinear && Hmode_rc_psi00 && Hmode_rc_phi00)
          ddt(Psi) -= b0xGrad_dot_Grad(B0*(phi+phi00), psi00)/B0;
        if (nonlinear && include_zonal_psi && Hmode_rc_psi00 && Hmode_rc_phi00)
	  {
	    if (!output_flux)
	      ...
	      {
		M_flux = zonal_part(b0xGrad_dot_Grad(B0*(phi00+phi), Psi)/B0, low_pass_z);
                if (nonlinear)
                  M_flux += zonal_part(b0xGrad_dot_Grad(B0*(phi00+phi), psi00)/B0, low_pass_z);
                if(diamag_phi0)
                  M_flux += zonal_part(b0xGrad_dot_Grad(phi0, psi00+Psi), low_pass_z);   // Equilibrium flow
		mesh->communicate(M_flux);
		M_flux.applyBoundary();
		M_flux_ave = M_flux;
                if (diamag_phi0)
                  ddt(Psi) += M_flux - b0xGrad_dot_Grad(phi0, psi00);
                else
		  ddt(Psi) += M_flux;

        if ( (time_average>0) && (!first_run) && time_crt1 )
          {
            for(int i=0;i<mesh->ngx;i++)
              for(int j=0;j<mesh->ngy;j++)
                  {
                    M_flux_interval[i][j][time_average_count] = M_flux[i][j];
                  }
          }

      if ( time_crt1 || first_run )
        {
//          output.write("The inversion of P00 is on, time is %f.\n",t);

          if (first_run || (time_average<0) || ((time_average_count == 0)&&(!time_average_loop)) )
            M_flux_ave = M_flux;
          else
            {
              Field2D Vexp_tave_tmp;
//              Vexp_tave_tmp.setLocation(CELL_CENTRE);
              Vexp_tave_tmp = 0.;

              if (time_average_loop)
                {
//                  output.write("Test 1.\n");
                  for(int i=0;i<mesh->ngx;i++)
                    for(int j=0;j<mesh->ngy;j++)
                      for (int ttim=0;ttim<time_average;ttim++)
                        {
                          Vexp_tave_tmp[i][j] += M_flux_interval[i][j][ttim];
                        }
                 M_flux_ave = Vexp_tave_tmp/time_average;
...
//                   output.write("Test 2.\n");
                  for(int i=0;i<mesh->ngx;i++)
                    for(int j=0;j<mesh->ngy;j++)
                      for (int ttim=0;ttim<time_average_count;ttim++)
                        {
                          Vexp_tave_tmp[i][j] += M_flux_interval[i][j][ttim];
                        }
                  M_flux_ave = Vexp_tave_tmp/(time_average_count+1);
//output.write("Test 2.1\n");
                }
//	      M_flux_ave = smooth_x(M_flux_ave);
            }
          }
	...
	if(eHall) {
	  ddt(Psi) +=  0.25*delta_i*(Grad_parP(B0*(P+P00), CELL_CENTRE) / B0
                                 +b0xGrad_dot_Grad(P0, Psi));   // electron parallel pressure
	  if (Hmode_rc_psi00 && Hmode_rc_phi00)
	    {
	      ddt(Psi) -=  0.25*delta_i*zonal_part(Grad_parP(B0*(P+P00), CELL_CENTRE) / B0
                                 +b0xGrad_dot_Grad(P0, Psi), low_pass_z);
	      if (nonlinear)
		{
		  ddt(Psi) +=  0.25*delta_i*b0xGrad_dot_Grad(P+P00, psi00);
		  ddt(Psi) -=  0.25*delta_i*zonal_part(b0xGrad_dot_Grad(P+P00, psi00), low_pass_z);
		}
	    }
	}
...
/*      if (Hmode_rc_psi00)
       ...
       if(diamag_phi0)
        {
          if (Hmode_rc_U00)
	  {
            ddt(U) -= b0xGrad_dot_Grad(phi0, U+U00);   // Equilibrium flow
	    ddt(U) += zonal_part(b0xGrad_dot_Grad(phi0, U+U00), low_pass_z);
	  }
          else
      ...
      else
	{
	  if (Hmode_rc_U00)
	    {
	      ddt(U) -= b0xGrad_dot_Grad(phi0, U+U00);
	      if (nonlinear)
		ddt(U) -= bracket(phi+phi00, U+U00, bm_exb)*B0;
	      U_flux = zonal_part(b0xGrad_dot_Grad(phi0, U00+U), low_pass_z);
	      U_flux += zonal_part((bracket(phi+phi00, U+U00, bm_exb)*B0), low_pass_z);
	      mesh->communicate(U_flux);
	      U_flux.applyBoundary();
	      U_flux_ave = U_flux;
	      ddt(U) += U_flux;
        ...
        if ( (time_average>0) && (!first_run) && time_crt1 )
          {
            for(int i=0;i<mesh->ngx;i++)
              for(int j=0;j<mesh->ngy;j++)
                  {
                    U_flux_interval[i][j][time_average_count] = U_flux[i][j];
                  }
	  }
...
//                  output.write("Test 1.\n");
                  for(int i=0;i<mesh->ngx;i++)
                    for(int j=0;j<mesh->ngy;j++)
                      for (int ttim=0;ttim<time_average;ttim++)
                        {
                          Vexp_tave_tmp[i][j] += U_flux_interval[i][j][ttim];
                        }
                 U_flux_ave = Vexp_tave_tmp/time_average;
...
//                   output.write("Test 2.\n");
                  ...
                  U_flux_ave = Vexp_tave_tmp/(time_average_count+1);
//output.write("Test 2.1\n");
                }
//	      U_flux_ave = smooth_x(U_flux_ave);
            }
          }
	    }
	  else
        ...
        if (Hmode_rc_psi00)
	{
	  ...
	  ddt(U) += (B0^2)*zonal_part(Grad_parP(Jpar+Jpar00, CELL_CENTRE), low_pass_z);
	}
        else
	{
          ddt(U) -= (B0^2)*Grad_parP(Jpar, CELL_CENTRE); // b dot grad j
	  if (Hmode_rc_U00)
	    ddt(U) += (B0^2)*zonal_part(Grad_parP(Jpar, CELL_CENTRE), low_pass_z);
	}
        if (Hmode_rc_psi00 && nonlinear)
          {
            ddt(U) += (B0^2)*b0xGrad_dot_Grad(psi00, Jpar+Jpar00);
            if (include_zonal_psi)
              ddt(U) -= zonal_part(((B0^2)*b0xGrad_dot_Grad(psi00, Jpar+Jpar00)), low_pass_z);
          }
      }
...
//output.write("Now I see you t0_1.\n");
	  ...
	  if (!Hmode_rc_phi00)
	    {
	      E_flux = zonal_part(VeXp, low_pass_z);
	    }
	  else
	    {
...
//output.write("The time is %f\n", t);

        if ( (time_average>0) && (!first_run) && time_crt1 )
          {
//output.write("Oh! The delta time is %f\n", t);
//output.write("The time average count is %d\n", time_average_count);
            for(int i=0;i<mesh->ngx;i++)
              for(int j=0;j<mesh->ngy;j++)
                  {
                    E_flux_interval[i][j][time_average_count] = E_flux[i][j];
//output.write("E_flux_interal is %e. The time average count is %d.\n", E_flux_interval[i][j][time_average_count], time_average_count);
                  }

/*            if ( time_average_count<time_average )
              ++time_average_count;
            else if (time_average_count==time_average)
              {
                time_average_count = 0.;
                time_average_loop = true;
              }
*/
//          output.write("time_average_count is %d.\n", time_average_count);
          //output.write("Value is %d.\n", Vexp_ave_interval[mesh->ngx-1][mesh->ngy-1][mesh->ngz-1][time_average_count]);
//output.write("The inversion of P00 is on, time is %f. Previous time is %f.\n",t,t_prev);
//          output.write("Time_average_loop is %d. Time_average_count is %d.\n", time_average_loop, time_average_count);
          }
...
//                  output.write("Test 1.\n");
                  for(int i=0;i<mesh->ngx;i++)
                    for(int j=0;j<mesh->ngy;j++)
                      for (int ttim=0;ttim<time_average;ttim++)
                        {
                          Vexp_tave_tmp[i][j] += E_flux_interval[i][j][ttim];
                        }
                 E_flux_ave = Vexp_tave_tmp/time_average;
...
//                   output.write("Test 2.\n");
                  ...
                  E_flux_ave = Vexp_tave_tmp/(time_average_count+1);
//output.write("Test 2.1\n");
		}
//	      E_flux_ave = smooth_x(E_flux_ave);
//output.write("Max of E_flux is %e. Max of E_flux_ave is %e. Max of Vexp_tave is %e.\n",max(E_flux, true), max(E_flux_ave, true), max(Vexp_tave_tmp, true));
	    }
	  }
	}
      else
	{
      ...
      if ( (time_average>0) && (!first_run) && time_crt1 )
	{
            if ( time_average_count<(time_average-1) )
              ++time_average_count;
            else //if (time_average_count==time_average)
              {
                time_average_count = 0.;
      ...
      if (Hmode_rc_vexp)
	{
	  ddt(P) -= VeXp - E_flux;
	  if ( time_crt2 )
	    ddt(P00) -= E_flux_ave/converge_const;
	  else if ( calc_interval < 0 )
	    ddt(P00) -= E_flux/converge_const;
	}

//output.write("Test 2.2\n");

      if ( Hmode_rc_par && time_crt2 )
	{
...
//     output.write("Now I see you t0_2.\n");
      ...
      if (Hmode_rc_p0 && time_crt2 )
	{
	  ...
	  ddt(P00) += d2dx2p0/converge_const;
        ...
        if( (diffusion_p004>0.0) && (calc_interval>1) ) {
          tmpP002 = D2DX2(P00);
          mesh->communicate(tmpP002);
          tmpP002.applyBoundary();
          ddt(P00) -= diffusion_p004 * D2DX2(tmpP002);}

          // sink terms
          if(sink_P > 0.0){
            ddt(P00) -= sink_P*P00/converge_const;
//            ddt(P00) -= sink_P*zonal_part(sink_tanhxr(P0,P00,sp_width,sp_length), low_pass_z)*Tbar/converge_const; // sink
      ...
      ddt(U00) = 0.0;
      if (Hmode_rc_phi00 && time_crt2 )
	{

	  if (Hmode_rc_U00)
	    {
	      if (Hmode_rc_psi00)
                ddt(U00) = (B0^2) * zonal_part(b0xGrad_dot_Grad(psi00+Psi, J0, CELL_CENTRE), low_pass_z); // Grad j term
	      else
		...
		// Parallel current term
                if (Hmode_rc_psi00)
		  {
		    ddt(U00) -= (B0^2) * zonal_part(Grad_parP(Jpar00+Jpar, CELL_CENTRE), low_pass_z); // b dot grad j
		    if (nonlinear && include_zonal_psi)
		      ddt(U00) += (B0^2) * zonal_part(b0xGrad_dot_Grad(psi00, Jpar00+Jpar), low_pass_z);
		  }
                else
...
//		    ddt(U00) -= b0xGrad_dot_Grad(phi0, U00);   // Equilibrium flow
	            ddt(U00) -= zonal_part(b0xGrad_dot_Grad(phi0, U00+U), low_pass_z);
	      ...
	      else
		{
		  ddt(U00) -= U_flux_ave;
	        ...
	        ddt(U00) += viscos_perp * Delp2(U00);     // Perpendicular viscosity
              ...
              if( (diffusion_u004>0.0) && (calc_interval>1) ) {
                tmpU002 = D2DX2(U00);
                mesh->communicate(tmpU002);
                tmpU002.applyBoundary();
                ddt(U00) -= diffusion_u004 * D2DX2(tmpU002);}
...
//	      ddt(U00) = zonal_part(ddt(U), low_pass_z);
...
*/	}
     //output.write("Now I see you t0_3.\n");
      ...
      ddt(psi00) = 0.;
      if (Hmode_rc_psi00 && time_crt2 )
      {
	  ddt(psi00) = -zonal_part((Grad_par(B0*(phi00+phi), CELL_CENTRE))/B0, low_pass_z) + zonal_part((eta*(Jpar00)), low_pass_z);
	  if (!output_flux)
	    {
              if (nonlinear)
		{
                  ddt(psi00) -= zonal_part(b0xGrad_dot_Grad(B0*(phi00+phi), psi00)/B0, low_pass_z);
		  ddt(psi00) -= zonal_part(b0xGrad_dot_Grad(B0*(phi00+phi), Psi)/B0, low_pass_z);
		}

              if(diamag_phi0)
                ddt(psi00) -=zonal_part(b0xGrad_dot_Grad(phi0, psi00+Psi), low_pass_z);   // Equilibrium flow
	    }
	  else
	    {
	      ddt(psi00) -= M_flux_ave;
	    }

          if(eHall) {
            ddt(psi00) += 0.25*delta_i*zonal_part(Grad_parP(B0*(P+P00), CELL_CENTRE) / B0
                                 +b0xGrad_dot_Grad(P0, Psi+psi00), low_pass_z);
	    ddt(psi00) += 0.25*delta_i*zonal_part(b0xGrad_dot_Grad(P+P00, psi00), low_pass_z);
	  }
        ...
        if( (diffusion_ps004>0.0) && (calc_interval>1) ) {
          tmpPs002 = D2DX2(psi00);
          mesh->communicate(tmpPs002);
          tmpPs002.applyBoundary();
          ddt(psi00) -= diffusion_ps004 * D2DX2(tmpPs002);}

//	  ddt(psi00) = ddt(Psi).DC();
          ddt(psi00) /= converge_const_psi00;
//	  ddt(psi00) = zonal_part(nl_filter_x(ddt(psi00), 1), low_pass_z);
       }
    }
  else
    {
