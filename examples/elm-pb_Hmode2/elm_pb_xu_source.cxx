...
Field2D phi00_tmp1, phi00_tmp2, P00, Vexp_ave_tmp;
...
// create a radial buffer zone to set jpar zero near radial boundary
  ...
  for(int jx=0;jx<mesh->ngx;jx++)
	...
	{
	  //if ( (mesh->YGLOBAL(jy) <= 0) || (mesh->YGLOBAL(jy) >= Grid_NY-1) )
	    //result[jx][jy][jz] = 0.;
	  //else
	  //{
...
int physics_run(BoutReal t)
{
  ...
  if (iterate_phi0 && diamag_phi0)
      ...
      if (!constn0)
	{
	  ...
	  ddt(phi0) = (Delp2(phi0) + 0.5*dnorm/(N0*B0) * Delp2(P0) + cross_tmp.DC())*vac_mask2;
	}
      else
	{
	  ddt(phi0) = ( Delp2(phi0) + 0.5*dnorm * Delp2(P0/B0) )*vac_mask2;
	}
      //phi00_tmp2 = ddt(phi0);
      //mesh->communicate(phi00_tmp2);
      ddt(phi0) /= converge_const_phi00;
    }
	  ...
	  if (!constn0)
	    {
	      ...
	      ddt(phi00) = 0.0*(Delp2(phi00) + 0.5*dnorm/(N0*B0) * Delp2(P0_tmp) + cross_tmp.DC())*vac_mask2;
	      if (Hmode_rc_U00)
		ddt(phi00) -= U00/N0;
	      //ddt(phi00) -= U_dc/N0;
	    }
	  else
	    {
	      ...
	      phi00_tmp1 = ( Delp2(phi00) + 0.5*dnorm/(B0) * Delp2(P0_tmp) )*vac_mask2;
	      //phi00_tmp2 = U_dc;
	      //phi00_tmp1 = ( (phi00*B0+0.5*dnorm*P0_tmp) )/B0*vac_mask2;
	      ddt(phi00) = phi00_tmp1;// - U_dc;
	      if (Hmode_rc_U00)
		ddt(phi00) -= U00;
	    }
	  ddt(phi00) /= converge_const_phi00;
	}
