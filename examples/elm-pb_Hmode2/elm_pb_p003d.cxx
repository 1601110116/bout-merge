...
Field3D VeXp, VeXp_ave, P00, Vexp_ave_tmp;
Vector3D ve_tmp;
Field3D ddtPdc, phi00, phi0_origin, U00, psi00, Jpar00;
Field3D phi00_tmp1, phi00_tmp2, phi00_inv;
Field2D phi0_cal, d2dx2p0;
int phi00_flags;
...
const Field3D zonal_part(const Field3D &var, int zmax);
...
#ifdef CHECK
  msg_stack.push("zonal_part(Field3D, %d)", zmax);
#endif
  ...
  if(transport_only)
    {
      ...
      P00.allocate();
      file->read_rec(**(P00.getData()), "P00", mesh->ngx, mesh->ngy, mesh->ngz);
...
*/
      ...
      if (Hmode_rc_vexp)
	{
      ...
      if (Hmode_rc_par)
	{
	  ddt(P00) += 2. / 3. * kappa_par * Grad2_par2(P00)/converge_const;
	  ddt(P00) += 2. / 3. * Grad_par(kappa_par) * Grad_par(P00)/converge_const;
	}
      ...
      if (Hmode_rc_p0)
	{
	  ...
	  // sink terms
	  if(sink_P > 0.0){
	    ddt(P00) -= sink_P*sink_tanhxr(P0,P00,sp_width,sp_length)*Tbar/converge_const; // sink
	  }
	  if(heating_P > 0.0){
	    ddt(P00) += heating_P*sink_P*sink_tanhxl(P0,P00,sp_width,sp_length)*Tbar/converge_const; // source
	  }
	}
