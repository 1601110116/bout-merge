...
bool Hmode_rc, Hmode_rc_par, Hmode_rc_p0, Hmode_rc_vexp, zero_transp_bndry, Hmode_rc_phi00, iterate_phi0, Hmode_rc_U00;
Field3D VeXp, VeXp_ave;
Vector3D ve_tmp;
Field2D d2dx2p0, ddtPdc, phi00, phi0_origin, U00;
...
BoutReal Hmode_rc_lowlimit, converge_const, converge_const_phi00;
 ...
 // Radial smoothing
  ...
  if (Hmode_rc2)
      ...
      if (Hmode_rc_phi00)
	{
	  ...
	  //phi00_tmp2.setLocation(CELL_CENTRE);
	  //phi00_tmp2.setBoundary("phi");
	  dump.add(phi00_tmp1, "phi00_tmp1", 1);
	  //dump.add(phi00_tmp2, "phi00_tmp2", 1);
  ...
  vac_mask2 = (mask_x_1d(false, 2, 0.03, 0.1)).DC();
  ...
  if(diamag_phi0) {
	...
	if (constn0)
	  ...
	  phi0_origin = -0.5*dnorm*P0/B0/N0;
      ...
      if (Hmode_rc_phi00)
	{
	  ...
	  if (!restarting)
	      ...
	      else
		{
	      ...
	      phi00 = mesh->averageY(phi.DC());
              //phi00_inv = mesh->averageY(U.DC());
              phi00_inv = phi00/phi00_ratio;
	      //phi00 = 0.;//phi0*B0;
	    }
          else
            {
              /*Field3D phi00_tmptmp;
              phi00_tmptmp = phi00_inv;
              phi00 = (invert_laplace(phi00_tmptmp, phi00_flags, NULL)).DC();
              mesh->communicate(phi00);*/
...
int physics_run(BoutReal t)
{
  ...
  /*xiaty******************************************/
      ...
      if ( t!=0. && first_run )
	{
	  ...
	  for(int i=0;i<mesh->ngx;i++)
		...
		if ( (outer_mid_plane > 0) && (mesh->YGLOBAL(j) == outer_mid_plane) )
		  {
		    if ( (Hmode_rc_lowlimit > 0) &&  (P00[i][j] < Hmode_rc_lowlimit) )
		      {
			P00[i][j] = Hmode_rc_lowlimit*P0_min/(Hmode_rc_lowlimit+P0_min);
		      }
		    P0_tmp[i][j] = P00[i][j];
		  }
    ...
    {
      //output.write("Now I see you -2.\n");
      if ( t!=0. && first_run && !transport_only)
	{
	  P00 = 0.;
	}

      // Diamagnetic phi0
      if (diamag_phi0 && !Hmode_rc_phi00)
	{
	  if (constn0)
	    phi += -0.5*dnorm*P00/B0;
	  else
	    // Stationary equilibrium plasma. ExB velocity balances diamagnetic drift
	    phi += -0.5*dnorm*P00/B0/N0;
	}
      else if (diamag_phi0 && Hmode_rc_phi00)
	{
	  //output.write("Now I see you 0_1.\n");
	  phi += phi00;
	}
  ...
  if (iterate_phi0 && diamag_phi0)
	...
	{
	  //Field3D cross_tmp;
	  //cross_tmp = Grad_perp(N0)*Grad_perp(phi0)/N0;
	  Field2D cross_tmp;
	  cross_tmp = phi0_ratio*mesh->g11*DDX(N0)*DDX(phi0_cal)/N0;
	  mesh->communicate(cross_tmp);

	  ddt(phi0_cal) = (phi0_ratio*Delp2(phi0_cal) + 0.5*dnorm/(N0*B0) * Delp2(P0) + cross_tmp)*vac_mask2;
	  //ddt(phi0) = (mesh->g11*D2DX2(phi0) + 0.5*dnorm/(N0*B0) * mesh->g11*D2DX2(P0) + cross_tmp)*vac_mask2;
	}
      else
	{
	  ddt(phi0_cal) = ( phi0_ratio*Delp2(phi0_cal) + 0.5*dnorm * Delp2(P0/B0) )*vac_mask2;
	  //ddt(phi0) = ( mesh->g11*D2DX2(phi0) + 0.5*dnorm * mesh->g11*D2DX2(P0/B0) )*vac_mask2;
	}
      //phi00_tmp2 = ddt(phi0);
      //mesh->communicate(phi00_tmp2);
  ...
  if (Hmode_rc2)
      ...
      if (!transport_only)
	{
	  ...
	  if (!Hmode_rc_phi00)
	    ...
	    {
	      //output.write("Now I see you 0_2.\n");
	      Vexp_ave_tmp = b0xGrad_dot_Grad(phi00, P0);
	      Vexp_ave_tmp += b0xGrad_dot_Grad(phi0, P00);
	      Vexp_ave_tmp += b0xGrad_dot_Grad(phi00, P00);
	      mesh->communicate(Vexp_ave_tmp);
	    }
	}
      else
      ...
      if (Hmode_rc_vexp)
	{
      ...
      if (Hmode_rc_par)
	{
	  ddt(P00) += 2. / 3. * kappa_par.DC() * Grad2_par2(P0)/converge_const;
//	  ddt(P00) += 2. / 3. * Grad_par(kappa_par) * Grad_par(P0)/converge_const;
	}
      ...
      if (Hmode_rc_p0)
	{
	  ...
	  // source terms
	  if(heating_P > 0.0){
	    ddt(P00) -= heating_P*sink_tanhxl(P0,P00,hp_width,hp_length).DC()*Tbar; // source
	  }
	}
      //output.write("Now I see you 0_3.\n");
      if (Hmode_rc_phi00)
	{
	  U_dc = U.DC();
	  ...
	  if (Hmode_rc_U00)
	    {
	      ddt(U00) = (B0^2) * b0xGrad_dot_Grad(Psi.DC(), J0, CELL_CENTRE).DC(); // Grad j term

	      ddt(U00) += b0xcv*Grad(P00);

	      if(!nogradparj) {
		// Parallel current term
		ddt(U00) -= (B0^2)*Grad_parP(Jpar.DC(), CELL_CENTRE).DC(); // b dot grad j
	      }
	  ...
	  if (!constn0)
	    {
	      Field2D cross_tmp;
	      //cross_tmp = Grad_perp(N0)*Grad_perp(phi00)/N0;
	      //cross_tmp = mesh->g11*DDX(N0)*DDX(phi00)/N0;
	      cross_tmp = mesh->averageY(mesh->g11)*VDDX(DDX(N0), phi00)/N0 ;
	      mesh->communicate(cross_tmp);

              phi00_tmp1 = (phi00_ratio*Delp2(phi00_inv) + 0.5*mesh->averageY(dnorm/(N0*B0) * Delp2(P0_tmp)) + cross_tmp)*vac_mask2;
	      //phi00_tmp1 = (phi00_inv + 0.5*mesh->averageY(dnorm/(N0*B0) * Delp2(P0_tmp)) + cross_tmp)*vac_mask2;
	      //phi00_tmp1 = (Delp2(phi00) + cross_tmp)*vac_mask2;
	      //phi00_tmp1 = ( mesh->averageY(mesh->g11)*D2DX2(phi00) + 0.5*mesh->averageY(dnorm/(N0*B0) * mesh->g11*D2DX2(P00)) -cross_tmp )*vac_mask2;
	      //phi00_tmp1 = ( mesh->averageY(mesh->g11)*D2DX2(phi00) - cross_tmp )*vac_mask2;
	      ddt(phi00_inv) = -phi00_tmp1;
	      ddt(phi00_inv) += mesh->averageY(U.DC()/N0);
	      //ddt(phi00_inv) += U.DC()/N0;
	      if (Hmode_rc_U00)
	    ...
	    {
	      //ddt(phi00) = -Delp2(phi00) - 0.5*dnorm/(B0) * Delp2(P0_tmp);
	      //ddt(phi00) += U_dc;

              phi00_tmp1 = ( phi00_ratio*Delp2(phi00_inv) + 0.5*dnorm/(B0) * Delp2(P0_tmp) )*vac_mask2;
	      //phi00_tmp1 = ( phi00_inv + 0.5*dnorm/(B0) * Delp2(P0_tmp) )*vac_mask2;
	      //phi00_tmp1 = ( mesh->averageY(mesh->g11)*D2DX2(phi00) + 0.5*mesh->averageY(dnorm/(B0) * mesh->g11*D2DX2(P00)) )*vac_mask2;
	      //phi00_tmp2 = U_dc;
	      //phi00_tmp1 = ( (phi00*B0+0.5*dnorm*P0_tmp) )/B0*vac_mask2;
	      ddt(phi00_inv) = -phi00_tmp1;// - U_dc;
	      if (Hmode_rc_U00)
		ddt(phi00_inv) += U00;
	    }
  ...
  if (Hmode_rc)
	...
	{
	  ddt(P00) += 2. / 3. * kappa_par.DC() * Grad2_par2(P0)/converge_const;
	  ddt(P00) += 2. / 3. * Grad_par(kappa_par.DC()) * Grad_par(P0)/converge_const;
	}
      ...
      if (Hmode_rc_vexp)
	{
	  VeXp = b0xGrad_dot_Grad(phi+phi0, P) + b0xGrad_dot_Grad(phi, P0);
