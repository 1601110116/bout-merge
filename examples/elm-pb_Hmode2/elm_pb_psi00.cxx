...
BoutReal Hmode_rc_lowlimit, converge_const, converge_const_phi00, converge_const_psi00;
      ...
      if (Hmode_rc_phi00)
	{
	  ...
	  if (!restarting)
	      ...
	      else
		{
	      ...
	      phi00 = mesh->averageY(zonal_part(phi,low_pass_z));
...
/*	      Vexp_ave_tmp = b0xGrad_dot_Grad(phi00, P0);
	      ...
	      mesh->communicate(Vexp_ave_tmp);
*/
	      VeXp += b0xGrad_dot_Grad(phi00, P0) * mask_VeXp;
              if (nonlinear)
...
//     output.write("Now I see you t0_2.\n");
      ...
      if (Hmode_rc_p0)
	{
	  ...
	  ddt(P00) += d2dx2p0/converge_const;
          ...
          // sink terms
          if(sink_P > 0.0){
            ddt(P00) -= sink_P*zonal_part(sink_tanhxr(P0,P00,sp_width,sp_length), low_pass_z)*Tbar; // sink
          }

	  // source terms
	  if(heating_P > 0.0){
	    ddt(P00) += heating_P*zonal_part(sink_tanhxl(P0,P00,hp_width,hp_length), low_pass_z)*Tbar; // source
            //phi00_tmp3 = sink_tanhxl(P0,P00,hp_width,hp_length);
	  }
	}

      if (Hmode_rc_phi00)
	{
	  U_dc = zonal_part(U, low_pass_z);
	  ...
	  if (Hmode_rc_U00)
	    {
	      if (Hmode_rc_psi00)
	        ...
	        ddt(U00) = (B0^2) * zonal_part(b0xGrad_dot_Grad(zonal_part(Psi,low_pass_z), J0, CELL_CENTRE), low_pass_z); // Grad j term
		...
		// Parallel current term
		  ...
		  ddt(U00) -= (B0^2) * zonal_part(Grad_parP(Jpar00, CELL_CENTRE), low_pass_z); // b dot grad j
                  ...
                  ddt(U00) -= (B0^2)*zonal_part(Grad_parP(zonal_part(Jpar,low_pass_z), CELL_CENTRE), low_pass_z); // b dot grad j
	      }
		...
		ddt(U00) -= bracket(phi00, U00, bm_exb)*B0;    // Advection
                ddt(U00) -= zonal_part((bracket(phi, U, bm_exb)*B0), low_pass_z);
              }
	  ...
	  if (!constn0)
	    {
	      ...
	      ddt(phi00_inv) += mesh->averageY(zonal_part(U,low_pass_z)/N0);
...
/*          if(eHall) {
            ddt(psi00) +=  0.25*delta_i*((Grad_parP(B0*P00, CELL_CENTRE)).DC() / B0
                                 +b0xGrad_dot_Grad(P0, psi00));   // electron parallel pressure
          }
*/
          if(diamag_phi0)
