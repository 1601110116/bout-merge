...
const Field3D zonal_part(const Field3D &var, Field3D &non_zonal, int zmax);
...
int physics_run(BoutReal t)
{
  ...
  if (Hmode_rc2)
      ...
      if (Hmode_rc_vexp)
	{
      ...
      if (Hmode_rc_par)
	{
	  ddt(P00) += 2. / 3. * kappa_par * Grad2_par2(P0)/converge_const;
//	  ddt(P00) += 2. / 3. * Grad_par(kappa_par) * Grad_par(P0)/converge_const;
      ...
      if (Hmode_rc_p0)
	{
	  ...
	  // sink terms
	  if(sink_P > 0.0){
	    ddt(P00) -= sink_P*sink_tanhxr(P0,P00,sp_width,sp_length)*Tbar; // sink
	    ddt(P00) -= sink_P*sink_tanhxl(P0,P00,sp_width,sp_length)*Tbar; // sink
	  }
	}
    }
  else
