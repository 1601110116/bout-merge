...
/*
	  ...
	  //	    phi = -dnorm_p0 * pow(double(P_height),0.3)*(P00^0.7)/B0;
*/
	}
  ...
  if (iterate_phi0 && diamag_phi0)
      ...
      if (!constn0)
	{
	  ...
	  //	  ddt(phi0_cal) = (phi0_ratio*mesh->g11*D2DX2(phi0_cal) + 0.5*dnorm/(N0*B0) * mesh->g11*D2DX2(P0) + cross_tmp)*vac_mask2;
	}
      else
	{
...
/*	      Vexp_ave_tmp = b0xGrad_dot_Grad(phi00, P0);
	      ...
	      mesh->communicate(Vexp_ave_tmp);
*/
              VeXp += b0xGrad_dot_Grad(phi00, P+P00) * mask_VeXp;
              Field3D Vexp_ave_tmp2;
              Vexp_ave_tmp2 = 0.;
              Vexp_ave_tmp = zonal_part(VeXp, Vexp_ave_tmp2, low_pass_z);
	    }
      ...
      if (Hmode_rc_phi00)
	{
	  ...
	  if (!constn0)
	  ...
	  else
	    {
	      ...
	      //xqxu	      ddt(phi00_inv) = -phi00_tmp1;// - U_dc;
