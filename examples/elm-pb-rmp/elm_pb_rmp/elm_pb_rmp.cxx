...
Vector3D Btot; //magnetic field of rmp_psi+psi
Vector3D Bpsi;
Vector3D Brmp; // Perturbed field
Field3D absBrmp;
Field3D Brmpx;
Field3D Brmpy;
Field3D Brmpz;
...
int physics_init(bool restarting)
{
  ...
  if(include_rmp) {
	...
	// Currently only implemented for 4th-order serial inversion (NXPE = 1)
      ...
      // Load perturbation from grid file.
        BoutReal tmp[516][64][64];
	rmp_Psi0 = 1.0;
	ifstream pFile;
	pFile.open ("data/dens6_m7n3.txt", ios::in );
	    output.write("\tCheck point 4 : file is open\n" );
	for(int jx=0;jx<516;jx++)
	...
	for(int jy=0;jy<mesh->ngy;jy++)
	for(int jz=0;jz<mesh->ngz;jz++) {
	rmp_Psi0[jx][jy][jz] = rmp_factor * tmp[mesh->XGLOBAL (jx)][mesh->YGLOBAL (jy)][jz];
	}
  ...
  //gui2, cal B using rmp_psi and the total of rmp_psi+psi
  ...
  if (include_rmp) {
    Brmp = Grad(rmp_Psi) ^ B0vec;
    // Want contravariant components: B^x is change in psi, B^z is change in toroidal angle

    output.write("\tB0vec flag= %s\n",(B0vec.covariant)?"true":"false" );
    output.write("\tBrmpflag1= %s\n",(Brmp.covariant)?"true":"false" );
    Brmp.toContravariant();
    output.write("\tBrmpflag2= %s\n",(Brmp.covariant)?"true":"false");

    absBrmp = sqrt(Brmp * Brmp);
    Brmpx = Brmp * Bx /sqrt(Bx * Bx);
    Brmpy = Brmp * By /sqrt(By * By);
    Brmpz = Brmp * Bz /sqrt(Bz * Bz);

    dump.add(absBrmp, "Brmp", 0);
    dump.add(Brmpx, "Brmpx", 0);
    dump.add(Brmpy, "Brmpy", 0);
    dump.add(Brmpz, "Brmpz", 0);

    dump.add(absBtot, "Btot", 1);
    dump.add(Btotx, "Btx", 1);
    dump.add(Btoty, "Bty", 1);
    dump.add(Btotz, "Btz", 1);
  }

    dump.add(absBpsi, "Bpsi", 1);
    dump.add(Bpsix, "Bpsix", 1);
    dump.add(Bpsiy, "Bpsiy", 1);
    dump.add(Bpsiz, "Bpsiz", 1);
...
int physics_run(BoutReal t)
{
 ...
 if(include_rmp) {
	...
	rmp_Psi = 0.0 * rmp_Psi0 ; // Linear ramp
  ...
  if(include_rmp) {
    Btot = Grad(Psi + rmp_Psi) ^ B0vec;
    // Want contravariant components: B^x is change in psi, B^z is change in toroidal angle
    Btot.toContravariant();
    absBtot = sqrt(Btot * Btot);
    Btotx = Btot * Bx /sqrt(Bx * Bx);
    Btoty = Btot * By /sqrt(By * By);
    Btotz = Btot * Bz /sqrt(Bz * Bz);


  }
    Bpsi = Grad(Psi) ^ B0vec;
    // Want contravariant components: B^x is change in psi, B^z is change in toroidal angle
    Bpsi.toContravariant();
    absBpsi = sqrt(Bpsi * Bpsi);
    Bpsix = Bpsi * Bx /sqrt(Bx * Bx);
    Bpsiy = Bpsi * By /sqrt(By * By);
    Bpsiz = Bpsi * Bz /sqrt(Bz * Bz);
  ...
  if(evolve_jpar) {
    ...
    */
    ...
    if(diamag_phi0)
      ...
      ddt(Psi) += bracket(Psi + rmp_Psi, phi0 * B0, bm_mag);   // Equilibrium flow  PWXI

    if(withflow)                                //net flow
      ddt(Psi)-= V_dot_Grad(V0net, Psi + rmp_Psi);//gui2
  ...
  if(withflow&&K_H_term)                         //K_H_term
    ...
    ddt(U) -= bracket(phi*B0,U0, bm_exb); //PWXI

  if(diamag_phi0)
    //    ddt(U) -= b0xGrad_dot_Grad(phi0, U);   // Equilibrium flow
    ddt(U) -= bracket(phi0 * B0, U, bm_exb);   // Equilibrium flow PWXI

  if(withflow)                            // net flow
    ddt(U) -= V_dot_Grad(V0net, U);

  if(nonlinear) {
    ddt(U) -= bracket(phi * B0, U, bm_exb);    // Advection
  }
  ...
  if(evolve_pressure) {
    ...
    ddt(P) -= bracket(phi*B0, P0);    //PWXI

    if(diamag_phi0)
      //      ddt(P) -= b0xGrad_dot_Grad(phi0, P);   // Equilibrium flow
     ddt(P) -= bracket(phi0 * B0, P, bm_exb);   // Equilibrium flow PWXI

    if(withflow)                              //net flow
      ddt(P) -= V_dot_Grad(V0net, P);

    if(nonlinear)
      ddt(P) -= bracket(phi * B0, P, bm_exb);    // Advection
  }
