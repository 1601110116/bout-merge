...
int physics_init(bool restarting)
{
  ...
  delta_i = AA*60.67*5.31e5/sqrt(density/1e6)/(Lbar*100.0)/2.;
...
int physics_run(BoutReal t)
{
  ...
  if(evolve_jpar) {
    ...
    */
    ...
    //xqx      ddt(Ajpar) = -Grad_parP(B0*phi, CELL_YLOW) / B0 + eta*Jpar;

    if(eHall) {
      ddt(Ajpar) +=  0.25*delta_i*(Grad_parP(B0*P, CELL_CENTRE) / B0
    ...
    if(Prf>0.0)
      {
	ddt(Ajpar) -= Bbar*Tbar/(2.*1836*MU0*B0*Lbar*Lbar)*Frf;
      }
  }
  ...
  if(compress0) {
    ...
    //ddt(P) -=  coef_jpar*beta* Div_par_LtoC(Jpar*B0)/B0;
