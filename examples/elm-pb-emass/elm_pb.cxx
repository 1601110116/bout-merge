...
Field3D chi_par;         // Pressure Parallel Thermal Diffusion Profile
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, nogradparj,       false);  // exclude (B0^2)*Grad_par(Jpar)?
   ...
   if(emass)
   {
      ...
      output.write("                delta_e = %e    wci*T_A = %e   delta_e_inv = %e \n", delta_e, gyroAlv, delta_e_inv);
   }

   if(spitzer_resist) {
      // Use Spitzer resistivity
      output.write("\tTemperature: %e -> %e [eV]\n", min(Te), max(Te));
      eta = 0.51*1.03e-4*Zeff*12.4*(Te^(-1.5)); // eta in Ohm-m. NOTE: ln(Lambda) = 12.4
   ...
   {
      output.write("Solving for Ajpar, Differentiating to get jpar\n");
      SOLVE_FOR(Ajpar);
...
// Parallel gradient along equilibrium field-line
   ...
   if(evolve_jpar) {
      ...
      */
      ...
      else
      {
         ...
         ddt(Ajpar) = -Grad_parP0(B0*phi, CELL_CENTRE) / B0 + eta*Jpar;
         ...
         if(diamag && diamag_phi0)
            ddt(Ajpar) -= b0xGrad_dot_Grad(phi0, Ajpar);   // Equilibrium flow
         ...
         if(nonlinear) {
            ddt(Ajpar) -= b0xGrad_dot_Grad(phi, Ajpar);    // Advection
         }
