...
bool Ni_Constant_Grad,Ni_Zero_Grad,Thermal_sheath,Te_Constant;
bool term_GparkappaTe_GparTe;
bool terms_recombination,terms_Gradpar_pn,terms_Gradpar_eta0n,terms_cross,terms_exb,include_J0;

//***solve potential and vorticity with relaxation method****//
bool Solve_phi;
...
//*********************************************************liend 
  ...
  OPTION(options, Solve_phi, false); //solve potential with relaxation method 
  ...
  OPTION(options, Te_Constant, false);
  OPTION(options, Thermal_sheath, true);
             ...
             for(int k=0;k<gNi.zSize();k++)
              {
               ...
               gQi_xin(i,j,k) = -diffusion_coef_Hmode0*g_g11(0,j)*g_J(0,j)*gDDX_Ti(0,j,k)*gNi(0,j,k);
               gQe_xin(i,j,k) = -diffusion_coef_Hmode0*g_g11(0,j)*g_J(0,j)*gDDX_Te(0,j,k)*gNi(0,j,k);
               */gGamma_ni_xin(i,j,k) = -diffusion_coef_Hmode0*g_g11(0,j)*g_J(0,j)*gDDX_Ni(0,j,k);
...
//  dump.add(q95,"q95",0);
  ...
  dump.add(te_target,"te_target",0);
  dump.add(ni_target,"ni_target",0);
...
//**********************************************************liend
 ...
 //      BoutReal x_glb= mesh->GlobalX(jx); 
       //BoutReal xindx= mesh->XGLOBAL(jx); 
      // BoutReal x_glb = (psixy[jx][jy] - psi_axis)/(psi_bndry-psi_axis);  
      // BoutReal x_glb = xindx/ixsep;
        for (int jy=0;jy<mesh->ngy;jy++)
          {
   //         BoutReal y_glb=mesh->GlobalY(jy);
            BoutReal x_glb = (psixy[jx][jy] - psi_axis)/(psi_bndry-psi_axis);  
            BoutReal yindx = mesh->YGLOBAL(jy);
            BoutReal y_glb = yindx/NY;
 	     for (int jz=0;jz<mesh->ngz;jz++)
                {
		   ...
		   if(x_glb >= CF_BC_x0 && y_glb >= CF_BC_y0 && y_glb <= CF_BC_y1 && z_glb >= CF_BC_z0 && z_glb <= CF_BC_z1) 
		     { 
	              ...
	              Vmx[jx][jy][jz]=Vm0;
                       ...
                       Nm[jx][jy][jz]=Nm[jx][jy][jz];
                       Vmx[jx][jy][jz]=Vmx[jx][jy][jz];
                  }
             }
	}
...
//  dump.add(c_se,"c_se",0); 
  ...
  te_target = Te_sheath/Te_x;
  ni_target = Ni_exp*Te_exp/te_target;
  ...
  if(Sheath_BC)
    {
      ...
      if(Te_Constant){
        SBC_Dirichlet_phi(Te, te_target); 
        SBC_Dirichlet_phi(Ti, te_target); 
        SBC_Dirichlet_phi(Ni, ni_target);}  
      if(Thermal_sheath){
...
//*************************************************************li2016
  ...
  if(Solve_phi)
    ...
    phi00=phi01/sbc_lambda1;
    J1_para =  -1.0/(eta*sbc_lambda1*B0)*Grad_par(phi01)+((KB*eV_K*Te_x*tbar)/(eta*Lbar*Lbar*ee*Ni*B0*Bbar))*Grad_par(Pe)+((0.71*KB*eV_K*Te_x*tbar)/(eta*Lbar*Lbar*B0*Bbar*ee))*Grad_par(Te);
  ...
  for(xrup.first(); !xrup.isDone(); xrup.next())

    {
     // BoutReal x_glb= mesh->GlobalX(xrup->ind); 
      ...
      int indx =  mesh->XGLOBAL(xrup.ind);
      BoutReal dindx = indx/ixsep;
      if(dindx >= 1.0) 
  ...
  for(xrdn.first(); !xrdn.isDone(); xrdn.next())
    {
      ...
      //if(x_glb>=Sheath_BC_x0) 
      int indx =  mesh->XGLOBAL(xrdn.ind);
      BoutReal dindx = indx/ixsep;
      if(dindx >= 1.0) 
