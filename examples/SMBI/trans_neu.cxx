...
//parameters for source of ni
BoutReal S0_ni, x_rela;
BoutReal pei_test,dpdx_test,heavi_test;
...
// Load 2D profiles
 ...
 if(!include_curvature)
  ...
  output.write("\tdiff_coef_step0 %e, diff_coef_step0 %e \n", diffusion_coef_step0,diffusion_coef_step1);
  ...
  Ni= Te = Ti = 0.0;
  Vi=0.;
  Nn = Tn= Vn=0.0;       // it is necessary to set values for evolving quantities if not read from grid 
  Nm = Vmx = 0.0;
...
const Field3D BS_ft(const int index)
{
  ...
  dxlam = 1./index;       // right when max(Bxy)=bmag
