...
bool diamag_er;
Vector2D Ve0net, Ve0, Er0, Er0_balance;
BoutReal Er_factor;
Field2D U0_net, dp0;
...
int physics_init(bool restarting)
{
  ...
  OPTION(options, diamag_er,         false);
  OPTION(options, Er_factor,         1.);
...
/*    else {
      ...
      Er0_balance = 0.5*dnorm*Grad(P0)/N0;
  ...
  if(include_rmp) {
	...
	Jpar = 0.0;
	rmp_Psi0 = invert_laplace(Jpar, INVERT_4TH_ORDER, NULL);
  ...
  Va = sqrt(Bbar*Bbar / (MU0*Nbar*density*AA*Mi));

  Tbar = Lbar / Va;

  dnorm = dia_fact * AA*Mi / (2.0*Zi*1.602e-19*Bbar*Tbar);

  delta_i = 2.278e7/sqrt(density/1e6/AA)/(Lbar*100.0)/Zi;
...
/*  //xqx test begin
  ...
  if (experiment_Er)
  {
    if (diamag_er)
    {
      ...
      phi0 = -0.5*dnorm*P0/B0;
      Er0.covariant = true;
      Er0.x = Er_factor * Er_tmp / ( Bbar*Va*Rxy*Bpxy ) / B0;
      Er0.y = 0.;
      Er0.z = 0.;
      Er0.x.applyBoundary();
      ...
      if (constn0)
        Er0_balance = 0.5*dnorm*Grad(P0)/B0;
      else
        Er0_balance = 0.5*dnorm*Grad(P0)/N0/B0;
      mesh->communicate(Er0_balance.x);
      mesh->communicate(Er0_balance.y);
      mesh->communicate(Er0_balance.z);
      (Er0_balance.x).applyBoundary();
...
//    if(experiment_Er && diamag_er)                                //net flow
...
//  if(experiment_er)                                //net flow
//    ddt(U)-= V_dot_Grad(Ve0net, U);
