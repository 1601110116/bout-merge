...
int physics_init(bool restarting)
{
  ...
  mesh->get(P0, "pressure_couple"); // Pascals
  ...
  if (output_vradial)
    {
      ...
      if (output_transport_couple)
        ...
        if(chicouple_calc_withP)      // chii_couple and chie_couple calculated with P
          {
           kaii_couple = (((P+P0)*Vexb.x*Lbar*Va*sqrt(mesh->g11)*Lbar*Bbar/(B0^2)).DC())/(abs(DDX(P.DC()+P0))+Low_limit);
          ...
          // then Ni = P/(N0*Ratio_tilde_TioverNi+Ti0)
           ...
           Te = Ti;
           Di_couple =Lbar*Va*(((Ni+N0)*Vexb.x*sqrt(mesh->g11)*Lbar*Bbar/(B0^2)).DC())/(abs(DDX(Ni.DC()+N0))+Low_limit);
           kaii_couple = (((Ti+Ti0)*Vexb.x*Lbar*Va*sqrt(mesh->g11)*Lbar*Bbar/(B0^2)).DC())/(abs(DDX(Ti.DC()+Ti0))+Low_limit);
           kaie_couple = (((Te+Te0)*Vexb.x*Lbar*Va*sqrt(mesh->g11)*Lbar*Bbar/(B0^2)).DC())/(abs(DDX(Te.DC()+Te0))+Low_limit);
          }
