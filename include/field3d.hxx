...
class Field3D : public Field, public FieldData {
 public:
  ...
  friend const Field3D getAC(const Field3D &var);  // H.SETO (QST)
  friend const Field2D getDC(const Field3D &var);  // H.SETO (QST)
  friend const Field3D margeDCandAC(const Field2D &v2d, const Field3D &v3d); // H.SETO (QST)
