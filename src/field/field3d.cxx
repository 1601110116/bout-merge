...
// H.SETO (QST) v2017.09.27
const Field3D getAC(const Field3D &var)
{
  Field3D result;
  static dcomplex *f = NULL;
  int jx, jy;
...
const Field2D getDC(const Field3D &var) {
  ...
  for(int jx=0;jx<mesh->ngx;jx++) {
    for(int jy=0;jy<mesh->ngy;jy++) {
      ...
      irfft(f, ncz, dc3d.block->data[jx][jy]); // Reverse FFT

      dc3d.block->data[jx][jy][ncz] = dc3d.block->data[jx][jy][0];
    }
  }
  ...
  for(int jx=0;jx<mesh->ngx;jx++) {
    for(int jy=0;jy<mesh->ngy;jy++) {
      result(jx,jy) = dc3d.block->data[jx][jy][0];
    }
  }
  return result;
}

// H.SETO (QST) v2018.01.31
const Field3D margeDCandAC(const Field2D &v2d, const Field3D &v3d)
{
  Field3D result;
  static dcomplex *f = NULL;
  int jx, jy;
...
#ifdef CHECK
  msg_stack.push("addDCtoAC(Field2D,Field3D)");
#endif
  ...
  for(jx=0;jx<mesh->ngx;jx++) {
    for(jy=0;jy<mesh->ngy;jy++) {
      // Take FFT in the Z direction
      rfft(v3d.block->data[jx][jy], ncz, f);

      f[0] += v2d[jx][jy];//add DC to AC in Fourier space (note f is nomarized with ncz in rfft)
