...
const Field3D InvertParCR::solve(const Field3D &f, const int bndry_flag,
    const Field3D &SBC_value) {
/*
  - f: RHS
  - bndry_flag: boundary condition flag for open surface
    * 0: Dirichlet
    * 1: Neumann, [default]
...
*/
  ...
  for(surf.first(); !surf.isDone(); surf.next()) {
    ...
    if(bndry_flag == 2) {
      for(int y=0;y<mesh->ngy-4;y++)
        rfft(SBC_value[x][y+2], mesh->ngz-1, sbc[y+y0]);
    }
	...
	rhsk[k][y+y0] = rhs[y+y0][k]; // Transpose
        if (bndry_flag == 2) sbck[k][y+y0] = sbc[y+y0][k];
      }
    }
    ...
    if(closed) {
      ...
      #ifdef DEBUG
        output << "InvertParCR: bndry_flag = " << bndry_flag << "\n";
      #endif
      switch(bndry_flag) {
        case 0: {
        #ifdef DEBUG
          output.write("InvertParCR: bndry_flag = 0, Dirichlet!\n");
        #endif
          if(surf.firstY()) {
            for(int k=0; k<nsys; k++) {
              for(int y=0;y<2;y++) {
        ...
        case 1: {
        #ifdef DEBUG
          output.write("InvertParCR: bndry_flag = 1, Neumann [DEFAULT]!\n");
        #endif
          if(surf.firstY()) {
            for(int k=0; k<nsys; k++) {
              for(int y=0;y<2;y++) {
        ...
        case 2: {
        #ifdef DEBUG
          output.write("InvertParCR: bndry_flag = 2, Sheath Boundary Condition!\n");
        #endif
          // if (SBC_value == NULL) {
            // throw BoutException("ERROR: InvertParCR: SBC_value: NULL");
          // }
          if(surf.firstY()) {
            for(int k=0; k<nsys; k++) {
              for(int y=0;y<3;y++) {
                ...
                //rhsk[k][y] = -SBC_value[x][y];
                rhsk[k][y] = -sbck[k][y];
                #ifdef DEBUG
                  std::cout << "rhsk: inline firstY: " << rhsk[k][y] << std::endl;
                #endif
              }
            }
          }
          if(surf.lastY()) {
            for(int k=0; k<nsys; k++) {
              for(int y=size-3;y<size;y++) {
                ...
                rhsk[k][y] = sbck[k][y];
                #ifdef DEBUG
                  std::cout << "rhsk: inline LastY: " << rhsk[k][y] << std::endl;
                #endif
              }
            }
          }
        ...
        default: {
          throw BoutException("InvertParCR: Open Surface bndry_flag MUST be 0-2!\n");
        }
      }
    }
