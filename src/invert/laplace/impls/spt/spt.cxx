...
const FieldPerp LaplaceSPT::solve(const FieldPerp &b, const FieldPerp &x0) {
  ...
  if( (inner_boundary_flags & INVERT_SET) || (outer_boundary_flags & INVERT_SET) ) {
    ...
    if((inner_boundary_flags & (INVERT_SET | INVERT_IN_GRAD2) ) && mesh->firstX()) {
      // Copy x0 inner boundary into bs
      for(int ix=0;ix<xbndry;ix++)
        for(int iz=0;iz<mesh->ngz-1;iz++)
          bs[ix][iz] = x0[ix][iz];
    }
    if((outer_boundary_flags & ( INVERT_SET | INVERT_OUT_GRAD2)) && mesh->lastX()) {
...
const Field3D LaplaceSPT::solve(const Field3D &b, const Field3D &x0) {
  if(  ((inner_boundary_flags & (INVERT_SET | INVERT_IN_GRAD2)) && mesh->firstX()) ||
       ((outer_boundary_flags & (INVERT_SET | INVERT_OUT_GRAD2)) && mesh->lastX()) ) {
    Field3D bs = copy(b);
...
void LaplaceSPT::tridagForward(dcomplex *a, dcomplex *b, dcomplex *c,
  ...
  for(j=1;j<n;j++) {
    gam[j] = c[j-1]/bet;
    bet = b[j]-a[j]*gam[j];
    //output.write("c[ %i ] = %e, %e\n",j-1,c[j-1].Real(), c[j-1].Imag());
    //output.write("b[ %i ] = %e, %e\n",j,b[j].Real(), b[j].Imag());
    //output.write("gam[ %i ] = %e, %e\n",j,gam[j].Real(), gam[j].Imag());
    //output.write("bet = %e, %e\n",bet.Real(), bet.Imag());
    if(bet == 0.0)
