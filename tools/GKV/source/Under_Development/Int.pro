...
;	4/30/02
;
result = GetKeyWord('Cumulative', Extra)
IF(Query_Integer(result)) THEN BEGIN
	IF(result EQ 1) THEN result = self -> CINT(arg, _EXTRA=Extra)
	RETURN, result
ENDIF
CASE N_PARAMS() OF
...
CASE axis OF
	1:	FOR i=0, info[1]-2 DO   values[i, *, *, *] =  0.5*( (*valuePtr)[i, *, *, *] + (*valuePtr)[i+1, *, *, *])*dx[i]
	2:	FOR i=0, info[1]-2 DO   values[*, i, *, *] =  0.5*( (*valuePtr)[*, i, *, *] + (*valuePtr)[*, i+1, *, *])*dx[i]
	3:	FOR i=0, info[1]-2 DO   values[*, *, i, *] =  0.5*( (*valuePtr)[*, *, i, *] + (*valuePtr)[*, *, i+1, *])*dx[i]
	4:	FOR i=0, info[1]-2 DO   values[*, *, *, i] =  0.5*( (*valuePtr)[*, *, *, i] + (*valuePtr)[*, *, *, i+1])*dx[i]
ENDCASE
;
; Compute Average and standard deviation
;
...
CASE axis OF
	...
	2:	FOR i=0, info[2]-2 DO   ErrorSq[*, i, *, *] = ( 0.5*(*errorPtr)[*, i, *, *] + (*errorPtr)[*, i+1, *, *] )^2*dx[i]
	3:	FOR i=0, info[3]-2 DO   ErrorSq[*, *, i, *] = ( 0.5*(*errorPtr)[*, *, i, *] + (*errorPtr)[*, *, i+1, *] )^2*dx[i]
	4:	FOR i=0, info[4]-2 DO   ErrorSq[*, *, *, i] = ( 0.5*(*errorPtr)[*, *, *, i] + (*errorPtr)[*, *, *, i+1] )^2*dx[i]
ENDCASE
	avgErrorSq = TOTAL(errorSq, axis)
	errorBars = SQRT(avgErrorSq)
	PTR_FREE, errorPtr
