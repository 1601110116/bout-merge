...
FUNCTION GKVs1D::Filter, arg, filter=filterObj, _Extra=extra
;
; Purpose:
;
...
; Output Keywords:
;
;	Filter		If this keyword is set equal to symbol on the command line then,
;			on return, that symbol will refer to a GKVs1D object containing
;			the frequency representation the filter applied.
;
;
...
;
; Load values into 'temp', and apply window function
;
twindow = Window_Fcn(nPoints, filterLength)
temp = MAKE_ARRAY(ntemp, TYPE=valueType)
temp[0:(npoints-1)] = values[0:(npoints-1)]*twindow
;
...
output -> restrict
;
; Check for 'Filter' keyword, and prepare filterObj
; if requested.
;
IF ARG_PRESENT(filterObj) THEN BEGIN
	filterStr = {GKVs1D}
	filterStr.mnemonic = "Filter"
	filterSTR.title = "F"
	filterIndices = ["*"]
	filterSTR.indices = PTR_NEW(filterIndices)
	filterSTR.units = ""
	filter = FLOAT(filter)
	filter = SHIFT(filter, nTemp/2)
	filterStr.Values = PTR_NEW(filter)
	fmin = GKVsd_MIN(filter, Max=fmax)
	filterStr.vrange = [0, fmax]
	filterStr.Codename = self.mnemonic
	dTstring = STRING(dT, FORMAT='(G10.3)')
	dTstring = STRCOMPRESS(dTstring, /REMOVE_ALL)
	filterStr.RunID = "dT = " + dTstring
	fGrid = {Grid}
	fGrid.mnemonic = "omega"
	fGrid.title = "!4x!X"
	fGrid.units = '1/(' + self.grid1.units + ')'
	dOmega = 2*!PI/(deltaT*nTemp)
	omegas = dOmega*(FINDGEN(nTemp) - nTemp/2)
	fGrid.values = PTR_NEW(omegas)
	fGrid.uniform = 1b
	fGrid.range = [omegas[0], omegas[ntemp-1]]
	fGrid.irange = [0, ntemp-1]
	filterStr.Grid1 = fGrid
	filterObj = OBJ_NEW("GKVs1D", filterStr)
ENDIF
;
; and we're done
;
...
;
; ***********************************************************
; *THERE IS A BUG FOR SPATIAL GRIDS THAT ARE NOT PERIODIC!!!*
; ***********************************************************
;
; Make copy of self
;
