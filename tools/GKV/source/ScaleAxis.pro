...
;
nSupplied = N_ELEMENTS(gridValues)
IF(nSupplied NE 0) THEN BEGIN
	nSelf = N_ELEMENTS(*grid.values)
	IF( nSupplied LT nSelf ) THEN BEGIN
		PRINT, 	grid.mnemonic, "-axis has ", nSelf, "grid points, while ", $
				nSupplied, " were supplied, Returning"
		RETURN
	ENDIF
	IF( nSupplied GT nSelf ) THEN BEGIN
		PRINT, 	grid.mnemonic, "-axis has ", nSelf, "grid points, while ", $
				nSupplied, " were supplied. Will use first ", nSelf, ' values.'
		gridValues = gridValues[0:(nSelf-1)]
	ENDIF	
...
;
;  commandStr = 'PTR_FREE, self.Grid' + axisStr + '.values'
;  ok = EXECUTE(commandStr)
...
;
	...
	commandStr = 'PTR_FREE, target.Grid' + axisStr + '.values'
	ok = EXECUTE(commandStr)
