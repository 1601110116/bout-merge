...
FUNCTION mGENE_NRG_Data, fileName, _Extra=extra
;
...
; Modified by W.M. Nevins
;	2/12/2009
; to interface with mutlitple GENE files
...
IF(N_ELEMENTS(nt) EQ 0) THEN BEGIN
	...
	PRINT, "must be specified, RETURNING"
	RETURN, 0
ENDIF
...
END ;  ****** mGENE_NRG_Data.pro ******  ;


FUNCTION MultipleGENE_Data, 	FileName=fileName, Path=path, Search=search,	$
...
; Modified by W.M. Nevins
;	2/12/2009
; to read multiple NRG files
; with embedded sequence numbers
;
deBug=0
...
;******		IF(cdfID GE 0) THEN *****, cdfID		; Close any open files
		IF(Query_String(CurrentWorkingDirectory)) $
			THEN CD, CurrentWorkingDirectory	; return to original directory
...
;
output = { NAME : "MultipleGENE_Data" }
;
; Find "parameters" files
;
ParamFiles = FILE_Search("parameters_*", COUNT=nParamFiles)
IF(nParamFiles EQ 0) THEN BEGIN
	MESSAGE, "Failed to find any PARAMETERS files, Returning", /INFORMATIONAL
	RETURN, output
ENDIF
SequenceNumber = STRARR(nParamFiles)
;
; Find number of NRG files
;
nrgFiles = FILE_SEARCH( "nrg_*", COUNT=nNrgFiles)
IF(nNrgFiles EQ 0) THEN BEGIN
	MESSAGE, "No NRG files found, Returning", /INFORMATIONAL
	RETURN, output
ENDIF
IF(nNrgFiles GT nParamFiles) THEN BEGIN
	MESSAGE, "More NRG files than PARAMETERS files", /INFORMATIONAL
	nNrgFiles = nParamFiles
ENDIF
;
; Loop for reading NRG files
;
iNRG = 0
FOR iParams = 0, nParamFiles-1 DO BEGIN
;
; Find sequence number
; 
subStrings = STRSPLIT(ParamFiles[iParams], "_", COUNT=nSubStrings, /EXTRACT)
IF(nSubStrings NE 2) THEN BEGIN
	MESSAGE, "Failed to find PARAMETERS sequence number, Returning", /INFORMATIONAL
	RETURN, output
ENDIF
sequenceNumber[iParams] = subStrings[1]
;
; Read corresponding PARAMETERS file
;
params = GENE_ReadParameters(ParamFiles[iParams])
;
; Create structure array to hold params structures
;
IF(iParams EQ 0) THEN BEGIN
	paramsArray = REPLICATE(params, nParamFiles)
	output = CREATE_STRUCT(output, "Params", paramsArray)
	modelParams = params
ENDIF ELSE BEGIN
	STRUCT_ASSIGN, /VERBOSE, params, modelParams
	params = modelParams
ENDELSE
;
; Add params structure to output structure
;
thisName = STRCOMPRESS('params_' + sequenceNumber[iParams], /REMOVE_ALL)
params.Name = thisName
output.params[iParams] = params
paramsArray[iParams]   = params
nt_NRG=params.info.iTime/params.in_out.istep_nrg
;
; nt_NRG is less than 1 indicates abnormal run termination.
; Just make it big -- GENE_NRG_DATA won't read beyond the 
; EOF.
IF(nt_NRG LT 1) THEN nt_NRG=10000

nSpecies = params.box.n_spec
;
; Find corresponding "nrg" file
;
fileName = STRCOMPRESS("nrg_" + sequenceNumber[iParams], /REMOVE_ALL)
NrgFile = FILE_SEARCH(filename, COUNT=nFiles)
IF(nFiles EQ 0) THEN BEGIN
	MESSAGE, "Failed to find " + fileName, /INFORMATIONAL
ENDIF ELSE BEGIN
;
; Read this NRG FILE
;
FORWARD_FUNCTION GENE_NRG_DATA
nt_NRG=params.info.iTime/params.in_out.istep_nrg
nSpecies = params.box.n_spec
IF(TypeOF(Extra) EQ 8) THEN nrgExtra = EXTRA
nrgStr = GENE_NRG_DATA(fileName, nt_NRG=nt_NRG, nSpecies=nSpecies, ParamStr=params, _Extra=nrgExtra)
;
; Create stucture array to hold data from nrg files
; 
IF(iNRG EQ 0) THEN BEGIN
	nrgArray = REPLICATE(nrgStr, nNrgFiles)
ENDIF
nrgArray[iNRG] = nrgStr
iNRG = iNRG + 1
ENDELSE
ENDFOR
;
...
;
output = CREATE_STRUCT(output, "sequenceNumber", sequenceNumber)

IF(iNRG LT 2) THEN BEGIN
	IF(iNRG) THEN output = CREATE_STRUCT(output, "nrgStr", nrgstr)
	CD, CurrentWorkingDirectory
	RETURN, output
ENDIF
;
; Concatenate all objects in nrgArray into a single nrg Structure
;
nTags = N_TAGS(nrgStr)
FOR iTag = 1, nTags-1 DO BEGIN
	nObjs = N_ELEMENTS(nrgStr.(iTag))
	thisArray = nrgStr.(iTag)
	FOR iObj = 0, nObjs-1 DO BEGIN
		lastObj = ((nrgArray[0]).(iTag))[iObj] 
		FOR jnrg = 1, iNRG-1 DO BEGIN
			thisObj = lastObj -> Cat( ((nrgArray[jnrg]).(iTag))[iObj] )
			((nrgArray[jnrg]).(iTag))[iObj] -> Trash
			lastObj -> Trash
			lastObj = thisObj
		ENDFOR
		thisArray[iObj] = thisObj
	ENDFOR
	nrgStr.(iTag) = thisArray
ENDFOR
;
; Put this NRG structure into output structure
;
output = CREATE_STRUCT("nrg", nrgStr, output)
;
; Find number of FIELD files
;
fieldFiles = FILE_SEARCH( "field_*", COUNT=nFieldFiles)
IF(nFieldFiles EQ 0) THEN BEGIN
	MESSAGE, "No FIELD files found, Returning", /INFORMATIONAL
	RETURN, output
ENDIF
IF(nFieldFiles GT nParamFiles) THEN BEGIN
	MESSAGE, "More Field files than PARAMETERS files", /INFORMATIONAL
	nFieldFiles = nParamFiles
ENDIF
;
; Loop for reading FIELD files
;
iField = 0
FOR iParams = 0, nParamFiles-1 DO BEGIN
thisParams = ParamsArray[iParams]
fileName = STRCOMPRESS("field_" + sequenceNumber(iParams), /REMOVE_ALL)
;
; Find corresponding "field" file
;
FieldFile = FILE_SEARCH(filename, COUNT=nFiles)
IF(nFiles EQ 0) THEN BEGIN
	MESSAGE, "Failed to find " + fileName, /INFORMATIONAL
ENDIF ELSE BEGIN
;
; Read this field FILE
;
IF(TypeOF(extra) EQ 8) THEN fieldExtra=Extra
FORWARD_FUNCTION GENE_FieldData
fieldStr = GENE_FieldData(FieldFile, paramStr=thisParams, _Extra=fieldExtra)
;
; Create stucture array to hold data from FIELD files
; 
IF(iField EQ 0) THEN BEGIN
	fieldArray = REPLICATE(fieldStr, nFieldFiles)
ENDIF
fieldArray[iField] = fieldStr
iField = iField + 1
ENDELSE
ENDFOR

IF(iField LT 2) THEN BEGIN
	IF(iField) THEN output = CREATE_STRUCT(output, "field", fieldstr)
	CD, CurrentWorkingDirectory
	RETURN, output
ENDIF
;
; Transfer field objects from structure array to object arrays
;
field = { Name	: "Field"	}
fieldTags = Tag_Names(fieldStr)
nTags = N_TAGS(FieldStr)
ObjArray = OBJARR(iField)
FOR iTag = 1, nTags-1 DO BEGIN
	FOR jField = 0, iField-1 DO BEGIN
		ObjArray[jField] = (FieldArray[jField]).(iTag) 
	ENDFOR
	thisObj = Make_mGeneData(ObjArray)
	field = CREATE_STRUCT(field, fieldTags[iTag], thisObj)
ENDFOR
;
; Put Field structure into output structure
;
output = CREATE_STRUCT(output, "Field", field)

CD, CurrentWorkingDirectory
RETURN, output
END  ; ****** FUNCTION MultipleGENE_Data ****** ;		
