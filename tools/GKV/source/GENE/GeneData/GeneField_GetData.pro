...
;	11/7/2008
;
; Revised by W.M. Nevins (at suggestion of E. Wang)
; to clear pointer to grid-values and thereby avoid heap leakage.
; 5/28/09
;
CASE arg OF
  1   : PTR_FREE, self.grid1.values
  2   : PTR_FREE, self.grid2.values
  3   : PTR_FREE, self.grid3.values
  4   : PTR_FREE, self.grid4.values
ENDCASE
...
FUNCTION GeneField::GetData, arg, _Extra=Extra
;
...
ENDFOR
;
; load new grids into output
;
newGrids = rangeInfo.GridArray
FOR i=1,4 DO output -> GridSet, i, newGrids[i-1]
;
; load field values into output
;
output -> Set, values = PTR_NEW(FieldValues)
;
; Slice axes with no range
;
slice = rangeInfo.SliceArray
dAxis = 0
FOR iaxis=1,4 DO BEGIN
	IF(slice(iAxis-1)) THEN BEGIN
		temp = output -> Slice(axis=iAxis-dAxis, index=0)
		dAxis=dAxis+1 	; Need to correct axis count
		output -> Trash	; as dimensinality is reduced
		output = temp	; by "slicing"
	ENDIF
ENDFOR
;
; ane we're done!
