...
IF thisWidth EQ targetWidth THEN RETURN, size	;  was size* Float(!D.Y_Size)/!D.X_Size
									;  persumably, this was an aspect ratio correction.
   ; Initial size is too big.				;  Removed by WMN 7/19/00
...
IF thisWidth GT targetWidth THEN BEGIN
   REPEAT BEGIN
     XYOUTS, 0.5, 0.5, ALIGN=0.5, string, WIDTH=thisWidth, $
        CHARSIZE=-size, /NORMAL
      size = size - step
      IF(size lt step) THEN RETURN, step
   ENDREP UNTIL thisWidth LE targetWidth
   RETURN, size 						;  was size* Float(!D.Y_Size)/!D.X_Size									;  persumably, this was an aspect ratio correction.
   									;  Removed by WMN 7/19/00
ENDIF
