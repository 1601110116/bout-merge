...
;-
; *** Various modifications by W.M. Nevins 8/00 for use in viewing GKV objects
...
CASE thisEvent OF
   ...
   'SELECT_FILE': BEGIN
      ...
      pick = Dialog_Pickfile(Path=startDirectory, file=info.FileName, /NoConfirm, $
         Get_Path=path, /Write)
...
info = { filenameID:filenameID, xsizeID:xsizeID,	$
         ...
         orderdropID:orderdropID, ptr:ptr, 		$
         sliderID:sliderID, FileName:filename}
...
IF error NE 0 THEN BEGIN
   ...
   IF N_ELEMENTS(thisDevice) GT 0 THEN BEGIN
   	SET_PLOT, thisDevice
   	!P.COLOR=1
   	!P.BACKGROUND=0
   ENDIF
...
; 
; Save old values of !P.BACKGROUND and !P.COLOR
;
      ...
      SET_PLOT, 'PS'
;
; Compute "thickness" (measured in pixel sizes ...) corresponding to 1 pt ( = 1/72 in)
;
	pixelSize = 0.3937/SQRT(!D.X_PX_CM*!D.Y_PX_CM)		; pixel size in inches
	psThick = (1/72.)/pixelSize/10.				; somehow there is an extra factor of 10 ...
;
; Reset default line thicknesses to 1 pt times previous thickness
;
	!P.THICK = PThick*psThick
	!X.THICK = XThick*psThick
	!Y.THICK = YThick*psThick
	!Z.THICK = ZThick*psThick
	!P.CHARTHICK = PCHARTHICK*psThick
 ;
 ; Set colors for background and lines
 ;	
      ...
      TVLCT, r, g, b
 ;
 ; Restore colors for background and lines
 ;	
      ...
      !P.Background = info.background
 ;
 ; Restore original default line thicknesses
 ;
      	!P.THICK = PThick
	!X.THICK = XThick
	!Y.THICK = YThick
...
PRO XWINDOW, proName, param1, param2, param3, $ ; (WMN removed these keyword 7/26/00) GROUP_LEADER=group, WID=wid, TOP=tlb, DRAWID=drawid, $
   _EXTRA=extra
   
; The following keywords were in Fanning's source.  I'm catching them
; with GetKeyWord below in order to SUPRESS keyword abreviation.
; (they all still work, just can't be abreviated!)
;
;   WXSIZE=xsize, WYSIZE=ysize, XCOLORS=colors, $
;   WTITLE=wtitle, JUST_REGISTER=justRegister, $
;   OUTPUT=output, NO_CHANGE_CONFIG=nochange, ERASE=erase, $
;   PROTECT=protect, NOMENU=nomenu, CPMENU=cpmenu, $
;   WXPOS=wxpos, WYPOS=wypos, BACKGROUND=background
;
FORWARD_FUNCTION GetKeyWord, Query_Integer

debug = 0
result = GetKeyWord('debug', extra)
IF(TypeOF(result) NE 7) THEN debug = result

IF(debug EQ 0) THEN BEGIN					; Catch errors.
	CATCH, thisError
...
ENDIF
;
; Get Fanning's keywords with GetKeyWord
;
result = GetKeyWord('WXSIZE', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN xsize = result		;	WXSIZE=xsize 
										; on the command line
										;
result = GetKeyWord('WYSIZE', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN ysize = result		;	WYSIZE=ysize 
										; on the command line
										;
result = GetKeyWord('XCOLORS', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN colors = result		;	XCOLORS=colors 
										; on the command line
										;
result = GetKeyWord('WTITLE', extra)				; Equivalent to having
IF( (TypeOf(result) EQ 7)) THEN BEGIN
	IF(result NE 'undefined') THEN wtitle = result	;	WTITLE=wtitle 
ENDIF										; on the command line
										;
result = GetKeyWord('JUST_REGISTER', extra)		; Equivalent to having
IF(TypeOf(result) NE 7) THEN justRegister = result	;	JUST_REGISTER=justRegister 
										; on the command line
										;
result = GetKeyWord('OUTPUT', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN output = result		;	OUTPUT=output
										; on the command line
										;
result = GetKeyWord('NO_CHANGE_CONFIG', extra)		; Equivalent to having
IF(TypeOf(result) NE 7) THEN nochange = result		;	NO_CHANGE_CONFIG=nochange
										; on the command line
										;
result = GetKeyWord('ERASE', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN erase = result		;	ERASE=erase
										; on the command line
										;
result = GetKeyWord('PROTECT', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN protect = result		;	PROTECT=protect
										; on the command line
										;
result = GetKeyWord('NOMENU', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN nomenu = result		;	NOMENU=nomenu
										; on the command line
										;
result = GetKeyWord('CPMENU', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN cpmenu = result		;	CPMENU=cpmenu
										; on the command line
										;
result = GetKeyWord('WXPOS', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN wxpos = result		;	WXPOS=wxpos
										; on the command line
										;
result = GetKeyWord('WYPOS', extra)				; Equivalent to having
IF(TypeOf(result) NE 7) THEN wypos = result		;	WYPOS=wypos
										; on the command line
										;
result = GetKeyWord('BACKGROUND', extra)			; Equivalent to having
IF(TypeOf(result) NE 7) THEN background = result	;	BACKGROUND=background
										; on the command line
										;
IF(Query_Integer(extra)) THEN extra=0
...
CD, Current=thisDir
param1 -> get, mnemonic=myMnemonic
myMnemonic=STRCOMPRESS(myMnemonic, /REMOVE_ALL)
ps =   {PS_FORM_INFO, XSIZE:5.0, XOFF:1.75, YSIZE:5.0, $
       YOFF:3.0, FILENAME:FilePath(Root_Dir=thisDir,myMnemonic + '.ps'), $
...
gif =  {XWINDOW_GIF,XSIZE:400, YSIZE:400, COLOR:1, $
       FILENAME:FilePath(Root_Dir=thisDir,myMnemonic + '.gif'), $
       ORDER:0, QUALITY:-1}
jpeg = {XWINDOW_JPEG,XSIZE:400, YSIZE:400, COLOR:1, $
       FILENAME:FilePath(Root_Dir=thisDir,myMnemonic + '.jpg'), $
       ORDER:0, QUALITY:75}
tiff = {XWINDOW_TIFF,XSIZE:400, YSIZE:400, COLOR:1, $
       FILENAME:FilePath(Root_Dir=thisDir,myMnemonic + '.tif'), $
       ORDER:1, QUALITY:-1}
...
WIDGET_CONTROL, tlb, SET_UVALUE=info, /NO_COPY
   ...
   ; Register the program as non-blocking.
