...
FUNCTION GKVs1D::Velocity_Spline, v=v, _extra=extra 
;
...
;
copy = self -> makeCopy()
copy -> Velocity_Spline, v=v, _extra=extra
RETURN, copy
END;   ******** Velocity_Spline *******

PRO GKVs1D::Velocity_Spline, v=v, _extra=extra
;
...
newGrid = gkvsd_gridcopy(rgrid)
newGridValues = *(newGrid.values)
newGridValues2 = newGridValues + overstep
PTR_FREE, newGrid.values
newGrid.values = PTR_NEW(newGridValues2)
tempObject = self -> slice(t=i*dt+tgrid.range[0])
CASE ndims OF
    2: BEGIN
        offset = values[*,i]
        temp1 = SHIFT(offset,steps)
        PTR_FREE, tempObject.values
        tempObject.values = PTR_NEW(temp1)
        PTR_FREE, tempObject.grid1.values
        tempObject.grid1 = newgrid
        final= fltarr(rUnits+1)
        sp = tempObject -> make_spline(axis=iaxis)
        ;PRINT, "overstep = ", overstep, " and t = ", i*dt + tgrid.range[0]
        final = tempObject -> splint_vector(axis=iaxis,spline=sp,delx=overstep)
        tempObject -> trash
        sp -> trash
    END
    3: BEGIN 
        ...
        CASE iaxis OF
            ...
            2: BEGIN
                ...
                PTR_FREE, tempObject.values
                tempObject.values = PTR_NEW(temp1)
                PTR_FREE, tempObject.grid2.values
                tempObject.grid2 = newgrid                
                sp = tempObject -> make_spline(axis=iaxis)                
                final = tempObject -> splint_vector(axis=iaxis,spline=sp,delx=overstep)
                tempObject-> trash
                sp -> trash
    ...
    4: BEGIN 
        ...
        CASE iaxis OF
            ...
            3:  BEGIN
                ...
                PTR_FREE, tempObject.values
                tempObject.values = PTR_NEW(temp1)
                PTR_FREE, tempObject.grid3.values
                tempObject.grid3 = newgrid                
                sp = tempObject -> make_spline(axis=iaxis)                
                final = tempObject -> splint_vector(axis=iaxis,spline=sp,delx=overstep)
                tempObject-> trash
                sp -> trash
