...
FOR i=0, nTags-1 DO BEGIN
	...
	IF( STRCMP(Gyro_tags(i), "NL_TRANSFER", /FOLD_CASE) ) THEN aNl_transfer = GyroStr.(i)
	IF( STRCMP(Gyro_tags(i), "TURB_ENERGY", /FOLD_CASE) ) THEN aTurb_energy = GyroStr.(i)
ENDFOR
...
;	aPressure -> Trash	
ENDIF 
IF(TypeOF(aNl_transfer) EQ 11) THEN BEGIN
	Nl_transfer = aNl_transfer -> GYRO_ScalePhi(q=q, AspectRatio=AspectRatio, rhoStar=rhoStar, 	$
					L_Perp=L_Perp, perpScale=perpScale)
	Nl_transfer -> Set, Title="!12NL!X", mnemonic="NL_transfer" ;, units=""
	Nl_transfer -> Set, axis=3, gridunits = PerpScale + "/v!Dti!N"
	Nl_transfer -> ScaleAxis, 't', const=mu
	Nl_transfer -> ScaleAxis, axis =1, const=.0545588, offset = -3.46383
	Nl_transfer -> ScaleAxis, axis =1, const=1.0, offset = -.028
	Nl_transfer -> set, axis = 1, gridtitle='k_r'
	Nl_transfer -> set, axis = 1, gridmnemonic='k_r'
	Nl_transfer -> set, axis = 1, gridunits=""
	Nl_transfer = Nl_transfer -> fft('d_perp')	
;	aNl_transfer -> Trash	
ENDIF 
 
IF(TypeOF(aTurb_energy) EQ 11) THEN BEGIN
	Turb_energy = aTurb_energy -> GYRO_ScalePhi(q=q, AspectRatio=AspectRatio, rhoStar=rhoStar, 	$
					L_Perp=L_Perp, perpScale=perpScale)
	Turb_energy -> Set, Title="!12TE!X", mnemonic="Turb_energy" ;, units=""
	Turb_energy -> Set, axis=3, gridunits = PerpScale + "/v!Dti!N"
	Turb_energy -> ScaleAxis, 't', const=mu
	Turb_energy -> ScaleAxis, axis=1, const=.0545588, offset=-3.46383
	Turb_energy -> ScaleAxis, axis=1, const=1.0, offset=-.028
	Turb_energy -> set, axis=1, gridtitle='k_r'
	Turb_energy -> set, axis=1, gridmnemonic='k_r'
	Turb_energy -> set, axis=1, gridunits=''
	Turb_energy = Turb_energy->fft('d_perp')
;	aTurb_energy -> Trash	
ENDIF 
; 
...
IF(TypeOF(Nl_transfer) EQ 11) THEN $
	output = CREATE_STRUCT(output, "Nl_transfer", Nl_transfer)
IF(TypeOF(Turb_energy) EQ 11) THEN $
	output = CREATE_STRUCT(output, "Turb_energy", Turb_energy)
