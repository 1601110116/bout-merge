FUNCTION GKVsd_GyroData2, GyroStr, q=qin, aspectRatio=aspectRatioIn, 		$
...
; Get Diff_t
;
Diff_T = GyroStr.Diff_t_1 -> Times(Mu*L_perp)
...
IF(TypeOf(a_par) EQ 11) THEN BEGIN
	a_parallel = a_par -> GYRO_ScalePhi(q=q, AspectRatio=AspectRatio, rhoStar=rhoStar, 	$
					L_Perp=L_Perp, perpScale=perpScale)
	a_parallel -> Set, axis=3, gridunits = PerpScale + "/v!Dte!N"
