...
FUNCTION GKVs1D::AxisIrange, arg, AxisValue=thisAxisValue, _Extra=extra
;
; Purpose:
;
	...
	FOR iaxis = 1, nDIms DO BEGIN
		...
		IF(TypeOf(axisValue) NE 7) THEN BEGIN
			CASE N_ELEMENTS(axisValue) OF
				1:	thisAxisValue = axisValue
				2:	self -> SignalWindow, axis=iaxis, range = axisValue
			     ELSE:
			ENDCASE
			RETURN, iaxis
		ENDIF
...
; form new values array
;
newValues = (*self.Values)[imin:imax, jmin:jmax, kmin:kmax]
;
; Load new values array into 'self'
;
PTR_FREE, self.values
