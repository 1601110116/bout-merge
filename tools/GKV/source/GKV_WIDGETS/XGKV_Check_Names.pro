PRO XGKV_Check_Names_CleanUp, tlb
;
...
END ; ****** XGKV_Check_Names_CleanUp ****** ;

PRO XGKV_Check_Names_Quit, event 
;
; Event handler to process 'DONE' button events
;
WIDGET_CONTROL, event.top, /Destroy    ; Destroy widget structure 
   
END ; ****** XGKV_Check_Names_Quit ****** ; 

PRO XGKV_Check_Names_All, event 
;
; Event handler to process 'ALL' button  events
;
; First get user info
;
...
;
; Make 'Checks' array, and check all elements
;
nNames = N_ELEMENTS(info.names)
(*info.checksPtr) = REPLICATE(1, nNames)
checkTxt = REPLICATE(checkmark, nNames)
...
END ; ****** XGKV_Check_Names_All ****** ; 


PRO XGKV_Check_Names_Resize, event
;
; Event handler to resize the XGKV_Check_Names widget
;
...
FUNCTION XGKV_Check_Names, parent, Names=names,  Window_Title=windowTitle, CheckMark=chkmark 
;
;
;	Purpose:
;			
...
;					(Required)
;
;		CheckMark		a string constant containing the character to be
;					displayed in front of the 'checked' names on the
;					list widget.  Defaults to '+'.  (Optional)
;
...
;
checkMark='+'
IF(TypeOf(chkmark) EQ 7) THEN ckeckMark=chkmark
...
ENDCASE
allID  = WIDGET_BUTTON(tlb, VALUE='ALL',  EVENT_PRO='XGKV_Check_Names_All' )
checkTxt = REPLICATE(' ', nNames)
checkedList = checkTxt + names
checks = BYTARR(nNames)
checksPtr = PTR_NEW(checks)
listID   = WIDGET_LIST(tlb, /MULTIPLE, VALUE=checkedList, EVENT_PRO='XGKV_Check_Names_Event', SCR_XSIZE=200, SCR_YSIZE=200)
doneID = WIDGET_BUTTON(tlb, VALUE='DONE', EVENT_PRO='XGKV_Check_Names_Quit')
info={listID:listID, allID:allID, doneID:doneID, names:names, checksPtr:checksPtr, CheckMark:checkMark}
Widget_Control, tlb, Set_UValue=info, /No_Copy ; Save info structure 
Widget_Control, tlb, /Realize			; Put widget on screen
XMANAGER, 'XGKV_Check_Names', tlb, EVENT_HANDLER='XGKV_Check_Names_Resize', CLEANUP="XGKV_Check_Names_CleanUp"
checks = *checksPtr
PTR_FREE, checksPTR
RETURN, checks
