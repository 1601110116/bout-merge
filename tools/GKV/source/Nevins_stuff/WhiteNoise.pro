...
FUNCTION GKVs1D::WhiteNoise, nuStar=nuStar, ChiObs=ChiOBs, _Extra=extra
;
...
ENDIF ELSE BEGIN
	...
	Norm = 1./(Npx*Npy*Ncell_z) ; number of particles in "slice" one grid-cell in z thick
ENDELSE
...
;
; Compute D_noise, Chi_noise
;
IF(TypeOF(Extra) NE 8) THEN EXTRA={aTag:0}
D_noise = nIntensityObj -> Dnoise(	kpMax=Ncoeff.kpplMax, 		$
				kSqHammett=Ncoeff.kSqHammett,		$
				KSqVolume=Ncoeff.kSqVolume,		$
				VolumeHammett=Ncoeff.VolumeHammett,	$
				nuStar=nuStar, ChiObs=ChiOBs,		$
				_Extra=Extra				)

ChiNoise = D_Noise -> Times(1.5)
ChiNoise -> Set, title="!4v!X!Dnoise!N", mnemonic="Chi_noise", units="(!4q!X/L!DT!N)!4q!Xv!Dt!N"
