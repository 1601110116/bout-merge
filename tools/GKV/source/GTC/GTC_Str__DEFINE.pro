...
GTC_Str = {	GTC_Str,					$	; Defining 'GTC_Str' class
			poloidalGrids	: PTR_NEW(),	$	; pointer to array containing poloidal grid dimension on each flux surface
			indexShift		: PTR_NEW(),	$	; pointer to array containing poloidal grid offset for one toroidal circuit
			nToroidalGrids	: 0L,			$	; Number of toroidal grid points in the Pxxxx files
			nRadialGrids	: 0L,			$	; Number of radial (flux?) grids in the Pxxxx files
			start			: PTR_NEW(),	$	; pointer to an array containing the indices to the START  of the poloidal data on each flux surface
			finish		: PTR_NEW(),	$	; pointer to an array containing the indices to the FINISH of the poloidal data on each flux surface
			iota			: PTR_NEW(),	$	; pointer to an array containing the iota profile vs. the radial coordinate
			q			: PTR_NEW(),	$	; pointer to an array containing the    q profile vs. the radial coordinate
			varNames		: PTR_NEW()	}	; pointer to a string array containing names of ALL variables in selected Pxxxx file
