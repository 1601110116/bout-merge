FUNCTION GKVs3D::nPhasePDFs, arg, kMax=kMax
;
...
FOR i=0,nk1-1 DO BEGIN
	...
	FOR j=0,nk2-1 DO BEGIN
		...
		value = COMPLEX(0,-1.*k2)
		Vk  = sk -> Times(value)
		Vk -> Set, title="V!DExB!N", Mnemonic="V_EXB"
		skStar = sk -> EXECUTE("CONJ")
		weight = sk -> AbsSQ()
		output1[i,j] = ak -> phasePDF(skStar)
		output2[i,j] = ak -> phasePDF(vk)
		sk  -> TRASH
		ak  -> TRASH
		vk  -> TRASH
	     skStar -> TRASH
