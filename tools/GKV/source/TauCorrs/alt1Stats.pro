FUNCTION Test_Alt1Stats, arg, _Extra=Extra
;
; arg should be an GKVs1D object array
;
n = N_ELEMENTS(arg)
output = FLTARR(n,2)
FOR i=0,n-1 DO BEGIN
	stats = arg[i] -> alt1Stats(_Extra=Extra)
	output[i,0] = stats.avg
	output[i,1] = stats.avgpm
...
FUNCTION GKVs1D::alt1Stats, lw=lagWindow, _Extra=Extra
;
; Purpose:
;
...
IF(nDims EQ 1) THEN BEGIN
	order = 1
	result = GetKeyWord('order', Extra)
	IF(Query_Integer(result)) THEN order = result 
	dself = self -> detrend(order=order, coeffs=coeffs)
	tValues = *self.grid1.values
	polynomialFit	= POLY(tValues, coeffs)
	pMin = MIN(polyNomialFit, MAX=pMax)
	;var1 = ((pMax - pMin)/2.)^2
	...
	;tTotal = tmax - tmin
	;var1 =    ABS(coeffs[1]* tTotal/2.)^2 	$
	;	+ ABS(coeffs[2]*(tTotal/2.)^2)^2
	; 
	; compute data window
	;
	idw = nPoints/10 > 1
	...
	avgPM = SQRT(dAvgSq + var1)  ;  added correction for any trend in data
