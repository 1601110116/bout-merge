...
PRO GKVsdNull::AxisShift, axis=axisID, debug=d
;
...
;
; NOTE:  This is exact copy of GKVs4D::AxisShift -EW
; Written 6/7/00 by
; W.M. Nevins
;
axis=axisID
...
RETURN 
END ; ****** GKVsdNull::AxisShift ****** ;



FUNCTION NetCDF_Data3, 	FileName=fileName, Path=path, ArrayStyle=array_Style, Threshold=thrshd, Species=species, 	$
			RI=real_or_imaginary, CodeName=CodeNameIN, CodePI=CodePIin, FileID=fileIDin, RunID=runIDin, Debug=d, Stick=stick
;
...
;				from the netCDF file.  Default is 1 (x10^6).  
;
...
;				penultimate line at the lower left hand corner of the plot).  This value will over ride
;				any value found in the NetCDF file. (Optional)
;
;	CodePI			Set to a string constant containing the desired 'CodePI' (ascii field appearing at the
;				last line at the lower left hand corner of the plot).  This value will over ride
;				any value found in the NetCDF file. (Optional)
;
;
...
;				the NetCDF file. (Optional)
;
;	RunID			Set to a string constant containing the desired 'RunID' (ascii field appearing in the 
;				penultimate line at the lower right hand corner of the plot).  This value will over 
;				ride any FileID set within any FileID set within the NetCDF file. (Optional)
;
;	Stick			Set this keyword (i.e., put '/Stick' on the command line) to cause NetCDF_Data to
;				exit with the current working directory set to the directory containing the NetCDF
;				file.  Default is to leave current working directory unchanged. (Optional)
;
;
...
;	6/7/00
;
; Modified 10/9/01 to add FileId and RunID keywords
;
; Rewritten 3/12/09 to handle long datasets E. Wang
;
deBug=0
...
threshold=1000000L ; took out 2 0's ... and put them back
...
IF(N_ELEMENTS(fileName) EQ 0) THEN fileIn=DIALOG_PICKFILE(Path=path, Filter='*.nc', Get_Path=NetCDF_DIR) ELSE fileIn=fileName
ok = FINDFILE(fileIn, Count=nfiles)			; Check if filename points to a valid file
NetCDF_DIR = FILE_DIRNAME(fileIn) + separator
IF (nfiles NE 1) THEN BEGIN				; 	we didn't find a file... 
	MESSAGE, "Couldn't find NetCDF file", /INFORMATIONAL
	RETURN, 0
ENDIF		
...
nGatts=cdfInfo.nGatts						; Number of global attributes in this netCDF file
...
defaultStructure.FileID = defaultName				; FileID defauts to name of NetCDF file
runID_Date = 'NetCDF file read ' + SYSTIME()
...
FOR gattID = 0, nGAtts-1 DO BEGIN
	...
	CASE lgattName OF
		...
		'codeid'	:	defaultStructure.CodeName	= gattValue
		'runid-version'	:	runID_Version			= gattValue
		'runid-date'	:	runID_Date			= 'Code run ' + gattValue
		'runid-case'	:	defaultStructure.RunID		= gattValue
		'runid-boundary' :	runID_Boundary			= gattValue
		'runid-model'	:	defaultStructure.FileID		= gattValue
	...
	ENDCASE
ENDFOR
IF Query_String(runID_Version) THEN defaultStructure.CodeName = defaultStructure.CodeName + " " + runId_Version
;
; Check for CodeName, CodePI, FileID, RunID on command line
;
IF(N_ELEMENTS(CodeNamein) EQ 1) THEN defaultStructure.CodeName=CodeNamein
IF(N_ELEMENTS(  CodePIin) EQ 1) THEN defaultStructure.CodePI  =  CodePIin
IF(N_ELEMENTS(  FileIDin) EQ 1) THEN defaultStructure.FileID  =  FileIDin
IF(N_ELEMENTS(   RunIDin) EQ 1) THEN defaultStructure.RunID   =   RunIDin
...
;
			...
			FOR attID = 0, nAtts-1 DO BEGIN					; Look for Mnemonic, title, units, etc.
				...
				CASE lattName OF
					...
					'units'		:  gridStructure.Units		= STRCOMPRESS(sattValue, /REMOVE_ALL) + '!N!X'
...
FOR varID=0, nVars-1 DO BEGIN
	...
	ENDIF ELSE BEGIN
		...
		Case (varSize GT threshold) OF
			...
			1	:	BEGIN
					varType[varID] = 'long'
					varSave[varID] = 0 ; Saving long variables under a different set of rules than NetCDF_data
...
;
		...
		IF(firstDimName EQ ri) THEN BEGIN
			complex = 1
			varDims = varDims-1						; *** NOTE *** varDims ALREADY takes account of 'complex'file:///usr/share/doc/HTML/en-US/index.html
		ENDIF
;
...
;
		FOR attID=0, nAtts-1 DO BEGIN
			...
			CASE lattName OF
				...
				'units'		:  objStructure.Units 		= STRCOMPRESS(sattValue, /REMOVE_ALL) + '!N!X'
...
;
	...
	;
	ENDIF
	; Handle long variables here =========================================================
DONE2:	; ...
IF (varType[varID] EQ 'long') THEN BEGIN
...
;
    ...
    IF(multiSpecies[varID]) THEN vVarDims = varDims-1 ; *** while vVarDims takes account of BOTH 'complex' and 'multiSpecies'
;
; Create GKVsdNull structure for this variable
;
    objStructure = {GKVsdNull}
;
; Copy default information into structure
;
    
    objStructure.FileID = defaultName
    objStructure.RunID = defaultStructure.RunID
    objStructure.CodeName = defaultStructure.CodeName
    objStructure.CodePI = defaultStructure.CodePI
    
    objStructure.mnemonic = STRCOMPRESS(varName, /REMOVE_ALL)
    objStructure.Title = varName
    objStructure.Indices = PTR_NEW(REPLICATE('*', vVarDims))   
    objStructure.nVars = vVarDims
    objStructure.reali = complex
    objStructure.VariableID = VarID
    objStructure.DirID = NetCDF_DIR
    objStructure.reali = complex
    objStructure.multi = multiSpecies[varID]
    objStructure.multiLoc = -1
;
; Check variable attributes
;
    FOR attID=0, nAtts-1 DO BEGIN
...
;
    IF(multiSpecies[varID]) THEN BEGIN
        objStructure.multi = 1        
;
; First, find 'nspecLoc', the location of 'specIndx' within this variable (note that nspecLoc is an element of [1, VarDims], NOT [0, VarDims-1], or [1, vVarDims])
;
        FOR dimID=0,varDims-1+complex DO IF(STRCMP(dimName[varDimID[dimID]], specIndx)) THEN nspecLoc = dimID+1
        cNspecLoc = nspecLoc - complex ; 'cNspecLoc' points to index in (possibly complex) 'values' array corresponding to 'specIndx'      
        objStructure.multiLoc = nspecLoc
    ENDIF         
...
;
    ...
    ENDIF ELSE  BEGIN
        ...
        FOR j=1, vVarDims DO BEGIN        
            ...
            ENDCASE
        ENDFOR
    ENDELSE ; In the unique case that the long variable is not 3D+time, extra GKVsdNull grids will be copies of previous Grids.
    IF (vVarDims EQ 3) THEN  objStructure.Grid4 = objStructure.Grid3
    IF (vVarDims EQ 2) THEN BEGIN
        objStructure.Grid3 = objStructure.Grid2
        objStructure.Grid4 = objStructure.Grid3
    ENDIF 
    IF (vVarDims EQ 1) THEN BEGIN 
        objStructure.Grid2 = objStructure.Grid1
        objStructure.Grid3 = objStructure.Grid2
        objStructure.Grid4 = objStructure.Grid3
    ENDIF
;
; Make appropriate GKVsd object, and store in gkvObjects array
;
    gkvObjects[iObjs] = OBJ_NEW('GKVsdNull' , objStructure)
;
; If this is a multispecies variable, then several objects must be created
    ...
    IF(multiSpecies[varID]) THEN BEGIN
      ...
      FOR ispecies=1,nspec-1 DO BEGIN
        ...
        title = objStructure.title + '!Ispec ' + speciesStr + '!N'
        specObjs[ispecies] = gkvObjects[iObjs] -> MakeCopy(/NoValues)
        specObjs[ispecies] -> Set, Mnemonic=mnemonic, Title=title          
      ENDFOR
...
FOR i=0,iObjs-1 DO BEGIN
    ...
    tagParts = STR_SEP(tagName, '-')
    nParts = N_ELEMENTS(tagParts)
    IF(nParts GT 1) THEN BEGIN	; Replace '-' with '_' in Tagname
        tagName=tagParts[0]
        FOR iii=1,nParts-1 DO tagName = tagName + '_' + tagParts[iii]
    ENDIF
    CASE i OF
...
;
; Create params file, if gyro netcdf
;

PRINT, "accessing run.out for parameters"
idLen = STRLEN(defaultName)
runout = netCDF_DIR + STRMID(defaultName, 0, idLen-3) + ".run.out"
params = gyro_params(runout)


result = CREATE_STRUCT(result, "params", params)


IF Query_String(runID_Boundary) THEN result = CREATE_STRUCT('Boundary', runID_Boundary, result)
result = CREATE_STRUCT('Name', 'NetCDF_Data', 'RunDate', runID_Date, result)
;
; Clean up ... must free up pointers in the gridstructures
;
FOR dimID = 0, nDims-1 DO  PTR_FREE, (dimGrid[dimID]).values

IF(KEYWORD_SET(Stick)) THEN CD, NetCDF_DIR
RETURN, result
