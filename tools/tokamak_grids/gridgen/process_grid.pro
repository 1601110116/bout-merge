...
PRO process_grid, rz_grid, mesh, output=output, poorquality=poorquality, $
  ...
  print, "bpsign",bpsign
  print, "Bzxy,Brxy[0,ymid]=",Bzxy[0,ymid],Brxy[0,ymid]
  print, "dRxy,dZxy=",Rxy[0,ymid+1] - Rxy[0,ymid-1],Zxy[0,ymid+1] - Zxy[0,ymid-1]
  ...
  IF dot LT 0. THEN BEGIN
    ...
    bpsign = -1.0
  ENDIF ELSE BEGIN
    PRINT, "**** Poloidal field is in the same direction to Grad Theta -> Bp positive"
  ...
  ENDIF ELSE BEGIN
    ...
    FOR i=0, ny-1 DO BEGIN
      pres[*,i] = int_func(bpsign*psixy[*,i], dpdx2[*,i])
  ...
  print,"pitch[0,ymidplane]",pitch[0,ymidplane]  
  ...
  g11 = (Rxy*Bpxy)^2;
  g22 = 1.0/hthe^2;
  g33 = I^2*g11 + Bxy*Bxy/g11;
  g12 = 0.0;
  g13 = -I*g11;
  g23 = -bpsign*Btxy/(hthe*Bpxy*Rxy);
  ...
  g_33 = Rxy^2;
  g_12 = bpsign*Btxy*hthe*I*Rxy/Bpxy;
  g_13 = I*Rxy*Rxy;
  g_23 = bpsign*Btxy*hthe*Rxy/Bpxy;

  ; check to make sure jacobian is good
  Jcheck = bpsign / sqrt(g11*g22*g33 + 2.0*g12*g13*g23 - g11*g23*g23 - g22*g13*g13 - g33*g12*g12);

  PRINT, "MAX(J-Jcheck) = ", MAX(ABS(J-Jcheck))  
  ...
  ;s = file_write(handle, "ShiftAngle", bpsign*qloop)
  ;s = file_write(handle, "zShift", bpsign*qinty)
  ;s = file_write(handle, "pol_angle", bpsign*pol_angle)
  ...
  s = file_write(handle, "bpsign", bpsign)
