...
PRO process_grid, rz_grid, mesh, output=output, poorquality=poorquality, $
  ...
  IF rz_grid.cpasma LT 0 THEN BEGIN
    PRINT, "CATION: negative plasma current detected"
;    bpsign=-1
;    psixy_eq=-psixy_eq
;    PRINT, rz_grid.npsigrid
;    PRINT, psixy_eq[*,ymid]
  ENDIF
  ...
  ENDIF ELSE BEGIN
    ...
    FOR i=0, ny-1 DO BEGIN
      pres[*,i] = int_func(bpsign*psixy[*,i], dpdx2[*,i], /simple)
...
;   dyshiftdy = dfdy(yshift,yxy,mesh)
  ...
  PRINT, "pitch anagle at outboard midplane:",pitch[0,ymid]
