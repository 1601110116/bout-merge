scriptID("$Id: east.bas,v 1.1 2008/05/16 21:06:33 meyer Exp $")

corsica   # put corsica on top

# The followg parameters control the disposition of output...
integer east_plot = [ 1, 1, 1, 1, 1, 1, 1, 1] # == 0 to turn off plots
integer east_save = [ 1, 1] # == 0 to disable save-file writes
logical east_debug = false # If true, call debugger before each direct-eq run
debug = yes
...
MDEF east() =
  ...
  if (trim(gname) == "help") then
    << " "
    << "Generate a Corsica equilibrium (and inverse equilibrium) to match a EAST EFIT"
    << "equilibrium as defined in EQDSK g- and a-file pairs."
    << " "
    << "   east(eqdsk_g-file;confirm,fix_edge,make_inverse,msrf,thetac,inv_k,inv_p)"
    << " "
    << "where..."
    ...
    << " "
    << "Logical global variable ""east_debug"" can be used to ""call debugger"" before each"
    << "direct-solve run."
    << " "
  ...
  elseif (basopen(gname,"i") == -1) then
    ...
    elseif (nprob == 0) # All ok, load generic equilibrium for this grid size...
      character*128 restore_file = trim("east"//format(nw_eqdsk,0)//"x" \
                                    //format(nh_eqdsk,0)//".sav")
      Comment("Restoring generic equilibrium: "//trim(restore_file))
      if(basopen(restore_file,"i")<>0) restore_file="east_snl.sav"
  ...
  if (exceptions == 0) then
    call eastf(gname,confirm,fix_edge,make_inverse,new_msrf,new_thetac)
  endif
  forget gname, confirm, fix_edge, make_inverse, new_msrf, new_thetac, exceptions

MEND # east

function eastf(gname,confirm,fix_edge,make_inverse,new_msrf,new_thetac)
  ...
  if (basopen(gname,"i") == -1) then
    ...
    call seteastid(shotName,shotTime)
    ...
    nplates = 0
    do i = 1,g_limitr-2
      dsq = (g_xylim(2*(i+1)-1) - g_xylim(2*i-1))**2 + \
            (g_xylim(2*(i+1)) - g_xylim(2*i))**2
      if (dsq > 0) then
        nplates = nplates + 1
        rplate(nplates,1) = g_xylim(2*i-1)
        zplate(nplates,1) = g_xylim(2*i)
        rplate(nplates,2) = g_xylim(2*(i+1)-1)
        zplate(nplates,2) = g_xylim(2*(i+1))
      endif
    enddo
    nplates = nplates + 1
    rplate(nplates,1) = g_xylim(2*nplates-1)
    zplate(nplates,1) = g_xylim(2*nplates)
    rplate(nplates,2) = g_xylim(1)
    zplate(nplates,2) = g_xylim(2)
    rplate = 100 * rplate
    zplate = 100 * zplate
    if(nplates<>0) then
      do i=nplates-1,1,-1
        if(rplate(i,1)<>rplate(i+1,1) | zplate(i,1)<>zplate(i+1,1)) break
      enddo
      if(i<>nplates-1)then
        rplate(2:nplates,1)=rplate(1:nplates-1,2)
        zplate(2:nplates,1)=zplate(1:nplates-1,2)
      endif
#      teq_sync
      limw_plates # gets (rlimw,zlimw) for limiting wall from plates
    endif
    ...
    package eq
    crdn = 2200 # Appropriate for EAST
    nbd = 0
    ...
    wvert=1
    # Refine constraints...
    rxpr = [min(rplate)+dr_factor*dr,200]
    ...
    if (sum(east_plot(1:8)) <> 0 & confirm <>0) then
      Comment("Check free-boundary equilibrium...")
      win on east-comparison
    endif
    if (east_plot(1) <> 0) then
      pb; pause("Plasma boundary")
    endif
    if (east_plot(2) <> 0) then
      ppprime; pause("Pressure")
    endif
    if (east_plot(3) <> 0) then
      pffprime; pause("Toroidal flux function")
    endif
    if (east_plot(4) <> 0) then
      pq; pause("q-profile")
    endif
    if (east_plot(5) <> 0) then
      peastcc; pause("Coil currents")
    endif
    if (east_plot(6) <> 0) then
      layout; pause("Configuration")
    endif
    # Save the direct-solve equilibrium...
    if (east_save(1) <> 0) then
      saveq(save_file)
    ...
    if (make_inverse <> 0) then # Make an inverse equilibrium save-file...
      ...
      if(east_plot(7) <> 0) then
        contour; pause("Configuration")
      endif
      if(east_plot(8) <> 0) then
        testGS; pause("GS error estimate")  
      endif

      if (east_save(2) <> 0) then
        saveq(inv_save_file)
  ...
  if (sum(east_plot(1:7)) <> 0 & confirm <>0) then
    win close east-comparison
  endif

endf # eastf

function eastplots(;p1,p2,p3,p4,p5,p6,p7)

  default(p1) = east_plot(1)
  default(p2) = east_plot(2)
  default(p3) = east_plot(3)
  default(p4) = east_plot(4)
  default(p5) = east_plot(5)
  default(p6) = east_plot(6)
  default(p7) = east_plot(7)

  east_plot = [p1,p2,p3,p4,p5,p6,p7]
  east_plot = where(east_plot <> 0, 1, east_plot)

  remark "PLOT SELECTION     ON/OFF"
  remark " (1) Equilibrium   "//format(east_plot(1),3)
  remark " (2) p' profile    "//format(east_plot(2),3)
  remark " (3) FF' profile   "//format(east_plot(3),3)
  remark " (4) q profile     "//format(east_plot(4),3)
  remark " (5) Coil currents "//format(east_plot(5),3)
  remark " (6) Configuration "//format(east_plot(6),3)
  remark " (7) Inverse eq.   "//format(east_plot(7),3)
...
function peastcc(;ordering) # Plot coil current comparison
 default(ordering) = 1 # == 0 for poloidal ordering of coil currents
                       # == 1 for EAST ordering of coil currents
 if (~exists("nfcc")) then
   Comment("Coil currents from EFIT are not available")
   return
 endif

 integer n,iceast(nfcc)
 real cceast(nfcc),cceast_efit(nfcc)
 ...
 if (ordering  ==  0) then
   ...
   iceast = [ic(1:5),ic(8:9),ic(7),ic(6),ic(10:14),ic(17:nfcc),ic(16),ic(15)]
   cceast = 1000*[cc(1:5),cc(8:9),cc(7),cc(6),cc(10:14),cc(17:nfcc), \
                cc(16),cc(15)]
   cceast_efit = 1000*[ccefit(1:5),ccefit(8:9),ccefit(7),ccefit(6), \
                     ccefit(10:14),ccefit(17:nfcc),ccefit(16),ccefit(15)]
 else
   titles "TEQ (curve) and EFIT (X) F-coil currents" \
          "EAST F-coil number","NIcoil   [kAt]"
   iceast = ic(1:nfcc)
   cceast = 1000*cc(1:nfcc)
   cceast_efit = 1000*ccefit(1:nfcc)
 endif
 plot cceast thick=2 color=orangered
 plot cceast_efit mark cross marksize 2
 plot 0.0*ones(nfcc) style dotted
 do n = 1,nfcc
   if (iceast(n) == 0) plot cceast(n),n mark=circle color=yellow
 enddo

 # E-coil currents...
 integer iceeast(6)
 real cc_ecoil(6), cc_efit_ecoil(6)
 iceeast(1) = ic( 19); cc_ecoil(1) = cc( 19); cc_efit_ecoil(1) = ccefit( 19)
 iceeast(2) = ic( 20); cc_ecoil(2) = cc( 20); cc_efit_ecoil(2) = ccefit( 20)
 iceeast(3) = ic( 67); cc_ecoil(3) = cc( 67); cc_efit_ecoil(3) = ccefit( 67)
 iceeast(4) = ic(128); cc_ecoil(4) = cc(128); cc_efit_ecoil(4) = ccefit(128)
 iceeast(5) = ic(132); cc_ecoil(5) = cc(132); cc_efit_ecoil(5) = ccefit(132)
 iceeast(6) = ic( 71); cc_ecoil(6) = cc( 71); cc_efit_ecoil(6) = ccefit( 71)
 ezcquad(34)
 frame 0 7
 titles "TEQ (curve) and EFIT (X) E-coil circuit currents" \
        "EAST E-coil circuit number","NIcoil   [kAt]"
 ...
 do n = 1,6
   if(iceeast(n) == 0) plot 1000*cc_ecoil(n),n mark=circle color=yellow
 enddo
...
function testGS
  real noint(2)=-999
  if(~exists("qsave_")) then
    global real qsave_=qsave,jtsave_=jtsave
  elseif(length(qsave_)<>msrf) then
    global real qsave_=qsave,jtsave_=jtsave
  endif
  real norm_
  norm_=(max(abs(cos1))+max(abs(cos2))+max(abs(cos3)))
  norm_=abs(cusrf(msrf)/dpsi0)
  cos0=-cusrf/frsrf/.4/pi  
  spline(psrf,cos0,msrf,splbdry,splbdry,&cos3,cos3,cos3,noint)
  cos3=cos3*frsrf**2/2/pi/qsrf
  if(which_way==-2) then 
    toggle_eq
    cos0 = phibar
    spline(phibar,cusrf,msrf,splbdry,splbdry,&cos1,cos1,cos1,noint)
    cos1=cos1/dphi0
    cos2=2*pi*fpsrf+2*vpsrf*pdsrf/qsrf
  else
    cos0 = psibar
    spline(psrf,cusrf,msrf,splbdry,splbdry,&cos1,cos1,cos1,noint)
    cos2=2*pi*qsrf*fpsrf+2*vpsrf*pdsrf
  endif
  cos3=(cos1+cos2)/norm_
  StartFrame
    ezctitfr = 0.06
    ezcquad(12)
      setyminor(8)
      titles probid " " "GS error"
#BASISBUG ftext "error in surface average GS equation" 0.58 0.6,,,0
      frame -.01,1.01,,1
      plot cos3 cos0 color red scale linlog
    ezcquad(34)
      setyminor(1)
      if(which_way==-2) then
        titles "thetac = "//format(thetac,0,2,2) \
        "phibar" "ohmic current [A/cm]"
      else
        titles "thetac = "//format(thetac,0,2,2) \
        "psibar" "ohmic current [A/cm]"
      endif
      frame ,1.01
      if (inv_k == 0) then
        ftext "q-profile input to inverse solver" 0.58 0.16,,,0
        ftext "jtsave_ (red), jtsrf (green)" 0.58 0.13,,,0
        plot jtsave_ cos0 color orangered
        plot jtsrf cos0 color springgreen style dotted
      elseif (inv_k == 1) then
        ftext "FF' input to inverse solver" 0.58 0.16,,,0
        ftext "qsave_ (red), qsrf (green)" 0.58 0.13,,,0
        plot qsave_ cos0 color orangered
        plot qsrf cos0 color springgreen style dotted
      elseif (inv_k == 3) then
        ftext "jpar-profile input to inverse solver" 0.58 0.16,,,0
        ftext "qsave_ (red), qsrf (green)" 0.58 0.13,,,0
        plot qsave_ cos0 color orangered
        plot qsrf cos0 color springgreen style dotted
      endif
...
function afiles(;fname)

  ## Return a summary of all a-files in the current directory, or write to
  ## the named filename.

  default(fname) = "stdout"

  global chameleon afilelist = filelist("a[0-9]*")
  if (tolower(afilelist(1)) == "none") return

  character*1 gFile = " "
  integer orphans = 0

  if (fname <> "stdout") then
    chameleon io = basopen(fname, "w")
  else
    chameleon io = stdout
  endif
  integer npad = strlen(afilelist(1)) - 5
  character*npad pad = " "
  io << "    A-FILE" << pad << "LIMLOC   RXPT     ZXPT     SSEP"
  integer i
  do i = 1,length(afilelist)
    call read_aeqdsk(afilelist(i))
    gFile = " "
    if (basopen("g"//substr(afilelist(i),2,strlen(afilelist(i))-1), "i") <> 0) then
      gFile = "*"
      orphans = orphans + 1
    endif
    if (exists("limloc")) then
      if (limloc <> "DN" & substr(limloc,1,2) <> "SN") then
        io << format(i,3)//gFile//afilelist(i)//" "//limloc
      else
        if (limloc == "DN") then
          io << format(i,3)//gFile//afilelist(i)//" "//limloc//" " \
              //format(rxpt_efit,9,2,1)//format(zxpt_efit,9,2,1) \
              //format(dsep_efit,9,3,1)//"*"
        else
          io << format(i,3)//gFile//afilelist(i)//" "//limloc//" " \
              //format(rxpt_efit,9,2,1)//format(zxpt_efit,9,2,1) \
              //format(dsep_efit,9,3,1)
  ...
  if (orphans > 0) then
    if (fname <> "stdout") then
      io << return << "   * " << format(orphans,0) << " g-file(s) missing."
      stdout << global.bell << format(orphans,0) << " g-file(s) missing."
    else
      io << global.bell << "   * " << format(orphans,0) << " g-file(s) missing."
    endif
  endif
...
function gfiles(;shot_number)

  # Display EQDSK g-files

  default(shot_number) = -1

  # Get list of all g-files in current directory...
  if (~exists("gfilelist")) then
    global chameleon gfilelist = filelist("g[0-9]*.[0-9]*")
  endif
  if (gfilelist(1) == "None") then
    return 0
  endif

  # Display summary of all g-files...
  if (shot_number == -1) then
    integer n, n1, n2, count, shot_sets
    n1 = 1
    chameleon shot_number = substr(gfilelist(n1),2,6)
    << return << "EQDSK g-file summary..."
    do n = 2,length(gfilelist)
      if (substr(gfilelist(n),2,6) <> shot_number) then
        shot_sets = shot_sets + 1
        n2 = n - 1
        count = count + n2 - n1 + 1
        chameleon str = " ("//format(n2-n1+1,0)//")"
        << "   "//format(n1,-3)//"-"//format(n2,-3)//": "//shot_number//str
        n1 = n
        shot_number = substr(gfilelist(n),2,6)
      endif
    enddo
    shot_sets = shot_sets + 1
    n2 = length(gfilelist)
    ...
    << "   "//format(n1,-3)//"-"//format(n2,-3)//": "//shot_number//str
    if (shot_sets > 1) then
      << "No. unique shots:  " << format(shot_sets,0)
      << "Total no. g-files: " << format(count,0)
    endif
    return shot_sets
  else # Display g-file time-slices for specific shot
    if (shot_number == 0) then
      chameleon shotNumber = substr(gfilelist(1),2,6)
    else
      chameleon shotNumber = format(shot_number,-6)
    endif

    # Set output width...
    integer rows, columns
    call basisexe("stty | grep columns > .stty")
    integer io = basopen(".stty","r")
    io >> rows >> columns
    call basclose(io)
    call basisexe("rm .stty")
    integer fw = log10(length(gfilelist)) + 3
    integer ncols = columns / (fw + 6)

    integer i
    << " "
    << "Shot " << shotNumber << " time-slice-number:time [ms]..."
    autocr = no
    do i = 1,length(gfilelist)
      if (substr(gfilelist(i),2,6) == shotNumber) then
        << format(i,fw) << ":" << substr(gfilelist(i),9,5)
    ...
    enddo
    << return
    if (mod(length(gfilelist),ncols) <> 0) then
      << return
    endif
    autocr = yes
  endif
...
endf # gfiles
  ...
  function seteastid(name,time)

    probid = "EAST   shot #"//substr(name,1,6)//" @ " \
           // format(time,0,3,1) // " s"
    if (exists("limloc")) then
